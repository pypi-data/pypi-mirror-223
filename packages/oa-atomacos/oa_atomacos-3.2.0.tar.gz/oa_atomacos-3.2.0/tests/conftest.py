...
def pytest_exception_interact(node, call, report):
    if os.getenv("CI", False):
        ...
        for app in oa_atomacos.NativeUIElement.getRunningApps():
            pid = app.processIdentifier()
            app_ref = oa_atomacos.NativeUIElement.from_pid(pid)
            print("\n%s" % app_ref)
            for child in app_ref._generateChildren(recursive=True):
                ...
                for attribute in child.ax_attributes:
                    ...
                    except oa_atomacos.errors.AXError as e:
                        print("Issue getting %s: %s" % (attribute, e))


def app_by_bid(bid):
    running_apps = list(oa_atomacos.NativeUIElement.getRunningApps())
    if not any([bid in str(app) for app in running_apps]):
        oa_atomacos.launchAppByBundleId(bid)
    app = oa_atomacos.NativeUIElement.from_bundle_id(bid)
...
@pytest.fixture
def axconverter():
    return _converter.Converter(oa_atomacos.NativeUIElement)
