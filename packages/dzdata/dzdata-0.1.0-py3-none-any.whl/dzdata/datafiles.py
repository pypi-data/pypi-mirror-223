...
def load_datafiles(path: str | pathlib.Path):
    path = Path(path)
    if not path.exists() and (_path := Path(f"{path}.zip")).exists():
        path = _path
    return DatafilesBase.load(path)


# The list of registered datatype (datatype that can be loaded). Automatically
# populated when a class inherit from DatafilesBase.
__datafiles_subclasses__: dict = {}


class NotADatafilesException(Exception):
...
class DatafilesBase:
    ...
    def load(cls, file: PathLike):
        """ Load the DatafilesBase subclass from 'file'"""
        if cls._is_zip(file):
    ...
    def _load(cls, root):
        ...
        if f"{INFO_FILENAME}" not in name_dict:
            raise NotADatafilesException(root)
