...
A good plan should start with the product requirements. Certainly, a product’s datasheet is a good place to start. The plan should have one line item for each declared requirement, claim in the datasheet body text or line of the datasheet electrical table.

It is a good idea to give each item (row in the plan) a unique identifier or *key*. A human-meaningful string is preferential for human comprehension and can be parsed by a computer as well. Other evaluation plan key schemas are possible of course.
...
By scripting each test individually, the system configuration and what was done to the d.u.t. to collect the data becomes a matter of record. One of the desirable features of Python is its readability. Therefore Python scripts make an excellent record of the procedure undertaken. This can be invaluable after the data collection is complete and is being reviewed with peers for accuracy and validity.
...
The **modules** section contains project-specific infrastructure files that PyICe needs to facilitate high level goals such environmental testing (e.g. temperature forcing on the outer loop of test suites), test results gathering and reporting, automated production to bench correlation, automated report generation, etc.
