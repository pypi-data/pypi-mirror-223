...
def scan_window() -> None:
    ...
    with dpg.window(
        ...
        headers = ["SSID", "MAC", "CH", "RSSI", "FREQ",  "SECURITY"]
...
def sort_results(sender, app_data, user_data):
    ...
    match sender:
        case "SSID":
            scan_data = sorted(scan_data, key=lambda x: x.get(
                "ssid"), reverse=order_query)
        case "MAC":
            scan_data = sorted(scan_data, key=lambda x: x.get(
                "mac"), reverse=order_query)
        case "CH":
            scan_data = sorted(scan_data, key=lambda x: x.get(
                "channel"), reverse=order_query)
        case "FREQ":
            scan_data = sorted(scan_data, key=lambda x: x.get(
                "frequency"), reverse=order_query)
        case "RSSI":
            scan_data = sorted(scan_data, key=lambda x: x.get(
                "signal"), reverse=not order_query)
        case "SECURITY":
            scan_data = sorted(scan_data, key=lambda x: x.get(
                "security"), reverse=order_query)
        case _:
...
def create_scan_results_table(scan_data: list[list]) -> None:
    """Create the table of scan results."""
...
def return_indicated_results() -> dict[str, dict[str, str]]:
    ...
    [
        loggei.warning("Selected scan results: %s", i)
...
def chase(
    ...
    chase_freqs: dict[int, float] = {
        i: float(j) for i, j in enumerate(
            chase_freqs,
            start=1
        )
    }
...
def wifi_kill_all() -> None:
    ...
    threaded_wifi_scan(
        wifi_action=send_to_n_cards,
        vital_data=freq_and_strength
    )
...
def send_to_n_cards(freq_and_strength: dict[int, float]) -> None:
    ...
    _ = [
        (
            ...
            port.pop(0) if (i % 8 if i % 8 else 8) == 1 and isinstance(
                port,
                list
            ) else None,
            ...
            loggei.warning("Port: %s", port[0]
                           if isinstance(port, list) else port),
            # device_refresh() if i == 1 else None,

            # Since the list is immediately popped, put it back at the end
            port.append(
                first_port_popped
            ) if first_port_popped not in port else None
        )
...
def threaded_wifi_scan(
    ...
    with dpg.window(
        ...
        dpg.add_text(
            tag="scan_text",
            default_value=text_to_display,
            pos=(300, 265)
        )

    wifi = ThreadWithReturnValue(
        target=wifi_action,
        args=(vital_data, )
    )
    wifi.start()
    ...
    while wifi.is_alive():
        ...
        dpg.configure_item(
            item="scan_text",
            default_value=text_to_display + "." * count
        )
