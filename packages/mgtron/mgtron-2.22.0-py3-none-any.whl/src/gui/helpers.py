...
def callstack_helper(
    ...
    loggey.info("Channel %s Information Sent", channel)
    ...
    data_vehicle.change_bandwidth(
        channel=channel,
        percentage=int(dpg.get_value(f"bandwidth_{channel}")),
        teensy_port=get_device_path_from_serial_number(),
    )

    data_vehicle.change_freq(
        channel=channel,
        freq=float(dpg.get_value(f"freq_{channel}")),
        teensy_port=get_device_path_from_serial_number(),
    )
...
def launch_api_server(path: pathlib.Path) -> None:
    ...
    if exit_code != 0:
        loggey.warning(msg=f"API server is likely running: {exit_code}")
