...
class ApplyService:
    ...
    async def plan_apps(self, config_: schemas.ProjectManifest) -> AppSpecPlan:
        ...
        custom_apps = [app for app in config_.apps if isinstance(app, schemas.SlingshotCustomAppSpec)]
        sessions = [app for app in config_.apps if isinstance(app, schemas.SessionAppSpec)]
        redis_apps = [app for app in config_.apps if isinstance(app, schemas.RedisAppSpec)]
        (
            custom_app_specs_to_create,
            custom_app_specs_to_update,
            custom_app_specs_to_delete,
            custom_app_change_msgs,
        ) = _diff_existing_app_yaml_specs(existing_app_specs, custom_apps, schemas.AppType.CUSTOM)
        (
            ...
            existing_app_specs, sessions, schemas.AppType.CUSTOM, schemas.AppSubType.SESSION
        )
        (
            redis_app_specs_to_create,
            redis_app_specs_to_update,
            redis_app_specs_to_delete,
            redis_app_change_msgs,
        ) = _diff_existing_app_yaml_specs(
            existing_app_specs, redis_apps, schemas.AppType.CUSTOM, schemas.AppSubType.REDIS
        )
        (
            run_specs_to_create,
            run_specs_to_update,
            run_specs_to_delete,
            run_change_msgs,
        ) = _diff_existing_app_yaml_specs(existing_app_specs, config_.runs, schemas.AppType.RUN)
        (
            deployment_specs_to_create,
            deployment_specs_to_update,
            deployment_specs_to_delete,
            deployment_change_msgs,
        ) = _diff_existing_app_yaml_specs(existing_app_specs, config_.deployments, schemas.AppType.DEPLOYMENT)
    ...
    async def apply_app_specs(self, app_spec_plan: AppSpecPlan, env_spec_name_to_id: dict[str, str]) -> None:
        ...
        for spec, spec_id in app_specs_to_update:
            try:
                ...
                raise SlingshotException(f"Failed to update: {e}") from e
...
async def remote_project_manifest(sdk: "SlingshotSDK", raise_on_error: bool = True) -> schemas.ProjectManifest:
    ...
    for spec in existing_app_specs:
        ...
        mounts = [
            TypeAdapter(schemas.MountSpecUnion).validate_python(
                {"mode": i.mode, "name": i.name, "path": i.path, "tag": i.tag}
            )
