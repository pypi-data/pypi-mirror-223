...
class Unet3dMediaPipe(MediaPipe):
    ...
    def __init__(self, a_device=None, a_batch_size=1, a_prefetch_count=1, a_num_instances=1, a_instance_id=0, a_pipeline=None, a_enable_zoom=False, a_num_threads=1, **kwargs):
        ...
        if (self.pipeName == "TrainPipeline"):
            ...
            #a_batch_size
            self.is_hflip = fn.CoinFlip(seed=get_random_seed(), dtype=dt.INT8, device='cpu')#[[batch_size]]
            self.hflip_reshape_p = fn.Reshape(size=[self.batch_size],
                                                tensorDim=1,
                                                layout='',
                                                dtype=dt.UINT8, device='hpu')#[batch_size]
            self.img_hflip = fn.RandomFlip(horizontal=1, dtype=dt.FLOAT32, device='hpu') #img [W,H,D*C,N]
            self.lbl_hflip = fn.RandomFlip(horizontal=1, dtype=dt.UINT8, device='hpu') #lbl [W,H,D*C,N]

            # ------------V Flip------------------------------
            # random vertical flip node
            #a_batch_size
            self.is_vflip = fn.CoinFlip(seed=get_random_seed(), dtype=dt.INT8, device='cpu')#[[batch_size]]
            self.vflip_reshape_p = fn.Reshape(size=[self.batch_size],
                                                tensorDim=1,
                                                layout='',
                                                dtype=dt.UINT8, device='hpu')#[batch_size]
            self.img_vflip = fn.RandomFlip(vertical=1, dtype=dt.FLOAT32, device='hpu')#img [W,H,D*C,N]
            self.lbl_vflip = fn.RandomFlip(vertical=1, dtype=dt.UINT8, device='hpu')#lbl [W,H,D*C,N]
            ...
            if self.dim == 3:
                ...
                self.is_dflip = fn.CoinFlip(seed=get_random_seed(), dtype=dt.INT8, device='cpu')#[[batch_size]]
                ##a_batch_size
                self.dflip_reshape_p = fn.Reshape(size=[self.batch_size],
                                                tensorDim=1,
                                                layout='',
                                                dtype=dt.UINT8, device='hpu')#[batch_size]
                self.img_dflip = fn.RandomFlip(vertical=1, dtype=dt.FLOAT32, device='hpu')#img [W*H,D,C,N]
                self.lbl_dflip = fn.RandomFlip(vertical=1, dtype=dt.UINT8, device='hpu')#lbl [W*H,D,C,N]
            ...
            self.where_gnoise = fn.Where(dtype=dt.FLOAT32, device="cpu")#[[batch_size]]
            ##a_batch_size
            ...
            self.add = fn.Add(dtype=dt.FLOAT32, device='hpu')#img [W,H,D*C,N]
            ...
            self.is_brightness = fn.CoinFlip(
                seed=get_random_seed(), dtype=dt.INT8, device='cpu')
            self.where_brigtness = fn.Where(dtype=dt.FLOAT32, device='cpu')#[[batch_size]]
            ##a_batch_size
            ...
            self.brightness = fn.Brightness(dtype=dt.FLOAT32, device='hpu')#img [W,H,D*C,N]
            # ------------Contrast------------------------------
            ...
            self.min0 = fn.ReduceMin(
                reductionDimension=0, dtype=dt.FLOAT32, device='hpu')
            self.max0 = fn.ReduceMax(
                reductionDimension=0, dtype=dt.FLOAT32, device='hpu')
            self.min1 = fn.ReduceMin(
                reductionDimension=1, dtype=dt.FLOAT32, device='hpu')
            self.max1 = fn.ReduceMax(
                reductionDimension=1, dtype=dt.FLOAT32, device='hpu')
            self.min2 = fn.ReduceMin(
                reductionDimension=2, dtype=dt.FLOAT32, device='hpu')
            self.max2 = fn.ReduceMax(
                reductionDimension=2, dtype=dt.FLOAT32, device='hpu')
            self.min3 = fn.ReduceMin(
                reductionDimension=3, dtype=dt.FLOAT32, device='hpu')
            self.max3 = fn.ReduceMax(
                reductionDimension=3, dtype=dt.FLOAT32, device='hpu')
    ...
    def definegraph(self):
        ...
        if (self.pipeName == "TrainPipeline"):
            ...
            if self.augment == True:
                ...
                img, lbl = self.img_hflip(img, h_predicate), self.lbl_hflip(lbl, h_predicate)
                # ------------V Flip------------------------------
                ...
                min3_c, min3_i = self.min3(img)
                max3_c, max3_i = self.max3(img)
                min2_c, min2_i = self.min2(min3_c)
                max2_c, max2_i = self.max2(max3_c)
                min1_c, min1_i = self.min1(min2_c)
                max1_c, max1_i = self.max1(max2_c)
                min0_c, min_i = self.min0(min1_c)
                max0_c, max_i = self.max0(max1_c)
