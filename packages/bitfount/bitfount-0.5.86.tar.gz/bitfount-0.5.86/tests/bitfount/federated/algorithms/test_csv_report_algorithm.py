...
class TestCSVAlgorithm:
    ...
    def test_worker_side_with_task_id(
        ...
        df = csv_report_algo.run(
            data=datasource, results_df=[results_df, results_df2], task_id="blah"
        )
    ...
    def test_worker_run_method_doesnt_append_to_csv_without_task_id(
        ...
        """
        csv_report_algo = CSVReportAlgorithm(save_path=str(tmp_path)).worker()
        df = csv_report_algo.run(data=datasource, results_df=[results_df, results_df2])
    ...
    def test_worker_side_filter_TypeError(
        ...
        df = csv_report_algo.run(
            data=datasource, results_df=[results_df, results_df2, aux_df]
        )
    ...
    def test_worker_side_no_test_idxs(
        ...
        if not datasource.iterable:
            with pytest.raises(AlgorithmError):
                csv_report_algo.run(
                    data=datasource,
                    results_df=[results_df, results_df2],
                    task_id="blah",
                )
