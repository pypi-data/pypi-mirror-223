...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def run(
        ...
        Args:
            data: The datasource whose rows are to be processed.
        ...
        if data._test_idxs is None:
            if not data.iterable:
                raise DataSourceError("Datasource has no test set, cannot produce CSV.")
            else:
                # If the datasource is iterable, we need to use the `get_filenames`
                # method of the data splitter to get the filenames that correspond to
                # the test set. These filenames are then used to filter the dataframe.
                data_splitter = (
                    data.data_splitter if data.data_splitter else PercentageSplitter()
                )
                data = cast(FileSystemIterableSource, data)
            ...
            df = data.data.loc[data._test_idxs].reset_index(drop=True)

        if isinstance(results_df, list):
            aux_results_df = results_df[0]
            for index in range(1, len(results_df)):
                aux_results_df = pd.concat([aux_results_df, results_df[index]], axis=1)
        else:
        ...
        if self.original_cols is not None:
            drop_cols = [
                col for col in data.data.columns if col not in self.original_cols
            ]
        ...
        df.round(decimals=2).to_csv(csv_path, mode="a", header=not csv_path.exists())
