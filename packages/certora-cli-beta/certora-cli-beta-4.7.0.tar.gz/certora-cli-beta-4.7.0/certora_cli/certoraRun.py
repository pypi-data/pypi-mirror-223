...
from Shared.certoraUtils import run_jar_cmd, get_certora_sources_dir
...
from Shared.certoraUtils import CertoraUserInputError, TestResultsReady
from Shared.certoraUtils import get_build_dir, safe_create_dir
...
def run_certora(args: List[str], is_library: bool = False) -> Optional[CertoraRunResult]:
    ...
    safe_create_dir(get_build_dir(), revert=False)
        ...
        if context.mode == Mode.SOLANA:
            ...
            if not context.build_only and exit_code == 0:  # either we skipped building (TAC MODE) or build succeeded
                if context.local:
                    ...
                    print(f"Verifier run command:\n {check_cmd}", flush=True)
                    run_result = \
                        run_jar_cmd(check_cmd, compare_with_expected_file,
                                    logger_topic="verification", print_output=True)
                    emv_dir = latest_emv_dir()
                    return_value = CertoraRunResult(str(emv_dir) if emv_dir else None, True, get_certora_sources_dir())
                    if run_result != 0:
                    ...
                    if not context.typecheck_only and exit_code == 0:  # Local typechecking either succeeded or skipped
                        ...
                        if cloud_verifier.statusUrl:
                            return_value = CertoraRunResult(cloud_verifier.statusUrl, False, get_certora_sources_dir())
                        if not result:
                            exit_code = 1
    except TestResultsReady:
        raise
