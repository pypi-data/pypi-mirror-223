...
class REST:
    ...
    def get(self, uri: str, params: dict = {}, headers: dict = {}) -> dict:
        ...
        try:
            ...
            if response['response'] is None:
                print(f"REST Error: {uri} - {response['status_code']} - {response['description']}",
                      file=sys.stderr)
                return {}
            return response['response']

    def post(self, uri: str, params: dict = {}, headers: dict = {}) -> dict:
        uri = f"/{uri}" if uri.find('/') != 0 else uri
        if uri.find(self.base_url) > 0:
            uri = uri
        else:
        ...
        try:
            ...
            if 'response' not in response.keys() or response['response'] is None:
                print(f"REST Error: {uri} - {response} - {params}",
                      file=sys.stderr)
                return {}
            return response['response']
