...
def upsert_from_path(
    ...
    if set_row_creation_time:
        rct_cols = {m.metric_metadata.dataset_time_column_name for m in mil.metrics if
                    m.metric_metadata.dataset_time_column_name}
...
def dbt_tests_to_metrics(
    ...
    for t, tests in dbt_tables.items():
        try:
            ...
            log.error(f"Table {t} was not found in {source.name}.{schema_name}. Check that Bigeye has access to the "
                      f"dataset.")
            continue
        ...
        for test in tests:
            ...
            test_column = build_fq_name(source.name, schema_name, t, test.parameters[0].column_name)
    ...
    if to_bigconfig:
        collection: SimpleCollection = SimpleCollection(name='DBT Tests',
                                                        description='Monitoring results of dbt tests.')
        bigconfig = metric_controller.tag_deployments_to_bigconfig(tag_deployments=tag_deployments,
                                                                   collection=collection)
        file_name = collection.name.lower().replace(" ", "_")
        output = bigconfig.save(output_path=output_path, default_file_name=f'{file_name}.bigconfig',
