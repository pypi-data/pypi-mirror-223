from numpy import linspace, unique, median, percentile, mean, var
        ...
        for i in range(len(self.observation)):
            for k, n in enumerate([self.observation, self.hyppighed, self.frekvens, self.kumuleret_frekvens]):
                if hasattr(n[i], "__iter__") and len(n[i]) == 2:
                    s += f"]{n[i][0]:>{floor((len(names[k])-3)/2)}.1f};{n[i][1]:>{ceil((len(names[k])-3)/2)}.1f}] | "
                elif type(n[i]) == float:
                    s += f"{n[i]:>{len(names[k])}.3f} | "
                else:
                    s += f"{n[i]:>{len(names[k])}} | "
            s += "\n"
        return s
...
def frequency(x, groups=[], show=True):
    """
...
if __name__ == "__main__":
    # t = frequency([8, 3, 2, 1, 1, 2, 3, 4])
    # print(t.frekvens)
    # print(t.hyppighed)
    # print(t.observation)
    # print(t.kumuleret_frekvens)
    # t = frequency([8, 3, 2, 1], [1, 2, 3, 4, 5])
    # t = frequency([8, 3, 2, 1], [[1, 2], [2, 3], [3, 4], [4, 5]])
    # print(t.observation)
