...
from ..algorithms.models import HierarchicalReconcileModel, BoostingReconcileModel
...
class JobClass():
    ...
    def predict(self, start_date: str, num_forecast_periods: str, decision_function: str ='rmse*0.5 + mae*0.5') -> pd.DataFrame:
        ...
        if self.apply_reconciler.lower() == 'hierarchical':
            try:
                reconcile = f'{self.best_reconciler[0].__class__.__name__}(method="{self.best_reconciler[0].method}")'
            except AttributeError:
                reconcile = f'{self.best_reconciler[0].__class__.__name__}()'
            
            logger.info(message = f'Applying Hierarchical Reconciliation Model: {reconcile}')
            ...
            logger.info(message = f'Applying Boosting Reconcile Model')

            forecast_rec = self.best_reconciler.predict(forecast_model)
