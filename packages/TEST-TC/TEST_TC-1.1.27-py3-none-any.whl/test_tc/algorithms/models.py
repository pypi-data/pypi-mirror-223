        ...
        for params in self.hier_params:
            ...
            for train, val in zip(train_data, val_data):
                ...
                if params["n_hier"] == 1:
                    ...
                    model = XGBoost(
                        regression=True, cat_features=self.cat_features
                    ).fit(X_train, y_train, X_val, y_val)
                else:
                    id_pred = f"class_{id_pred}"
                    model = XGBoost(
                        regression=False, cat_features=self.cat_features
                    ).fit(X_train, y_train, X_val, y_val)
        ...
        for params in self.hier_params:
            ...
            for data in predict_data:
                ...
                if params["n_hier"] == 1:
                    ...
                    pi_lower[id_pred] = data[f"Pi_Lower_{id_pred}"]
                    pi_upper[id_pred] = data[f"Pi_Upper_{id_pred}"]
                else:
                    proportions = self._models[f"class_{id_pred}"].predict(X)
                    for i, col in enumerate(y.columns):
                        pred_mean[col.replace("Target_", "")] = (
                            pred_mean[id_pred] * proportions[:, i]
                        )
                        pi_lower[col.replace("Target_", "")] = (
                            pi_lower[id_pred] * proportions[:, i]
                        )
                        pi_upper[col.replace("Target_", "")] = (
                            pi_upper[id_pred] * proportions[:, i]
                        )
