...
def test_get_configuration():
    ...
    expected_config =     {'logging': {'logging_folder': 'logs'},
                           ...
                           'data_file': {'data_input_filename': 'unittest_teleconsulto.parquet'}}
...
def test_check_model():
    ...
    valid_apply_hierarchical = True
    valid_reference_apply_hierarchical = [True, False]

    # Test for valid model and apply_hierarchical (should not raise an exception)
    check_model(valid_model_name, valid_reference_model_name, valid_apply_hierarchical, valid_reference_apply_hierarchical)

    # Test for invalid model_name (should raise an exception)
    invalid_model_name = "invalid_model"

    with pytest.raises(InputTypeError):
        check_model(invalid_model_name, valid_reference_model_name, valid_apply_hierarchical, valid_reference_apply_hierarchical)
    ...
    with pytest.raises(InputTypeError, match=f"model_name {inconsistent_model_name} MUST be equal to reference_model_name {valid_reference_model_name}"):
        check_model(inconsistent_model_name, valid_reference_model_name, valid_apply_hierarchical, valid_reference_apply_hierarchical)

    # Test for invalid apply_hierarchical (should raise an exception)
    invalid_apply_hierarchical = "not_a_bool"

    with pytest.raises(InputTypeError):
        check_model(valid_model_name, valid_reference_model_name, invalid_apply_hierarchical, valid_reference_apply_hierarchical)
