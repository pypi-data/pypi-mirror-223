...
def test_HierarchicalReconcileMethod_score():
    ...
    with warnings.catch_warnings():
        ...
        _, scores = h_model.score(X_true = X_future_true.reset_index(), X_pred = Y_rec, check_model_pred = Y_rec, hierarchical_output = False)
        ...
        scores_aggr, _ = h_model.score(X_true = X_future_true.reset_index(), X_pred = Y_rec, check_model_pred = Y_rec, hierarchical_output = False)
        expected_scores = [0.0, 0.0, 0.0, 0.0, np.nan, 0.0]
...
def test_HierarchicalReconcileMethod_best_reconciler():
    with warnings.catch_warnings():
            ...
            best_reconciler = h_model.best_reconciler(X_true=X_true, X_pred=X_pred, X_future_true = X_future_true, X_future_pred=X_future_pred)
            reconciler_string = str(best_reconciler).split(' ')[0]
