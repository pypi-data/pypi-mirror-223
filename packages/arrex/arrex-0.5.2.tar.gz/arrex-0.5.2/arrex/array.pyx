...
cdef class typedarray:
	...
	@property
	def shape(self):
		return tuple(self.shape[i]  for i in range(self.dims))
		
	@property
	def size(self):
		return prod(self.shape[i]  for i in range(self.dims))
	

	def __init__(self, iterable=None, dtype=None, reserve=None):
		indev
				
	@staticmethod
	def full(value, shape, dtype=None):
		cdef typedarray array = typedarray(dtype=dtype or type(value), shape)
		for i in range(size):
	...
	def empty(dtype, shape):
		...
		'''
		cdef typedlist array = typedlist(dtype=dtype, reserve=shape)
		array.shape[0] = shape[0]
		return array
	...
	def transpose(self, axes=None):
		...
		char *data = (char*) malloc((sizeof(size_t)+sizeof(ssize_t))*self.dims)
		new.shape = (size_t*) data
		new.strides = (ssize_t*) (data+self.dims*sizeof(size_t))
		memcpy(new.shape, self.shape, self.dims*sizeof(size_t))
		memcpy(new.strides, self.strides, self.dims*sizeof(ssize_t))
		return new
