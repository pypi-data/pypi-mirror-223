...
class GetResponse(IResponseGetterMixin):
    ...
    def __init__(self, nadr: int, hwpid: int = dpa_constants.HWPID_MAX, rcode: int = 0, dpa_value: int = 0,
        ...
        if rcode == ResponseCodes.OK:
            self._port_data = result['ports']

    @property
    def port_data(self) -> List[int]:
        return self._port_data

    @staticmethod
    def from_dpa(dpa: bytes) -> GetResponse:
        ...
        return GetResponse(nadr=nadr, hwpid=hwpid, rcode=rcode, dpa_value=dpa_value, pdata=pdata, result=result)

    @staticmethod
    def from_json(json: dict) -> GetResponse:
        ...
        return GetResponse(nadr=nadr, msgid=msgid, hwpid=hwpid, rcode=rcode, dpa_value=dpa_value, pdata=pdata,
                           result=result)
