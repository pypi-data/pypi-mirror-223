...
class PIH:
    ...
    class UPDATER:
        ...
        @staticmethod
        def update_for_service(role_or_information: ServiceRoles | ServiceDescription, modules: list[str] | None = None, pih_update: bool = False, modules_update: bool = True, show_output: bool = True) -> bool:
        ...
        def get_module_updater_command_list(module_name: str, version: str | None = None) -> list[str]:
            return [CONST.PYTHON.EXECUTOR, "-m", CONST.PYTHON.PYPI, CONST.PYTHON.COMMAND.INSTALL] + (if_else(DataTool.is_none(version), [module_name, "-U"], lambda: [j((module_name, "==", version))]))
    ...
    class EXECUTOR:
        ...
        def create_command_for_executor(executor_name: str, command: list[str], login: str | None = None, password: str | None = None) -> list[str]:
            ...
            return [PIH.PATH.standarted(PIH.EXECUTOR.get_executor_path(executor_name)), CONST.PSTOOLS.NO_BANNER, CONST.PSTOOLS.ACCEPTEULA] + if_else(DataTool.is_empty(login), [],  ["-u", login]) + if_else(DataTool.is_empty(password), [], ["-p", password]) + command
        ...
        def create_command_for_start_service(role_or_information: ServiceRoles | ServiceInformationBase, interactive: bool | None = False) -> list[str]:
            ...
            local: bool = socket.gethostbyname(
                socket.gethostname()) == socket.gethostbyname(description.host)
            local_action = lambda value: if_else(local, None, value)
            login: str | None = local_action(DataTool.check_not_none(description.login, lambda: PIH.DATA.FORMAT.variable(description.login), PIH.DATA.variable_value(CONST.ADMINISTRATOR_LOGIN_ALIAS)))
            password: str | None = local_action(DataTool.check_not_none(description.password, lambda: PIH.DATA.FORMAT.variable(description.password), PIH.DATA.variable_value(CONST.ADMINISTRATOR_PASSOWORD_ALIAS)))
            interactive = local_action(interactive)
            return PIH.EXECUTOR.create_command_for_psexec(python_command, local_action(PIH.SERVICE.get_host(role_or_information)), login, password, interactive, description.run_from_system_account)
        ...
        def kill_process(name_or_pid: str | int, host: str, via_taskkill: bool = True, show_output: bool = False) -> bool:
           if via_taskkill:
                ...
                return PIH.EXECUTOR.execute(["taskkill", "/s", host, "/t", "/f", "/im" if is_string else "/pid", PIH.PATH.add_extension(name_or_pid, FILE.EXTENSION.EXE) if is_string else str(name_or_pid)], show_output).returncode < 2
        ...
        def process_is_exists(pid: int, host: str | None = None, login: str | None = None, password: str | None = None) -> str:
            ...
            login = "\\".join(
                [AD.DOMAIN_NAME, login or PIH.DATA.variable_value(CONST.ADMINISTRATOR_LOGIN_ALIAS)])
            password = password or PIH.DATA.variable_value(CONST.ADMINISTRATOR_PASSOWORD_ALIAS)
            if not DataTool.is_empty(host):
                command_list += ["/s", host]
        ...
        def kill_python_process(host: str, via_taskkill: bool) -> bool:
            return PIH.EXECUTOR.kill_process(CONST.PYTHON.EXECUTOR, host, via_taskkill)
        ...
        class ADMIN:
            ...
            def kill_all(local: bool = True, via_taskkill: bool = True, exclude_me: bool = False) -> bool:
                ...
                def kill(host: str, local: bool, via_taskkill: bool) -> None:
                    if local:
                        PIH.EXECUTOR.kill_python_process(host, via_taskkill)
                    else:
            ...
            def kill(role_or_information: ServiceRoles | ServiceInformationBase, local: bool = True, via_taskkill: bool = True) -> bool | None:
                ...
                if local:
                    return PIH.EXECUTOR.kill_process(pid, host, via_taskkill)
    ...
    class PATH(PATHS, PathTool):
        ...
        @staticmethod
        def standarted(value: str) -> str:
            return value.replace("\\", "/").replace("\\\\", "//")

        @staticmethod
        def join(path: str, *paths) -> str:
            return PIH.PATH.standarted(os.path.join(path, *paths))
        ...
        @staticmethod
        def mount_storage_on_linux_host(linux_host: str, storage_path: str, linux_mount_point: str, linux_host_user_password: str, storage_host_user_login: str, storage_host_user_password: str) -> None:
            PIH.RESULT.SSH.execute(PIH.PATH.get_command_for_mount_storage_on_linux_host(
                storage_path, linux_mount_point, linux_host_user_password, storage_host_user_login, storage_host_user_password), linux_host, use_sudo=True)
        ...
        def get_command_for_mount_facade_storage_on_linux_host() -> str:
            return PIH.PATH.get_command_for_mount_storage_on_linux_host(PATHS.FACADE.STORAGE_PATH(), 
                                                                        ...
                                                                        PIH.DATA.variable_value(HOSTS.SERVICES.PASSWORD_ALIAS), 
                                                                        PIH.DATA.variable_value(CONST.ADMINISTRATOR_LOGIN_ALIAS),
                                                                        PIH.DATA.variable_value(CONST.ADMINISTRATOR_PASSOWORD_ALIAS))
        @staticmethod
    ...
    class DATA(DataTool, StringTool, ListTool, DateTimeTool, EnumTool, FullNameTool):
        ...
        class TELEPHONE_NUMBER:
            ...
            def it_administrator() -> str:
                return PIH.DATA.TELEPHONE_NUMBER.by_login(PIH.DATA.variable_value(CONST.ADMINISTRATOR_LOGIN_ALIAS))
    ...
    class OS:
        ...
        def python_path(host: str | None = None) -> str | None:
            ...
            variable_list: list[str] = if_else(DataTool.is_none(host),
                PIH.OS.variable(WINDOWS.ENVIROMENT_VARIABLES.PATH),
            ...
            for variable_item in variable_list:
                ...
                if index != -1 and len(variable_item) - (index + len(search_pattern)) <= 4:
                    return PathTool.join(variable_item, PathTool.add_extension(CONST.PYTHON.EXECUTOR, FILE.EXTENSION.EXE))
            return None  

        @staticmethod
        def python_version(host: str | None = None) -> str:
            command_list: list[str] = [CONST.PYTHON.EXECUTOR_ALIAS, CONST.PYTHON.COMMAND.VERSION]
            if StringTool.contains(host, PIH.OS.host()):
                host = None
            return j(str(PIH.EXECUTOR.execute(
                    if_else(DataTool.is_none(host), 
                            command_list,
                            lambda:PIH.EXECUTOR.create_command_for_psexec(command_list, host, interactive=True)), True, True).stdout).strip().split(" ")[1:]
                    )
    ...
    class CHECK:
        ...
        class RESOURCE:
            ...
            def accessibility_by_ping(address_or_ip: str, host: str | None = None, count: int | None = None, check_for_all: bool = True) -> bool:
                ...
                process_result: CompletedProcess = PIH.EXECUTOR.execute(local_ping_commnad_list if host is None else PIH.EXECUTOR.create_command_for_psexec(local_ping_commnad_list, host, interactive=True), True, True)
        ...
        class WORKSTATION:
            ...
            def windows_service_running(name: str, workstation_name: str) -> bool | None:
                ...
                if DataTool.is_none(accessable):
                    return None
                return PIH.EXECUTOR.check_for_windows_service_running(name, workstation_name)
