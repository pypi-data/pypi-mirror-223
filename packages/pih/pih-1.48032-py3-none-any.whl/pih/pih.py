...
class PIH:
    ...
    class UPDATER:
        ...
        def update_for_service(role_or_information: ServiceRoles | ServiceDescription, modules: list[str] | None = None, pih_update: bool = False, modules_update: bool = True, show_output: bool = True) -> bool:
            ...
            result: bool = True
            if pih_update:
                remote_executor_command_list: list[str] = PIH.EXECUTOR.create_remote_process_executor_for_service(description, True)
        ...
        def update_remote(host: str, show_output: bool = False) -> bool:
            remote_executor_command_list: list[str] = PIH.EXECUTOR.create_command_list_for_psexec_command(
                host)
            command_list: list[str] = remote_executor_command_list + \
    ...
    class EXECUTOR:
        ...
        def get_disk_statistics_list(host: str) -> list[DiskStatistics]:
            output: str = PIH.EXECUTOR.execute_command(f"powershell Get-WmiObject -Class win32_logicalDisk -ComputerName {host}", True, True, as_shell=True).stdout
        ...
        def get_users_logged_on(host: str, active: bool | None = True) -> list[str]:
            def get_login_list(complete_process: CompletedProcess) -> set[str]:
                ...
                return result
            result: set[str] = set()
            result |= get_login_list(PIH.EXECUTOR.execute_command(
                ["query", "user", "/server:" + host], True, True, False))
            result |= get_login_list(PIH.EXECUTOR.execute_command(PIH.EXECUTOR.create_command_list_for_psexec_command(
        ...
        def stop_windows_service(name: str, workstation_name: str) -> bool:
            output: str = PIH.EXECUTOR.execute_command(PIH.EXECUTOR.create_command_list_for_psexec_command(
        ...
        @staticmethod
        def execute_command(command: list[str] | str, show_output: bool, capture_output: bool = False, as_text: bool = True, as_shell: bool = False) -> CompletedProcess:
            if show_output:
                if capture_output:
        ...
        def process_is_exists(pid: int, host: str | None = None, login: str | None = None, password: str | None = None) -> str:
            ...
            output: str = PIH.EXECUTOR.execute_command(
        ...
        def _ws_action(value: str, host: str, show_output: bool = False) -> bool:
            return PIH.EXECUTOR.execute_command(PIH.EXECUTOR.create_command_list_for_psexec_command(
            ...
            def start(role_or_information: ServiceRoles | ServiceInformationBase, check_if_started: bool = True, show_output: bool = True) -> bool | None:
                ...
                if check_if_started:
                    if PIH.SERVICE.check_accessibility(description):
                        return None
                remote_executor_command_list: list[str] = PIH.EXECUTOR.create_remote_process_executor_for_service(description, False)
                ...
                remote_executor_command_list.append("--isolated")
                remote_executor_command_list.append("0")
                process_result: CompletedProcess = PIH.EXECUTOR.execute_command(remote_executor_command_list, show_output)
                returncode = process_result.returncode
                if returncode == 2:
                    return False
                return True
    ...
    class PATH(PATHS, PathTool):
        ...
        def mount_facade_storage_on_linux_host(linux_host: str | None = None) -> bool:
            if PIH.OS.is_linux():
                ...
                if_else(DataTool.is_none(linux_host),
                        lambda: PIH.EXECUTOR.execute_command(commnad, True, as_shell=True),
        ...
        def get_command_for_mount_facade_storage_on_linux_host() -> str:
            return PIH.PATH.get_command_for_mount_storage_on_linux_host(PATHS.FACADE.STORAGE_PATH(), 
                                                                        ...
                                                                        PIH.DATA.get_variable_value(CONST.ADMINISTRATOR_LOGIN_ALIAS),
                                                                        PIH.DATA.get_variable_value(CONST.ADMINISTRATOR_PASSOWORD_ALIAS))
        @staticmethod
    ...
    class OS:
        ...
        def python_version(host: str | None = None) -> str:
            ...
            if StringTool.contains(host, PIH.OS.host()):
                host = None
            return j(str(PIH.EXECUTOR.execute_command(
                    if_else(DataTool.is_none(host), 
                            command_list,
                            lambda:PIH.EXECUTOR.create_command_list_for_psexec_command(command_list, host, interactive=True)), True, True).stdout).strip().split(" ")[1:]
                    )
        ...
        class WORKSTATION:
            ...
            def windows_service_running(name: str, workstation_name: str) -> bool | None:
                ...
                if DataTool.is_none(accessable):
                    return None
                return PIH.EXECUTOR.windows_service_running(name, workstation_name)
