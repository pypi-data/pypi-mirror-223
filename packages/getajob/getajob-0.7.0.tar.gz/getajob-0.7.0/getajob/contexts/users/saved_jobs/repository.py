...
from .models import UserSavedJob, SavedJob


entity_models = EntityModels(entity=SavedJob, create=UserSavedJob)


class UserSavedJobsRepository(MultipleChildrenRepository):
    ...
    def _get_saved_jobs(
        ...
        job_repo = JobsRepository(request_scope=request_scope, kafka=None)
        saved_jobs = []
        for company_id, job_ids in saved_company_jobs.items():
            company_jobs = job_repo.get_all(
