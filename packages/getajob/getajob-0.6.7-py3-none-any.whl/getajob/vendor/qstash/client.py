...
class QStashClient:
    ...
    def _create_url(self, kafka_topic: KafkaTopic):
        return f"{self.base_url}/{self.kafka_url}/webhook?topic={kafka_topic.value}&user={self.kafka_username}&pass={self.kafka_password}"
    ...
    def _send_request_to_kafka(
        ...
        return request("POST", url, headers=headers, json=message.dict())
