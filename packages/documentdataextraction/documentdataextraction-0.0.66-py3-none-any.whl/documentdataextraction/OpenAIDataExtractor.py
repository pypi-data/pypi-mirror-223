...
class OpenAIDataExtractor:
    ...
    def extractdatausing_turbo(self,proc_api_key: str, ai_proc_templ : str, ai_proc_variables : str, OCR_Text : str):
        ...
        if page_wise == "No" or self.fileExtension_lower == 'csv' or self.fileExtension_lower == 'xls' or self.fileExtension_lower == 'xlsx':
            ...
            joined_OCR_Text = " ".join(OCR_Text_withInstruction).strip()
        ...
        for i in range(len(ai_proc_templ_updated)):
                logger.log(f"\n\n inside 'page_wise = Yes' condition \n\n","0")
    ...
    def call_GPT_Service(self, text, proc_api_key, token_limit = 4096, max_response_tokens = 1800):
        ...
        openai.api_key = proc_api_key
        logger.log(f" text line 755 :::\n{text}, {type(text)}","0")    
        #to handle any unicode character
        clean_string = ''.join(c for c in text if unicodedata.category(c)[0] != 'C')
        logger.log(f" text line 758 :::\n{text}, {type(text)}","0")    
        message = json.loads(clean_string.replace("\\",'\/'))
        logger.log(f" message line 760 :::\n{message}, {type(message)}","0")    
        ...
        if messageTokenLength <= token_limit:
            ...
            logger.log(f"\n\n Completion result 3.5 :::{result} \t{type(result)}\n","0")
            return result
            ...
            result = (completion['choices'][0]['message']['content'])
            logger.log(f"\n\n Completion result 4 :::{result} \t{type(result)}\n","0")
            return result
