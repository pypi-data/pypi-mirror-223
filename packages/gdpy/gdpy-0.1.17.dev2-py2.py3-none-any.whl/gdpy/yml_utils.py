...
def parse_list_tools(result, body, tool_type='gwl'):
    ...
    if tool_type == 'gwl':
        ...
        result.tools = convert(body.get("tool_list"))
        result.count = body.get("count")
    else:
        raise NotImplementedError("Unsupported tool type '{}'".format(tool_type))
    return result



def parse_get_tool(result, body):
    body = json_loader(body)
    result.tool = convert(body)
    return result
