...
class TestKlarf(_TestKlarfReader):
    def setUp(self) -> None:
        ...
        self.path_klarf_1_2_single_wafer_tiff = ASSETS_PATH / "KLARF_TIFF.001"
    ...
    @load_expected_data(saved_path=ASSETS_SAVED_PATH)
    def test_klarf_single_wafer_tiff(self, expected_data, saved_path) -> None:
        # When
        content = Klarf.load_from_file(filepath=self.path_klarf_1_2_single_wafer_tiff)
        content_dict = asdict(content)
    ...
    def test_klarf_single_wafer_tiff_with_defects_generator(self) -> None:
        # When
        content = Klarf.load_from_file(
            filepath=self.path_klarf_1_2_single_wafer_tiff,
            defects_as_generator=True,
        )
        expected_defects = [45]

        # Then
        for index, item in enumerate(content.wafers):
            defects_tmp = len(list(item.defects))
            self.assertEqual(defects_tmp, expected_defects[index])
