...
def train_test_split_time(df, min_length=100, train_ratio=0.8):
    """
    Parameters:
    df: DataFrame, the original data.
    min_length: int, minimum length of data sequence.
    ...
    # Calculate the number of sequences based on min_length
    num_sequences = len(df) // min_length

    # Create an array of sequence indices and shuffle it
    sequence_indices = np.arange(num_sequences)
    np.random.shuffle(sequence_indices)

    # Split sequence indices into training and testing sets
    train_num_sequences = int(num_sequences * train_ratio)
    train_indices = sequence_indices[:train_num_sequences]
    test_indices = sequence_indices[train_num_sequences:]

    # Get the actual data indices
    train_data_indices = np.hstack([range(i * min_length, (i+1) * min_length) for i in train_indices])
    test_data_indices = np.hstack([range(i * min_length, (i+1) * min_length) for i in test_indices])

    # Extract training and testing data
    train = df.iloc[train_data_indices]
    test = df.iloc[test_data_indices]
