...
def _get_ib_devices():
    return _get_data_nics('lo')


def _get_data_nics(mgmt_ifname, data_nics=[]):
    ...
    iface_list = []
    for device in data:
        if not device or device['ifname'] == mgmt_ifname or device['ifname'] == 'lo':
            continue
        if data_nics and device['ifname'] not in data_nics:
            continue
...
def _get_nvme_list_from_spdk(cluster, rpc_client=None):
...
def get_node_port_iostats(port_id, history=None):
    ...
    data = db_controller.get_port_stats(port.get_id(), nd.get_id())
    out = []
    if not history:
        data = data[:1]
