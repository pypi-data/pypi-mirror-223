...
class ServiceObject:
    ...
    def service_start(self):
        out, _, ret_code = shell_utils.run_command("systemctl start %s" % self.name)
        return ret_code == 0

    def service_stop(self):
        out, _, ret_code = shell_utils.run_command("systemctl stop %s" % self.name)
        return ret_code == 0

    def install_service(self):
        out, err, ret_code = shell_utils.run_command(
            "bash %s %s %s" % (self.SERVICE_INSTALL_SCRIPT, self.name, self._build_unit_file()))
        return out, err, ret_code

    def service_remove(self):
        out, err, ret_code = shell_utils.run_command("bash %s %s" % (self.SERVICE_REMOVE_SCRIPT, self.name))
        return ret_code == 0

    def restart(self):
        out, _, ret_code = shell_utils.run_command("systemctl restart %s" % self.name)
        return ret_code == 0
...
rpc_http_proxy = ServiceObject(
    "rpc_http_proxy",
    "python %s" % os.path.join(SPDK_PATH, "scripts/rpc_http_proxy.py"))

ultra_node_monitor = ServiceObject(
    "ultra_node_monitor",
    "python %s" % os.path.join(constants.INSTALL_DIR, "management/services/node_monitor.py"))

ultra_device_monitor = ServiceObject(
    "ultra_device_monitor",
    "python %s" % os.path.join(constants.INSTALL_DIR, "management/services/device_monitor.py"))

ultra_stat_collector = ServiceObject(
    "ultra_stat_collector",
    "python %s" % os.path.join(constants.INSTALL_DIR, "management/services/stat_collector.py"))

ultra_mgmt_node_monitor = ServiceObject(
    "ultra_mgmt_node_monitor",
    "python %s" % os.path.join(constants.INSTALL_DIR, "management/services/mgmt_node_monitor.py"))
