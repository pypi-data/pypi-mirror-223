...
def spectra(uvw, Nx, dx, exclude_zero=True):

    k = np.arange(0, Nx / 2)[int(exclude_zero):] * 2 * np.pi / (Nx * dx)
    SU, SV, SW = [np.fft.fft(uvw, axis=0)[int(exclude_zero):Nx // 2] for uvw in uvw]

    return k, [(dx / (2 * np.pi * Nx)) * np.mean((np.real(np.conj(S1) * S2)), (1, 2))
               for S1, S2 in [(SU, SU), (SV, SV), (SW, SW), (SU, SW)]]
