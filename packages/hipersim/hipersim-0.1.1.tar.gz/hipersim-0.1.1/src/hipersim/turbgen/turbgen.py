...
class turb_field:
    ...
    def to_xarray(self):
        ...
        return xr.DataArray(np.array([self.u, self.v, self.w]), dims=('uvw', 'x', 'y', 'z'),
                            coords={'x': np.arange(Nx) * dx, 'y': np.arange(Ny) * dy, 'z': np.arange(Nz) * dz,
                                    'uvw': ['u', 'v', 'w']},
                            attrs={'ae': ae, 'L': L, 'G': G, 'Generator': 'Hipersim', 'seed': seed,
                                   'name': "hipersim_mann_l%.1f_ae%.4f_g%.1f_h%d_%dx%dx%d_%.3fx%.2fx%.2f_s%04d.nc" % (
                                       L, ae, G, hfc, Nx, Ny, Nz, dx, dy, dz, seed)})

    def to_netcdf(self, folder='', filename=None):
        da = self.to_xarray()
        filename = os.path.join(folder, filename or da.attrs['name'])
