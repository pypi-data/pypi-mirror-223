...
import time
from src.hipersim.turbgen.generate_field import SpectralTensor
...
def test_against_reference(hfc):
    ...
    ref = xr.load_dataarray(tfp + f'hipersim_mann_l40.0_ae0.5000_g3.0_h{hfc}_{Nx}x{Ny}x{Nz}_2.000x3.00x4.00_s0001.nc')
...
def test_time_and_memory():
    ...
    def run():
        sp = SpectralTensor(alphaepsilon=1, L=29.6, Gamma=3.9, Nx=Nxyz[0], Ny=Nxyz[1], Nz=Nxyz[2],
                            dx=1, dy=1, dz=1, HighFreqComp=False, double_x=False, double_y=False, double_z=False)
        sp.generate_field(1)
