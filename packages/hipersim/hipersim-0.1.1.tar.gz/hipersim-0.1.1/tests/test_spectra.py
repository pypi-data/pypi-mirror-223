...
def test_spectra():
    ...
    sp = SpectralTensor(alphaepsilon=ae, L=L, Gamma=G, Nx=1024, Ny=64, Nz=32, dx=1, dy=1, dz=1, HighFreqComp=False,
                        double_x=False, double_y=False, double_z=False)
    k1, phi = sp.spectra(k[::100], 200)
    if 0:
    ...
    for dx in [1, 2, 4]:
        ...
        for i, uvw in enumerate(['UU', 'VV', 'WW']):
            ...
            if cpp_SUVW:
                ...
                np.testing.assert_allclose(np.log10((k * hs_SUVW[i])[m]), np.log10((k * cpp_SUVW[i])[m]), atol=.16)
        ...
        if cpp_SUVW:
            ...
            np.testing.assert_allclose((k * hs_SUVW[3])[m], (k * cpp_SUVW[3])[m], atol=.016)
...
def test_coherence():
    ...
    for deltak2, deltak3 in zip(deltak2_vect, deltak3_vect):
        ...
        _, (SUPiens2, _, _, SUPijens2) = spectra([ui, ui[:8, :8, :8], uj], Nx, dx, exclude_zero=False)
        SUPiens, SUPijens = [SUP / (dx / (2 * np.pi * Nx)) * (1 / (np.sqrt(2) * Nx * dx))
                             for SUP in [SUPiens2, SUPijens2]]

        CoherenceP = np.real(SUPijens) / SUPiens
        ...
        np.testing.assert_allclose(np.interp(f1shift[m], f1plot_mean,
                                   CoherenceP_mean[:]), Coherence_ref[m], atol=0.07)
