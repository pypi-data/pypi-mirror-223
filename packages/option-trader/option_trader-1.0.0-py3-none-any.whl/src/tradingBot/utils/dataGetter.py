...
from tradingBot.utils.optionTool import theta_call
from tradingBot.utils.optionTool import theta_put
from tradingBot.utils.optionTool import gamma
from tradingBot.utils.optionTool import vega
...
from tradingBot.utils.dataGetter_IB import IB_get_option_chain
from tradingBot.utils.dataGetter_IB import IB_get_option_leg_details
...
def get_price_history(symbol, period='6mo', interval="1d"):     
    ...
    try:        
        ...
        logger.error('q.history raise exception %s' % symbol)
        pass
...
def pick_option_long( symbol, 
    ...
    for exp_date in exp_date_list:  
        ...
        if otype == at.CALL:              
            ...
            for i, call in calls.iterrows():                           
                ...
                if df.at[0, optionSummary.PNL] < min_pnl:
                    break              
    ...
    win_prob_range = '[%.2f|%.2f]' % (single_df[optionSummary.WIN_PROB].min(), single_df[optionSummary.WIN_PROB].max())
    i = single_df.shape[0]
    single_df= single_df[single_df[optionSummary.WIN_PROB] >= min_win_prob]
...
def pick_option_short( symbol, 
    ...
    open_price_range = '[%.2f|%.2f]' % (single_df[optionSummary.OPEN_PRICE].min(), single_df[optionSummary.OPEN_PRICE].max())
    i = single_df.shape[0]
    single_df= single_df[single_df[optionSummary.OPEN_PRICE] >= min_price]
...
def pick_vertical_put_spreads( symbol,                          
    ...
    if spread_df.shape[0] > 0:
        ...
        win_prob_range = '[%.2f|%.2f]' % (spread_df[optionSummary.WIN_PROB].min(), spread_df[optionSummary.WIN_PROB].max())
        i = spread_df.shape[0]
        spread_df= spread_df[spread_df[optionSummary.WIN_PROB] >= min_win_prob]
        logger.debug('%d -> %d reduced by win prob >= %.2f %s' % (i, spread_df.shape[0], min_win_prob, win_prob_range))

        pnl_range = '[%.2f|%.2f]' % (spread_df[optionSummary.PNL].min(), spread_df[optionSummary.PNL].max())
        i = spread_df.shape[0]
        spread_df= spread_df[spread_df[optionSummary.WIN_PROB] >= min_pnl]
...
def pick_iron_condor(symbol,                                              
    ...
    open_price_range = '[%.2f|%.2f]' % (iron_condor_df[optionSummary.OPEN_PRICE].min(), iron_condor_df[optionSummary.OPEN_PRICE].max())
    i = iron_condor_df.shape[0]
    iron_condor_df = iron_condor_df[iron_condor_df[optionSummary.OPEN_PRICE] >= min_price]
    logger.debug('%d -> %d reduced by open price >= %.2f %s' % (i, iron_condor_df.shape[0], min_price, open_price_range))

    win_prob_range = '[%.2f|%.2f]' % (iron_condor_df[optionSummary.WIN_PROB].min(), iron_condor_df[optionSummary.WIN_PROB].max())
    i = iron_condor_df.shape[0]
    iron_condor_df= iron_condor_df[iron_condor_df[optionSummary.WIN_PROB] >= min_win_prob]
    logger.debug('%d -> %d reduced by win prob >= %.2f %s' % (i, iron_condor_df.shape[0], min_win_prob, win_prob_range))

    pnl_range = '[%.2f|%.2f]' % (iron_condor_df[optionSummary.PNL].min(), iron_condor_df[optionSummary.PNL].max())
    i = iron_condor_df.shape[0]
    iron_condor_df= iron_condor_df[iron_condor_df[optionSummary.WIN_PROB] >= min_pnl]
...
def pick_call_butterfly(symbol,                         
    ...
    open_price_range = '[%.2f|%.2f]' % (butterfly_df[optionSummary.OPEN_PRICE].min(), butterfly_df[optionSummary.OPEN_PRICE].max())
    i = butterfly_df.shape[0]
    butterfly_df = butterfly_df[butterfly_df[optionSummary.OPEN_PRICE] >= min_price]
    logger.debug('%d -> %d reduce by open price >= %.2f %s' % (i, butterfly_df.shape[0], min_price, open_price_range))

    win_prob_range = '[%.2f|%.2f]' % (butterfly_df[optionSummary.WIN_PROB].min(), butterfly_df[optionSummary.WIN_PROB].max())
    i = butterfly_df.shape[0]
    butterfly_df= butterfly_df[butterfly_df[optionSummary.WIN_PROB] >= min_win_prob]
    logger.debug('%d -> %d reduced by win prob >= %.2f %s' % (i, butterfly_df.shape[0], min_win_prob, win_prob_range))

    pnl_range = '[%.2f|%.2f]' % (butterfly_df[optionSummary.PNL].min(), butterfly_df[optionSummary.PNL].max())
    i = butterfly_df.shape[0]
    butterfly_df= butterfly_df[butterfly_df[optionSummary.WIN_PROB] >= min_pnl]
...
def create_single_df(leg, stock_price, target_high, target_low, credit=True):
    ...
    if otype == at.CALL:
        dt[optionSummary.STRATEGY] = [st.COVERED_CALL if credit else st.LONG_CALL]         
        if credit:
            max_profit = price
        ...
        dt[optionSummary.STRATEGY] = [st.SHORT_PUT if credit else st.LONG_PUT]        
    ...
    dt[optionSummary.SYMBOL]            = [symbol]
    dt[optionSummary.EXP_DATE]          = [exp_date]
    dt[optionSummary.OPEN_PRICE]        = [price]
    dt[optionSummary.PNL]               = [pnl]
    dt[optionSummary.WIN_PROB]          = [win_prob]
    dt[optionSummary.MAX_LOSS]          = [max_loss]
    dt[optionSummary.MAX_PROFIT]        = [max_profit]
    dt[optionSummary.MARGIN]            = [margin]
    dt[optionSummary.BREAKEVEN_H]       = [breakeven_h]
    dt[optionSummary.BREAKEVEN_L]       = [breakeven_l]
    dt[optionSummary.TRADE_DATE]        = [today]
    dt[optionSummary.TRADE_STOCK_PRICE] = [stock_price]
    dt[optionSummary.SPREAD]            = [np.nan]

    dt[optionSummary.TARGET_HIGH]       = [target_high]
    dt[optionSummary.TARGET_LOW]        = [target_low]       

    leg[position.SCALE]                 = 1
    leg[position.OPEN_ACTION] = at.SELL_TO_OPEN if credit else at.BUY_TO_OPEN    

    dt[optionSummary.LEGS] = [[leg]]
...
def create_spread_df(target_low, target_high, sleg, lleg, otype, credit=True):
    ...
    if otype == at.CALL:
        dt[optionSummary.STRATEGY] = [st.CREDIT_CALL_SPREAD if credit else st.DEBIT_CALL_SPREAD]      
        if credit:
        ...
        dt[optionSummary.STRATEGY] = [st.CREDIT_PUT_SPREAD if credit else st.DEBIT_PUT_SPREAD]        
        if credit:
    ...
    dt[optionSummary.SYMBOL]            = [symbol]
    dt[optionSummary.EXP_DATE]          = [exp_date]
    dt[optionSummary.SPREAD]            = [spread]         
    dt[optionSummary.LAST_PRICE]        = [price]
    dt[optionSummary.OPEN_PRICE]        = [price]   
    dt[optionSummary.MAX_LOSS]          = [max_loss]
    dt[optionSummary.MAX_PROFIT]        = [max_profit]
    dt[optionSummary.MARGIN]            = [margin]
    ...
    dt[optionSummary.LEGS] = [[sleg, lleg]]
...
def create_buttrfly_df(strategy, target_low, target_high, l={}, m={}, h={}, iron={}):
    ...
    dt[optionSummary.SYMBOL]            = [symbol]
    dt[optionSummary.STRATEGY]          = [strategy]
    dt[optionSummary.SPREAD]            = [spread]
    dt[optionSummary.EXP_DATE]          = [exp_date]
    dt[optionSummary.LAST_PRICE]        = [price]
    dt[optionSummary.OPEN_PRICE]        = [price]   
...
def create_iron_condor_df(strategy, target_low, target_high, pl=None, ph=None, cl=None, ch=None):
    ...
    dt[optionSummary.STRATEGY] = [strategy]
    dt[optionSummary.TRADE_DATE] = datetime.now(timezone(app_settings.TIMEZONE))
    dt[optionSummary.TRADE_STOCK_PRICE] = get_price_history(symbol, period='1d', interval='1d')['Close'][-1]
    ...
    breakeven_l = ph[quote.STRIKE] - price if credit else cl[quote.STRIKE] + max_loss
    breakeven_h = cl[quote.STRIKE] + price if credit else ph[quote.STRIKE] - max_loss
    ...
    dt[optionSummary.MAX_LOSS]    = [max_loss]
    dt[optionSummary.MARGIN]      = [margin]    
    dt[optionSummary.PNL]         = [np.nan] if max_loss == 0 else [max_profit / max_loss]
    dt[optionSummary.BREAKEVEN_H] = [breakeven_h]
    dt[optionSummary.BREAKEVEN_L] = [breakeven_l]
    dt[optionSummary.TARGET_HIGH] = [target_high]
    dt[optionSummary.TARGET_LOW]  = [target_low]       
    ...
    dt[optionSummary.LEGS] = [[ph, pl, ch, cl]]
