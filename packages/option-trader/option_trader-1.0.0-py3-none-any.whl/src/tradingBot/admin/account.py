...
from tradingBot.settings.tradeConfig import entryCrit, riskManager, runtimeConfig, marketCondition
...
from tradingBot.admin import optionSummary
from tradingBot.admin import position
from tradingBot.admin import transaction
from tradingBot.admin import quote

from tradingBot.utils.dataGetter import pick_option_long, pick_option_short, afterHours
from tradingBot.utils.dataGetter import get_price_history,get_option_leg_details 
from tradingBot.utils.dataGetter import pick_vertical_call_spreads, pick_vertical_put_spreads
from tradingBot.utils.dataGetter import pick_call_butterfly, pick_put_butterfly, pick_iron_butterfly
from tradingBot.utils.dataGetter import pick_iron_condor, get_next_earning_date, get_option_exp_date
...
class account():
    ...
    def __init__(self, user, account_name, watchlist_name="", initial_balance=app_settings.DEFAULT_ACCOUNT_INITIAL_BALANCE):        
        ...
        if settings.DATABASES == 'sqlite3':
            try:
                ...
                cursor.execute("CREATE TABLE IF NOT EXISTS optionSummary("+optionSummary.schema+")")
    ...
    def expire_position(self, row):
        ...
        data = (row[position.LAST_PRICE], row[position.CURRENT_VALUE],row[position.TOTAL_GAIN_LOSS],\
                row[position.TOTAL_GAIN_LOSS_PERCENT], row[position.LEG_ID], row[position.UUID], asset.EXPIRED)
        ...
        for symbol in symbol_list:
            ...
            for i, row in sdf.iterrows():         
                ...
                if otype in [asset.CALL, asset.PUT]:
                    ...
                    if row['open_action'] == asset.BUY_TO_OPEN: 
                        gain = (row[position.LAST_PRICE] - row[position.OPEN_PRICE]) 
                        row[position.TOTAL_GAIN_LOSS] = gain * row[position.QUANTITY] * 100 
                        row[position.TOTAL_GAIN_LOSS_PERCENT] = (gain / row[position.OPEN_PRICE]) * 100                
                    else:          
                    ...
                    row[position.LAST_QUOTE_DATE] = today.strftime("%Y-%m-%d %H:%M:%S %Z")                          
                    ...
                    data = (row[position.LAST_PRICE], row[position.CURRENT_VALUE],row[position.TOTAL_GAIN_LOSS],\
                            ...
                            row[position.LAST_OPEN_INTEREST], row[position.LAST_VOLUME], today.strftime("%Y-%m-%d %H:%M:%S %Z"),\
                            row[position.LEG_ID],row[position.UUID])
                    cursor = self.db_conn.cursor()        
                    cursor.execute(sql, data)                    
                    self.db_conn.commit()               

        self.update_optionSummary(df)

    def create_optionSummary(self, symbol, exp_date, strategy, row, q):
        uuid_value = UUID.uuid4().hex

        open_price =   row[optionSummary.OPEN_PRICE] 
        breakeven_l =  row[optionSummary.BREAKEVEN_L]
        breakeven_h =  row[optionSummary.BREAKEVEN_H]
        max_profit =   row[optionSummary.MAX_PROFIT]
        max_loss =     row[optionSummary.MAX_LOSS]
        pnl =          row[optionSummary.PNL]
        win_prob =     row[optionSummary.WIN_PROB]
        credit = str(True) if row[optionSummary.MARGIN] > 0 else str(False) 
        trade_date =  str(row[optionSummary.TRADE_DATE])
        earning_date = str(get_next_earning_date(symbol))
        trade_stock_price = row[optionSummary.TRADE_STOCK_PRICE]
        margin =  (q * 100 * row[optionSummary.MARGIN])   
        cash = q * 100 * row[optionSummary.OPEN_PRICE]        
        spread =       row[optionSummary.SPREAD]         
        target_low  = row[optionSummary.TARGET_LOW]
        target_high = row[optionSummary.TARGET_HIGH]
        quantity =    q
        status =      asset.OPENED
        legs =        row[optionSummary.LEGS]          
        legs_desc =   self.create_position(symbol, legs, q, uuid_value, trade_date)        
        uuid = uuid_value

        if afterHours():
            msg = "[%s|%s] CREATE PENDING [%s|%s|%s] [price:%.2f|pnl:%.2f|win_prob:%.2f|q:%d]" % (self.user.user_name, self.account_name,\
                            strategy, symbol, exp_date, open_price, pnl, win_prob, quantity)
            self.logger.info(msg)
        ...
        msg = "[%s|%s] CREATE [%s|%s|%s] [price:%.2f|pnl:%.2f|win_prob:%.2f|q:%d]" % (self.user.user_name, self.account_name,\
                            strategy, symbol, exp_date, open_price, pnl, win_prob, quantity)
    ...
    def expire_optionSummary(self, srow):
        uuid = srow[optionSummary.UUID]
        pdf= pd.read_sql_query("SELECT * FROM position WHERE uuid = '"+uuid+"'", self.db_conn)

        last_price = 0
        for i, prow in pdf.iterrows():
            if prow[asset.STATUS] == asset.OPENED:
                exp_price = self.expire_position(prow)

        for i, prow in pdf.iterrows():                            
            if prow[position.OPEN_ACTION] == asset.BUY_TO_OPEN:
                last_price -= prow[position.LAST_PRICE]
            else:
                last_price += prow[position.LAST_PRICE]     

        exp_date = srow[optionSummary.EXP_DATE]        
        symbol   = srow[optionSummary.SYMBOL]
        data = get_price_history(symbol, period='1mo')        
        strategy = srow[optionSummary.STRATEGY]
        exp_stock_price = data['Close'][exp_date.tz_localize(data.index[-1].tz)]   
        open_price = srow[optionSummary.OPEN_PRICE] 
        credit = srow[optionSummary.CREDIT]=='True'
        quantity = srow[optionSummary.QUANTITY]
        ...
        if credit:             
            margin =  (quantity * 100 * srow[optionSummary.MARGIN])      
        ...
        sql = """update optionSummary set exp_stock_price=?, exp_price=?, last_price=?, pl=?, gain=? status=? stop_date=?, stop_reason=>, stop_price=?, where uuid==?"""        
        data = (exp_stock_price, exp_price, last_price, pl, gain, asset.EXPIRED, stop_date, stop_reason, stop_price, asset.uuid)
        cursor = self.db_conn.cursor()    
        cursor.execute(sql, data)     
        self.db_conn.commit()    

        msg = "[%s|%s] EXPIRE [%s|%s|%s] [profit:%.2f|gain:%.2f|q:%d]" % (self.user.user_name, self.account_name,\
    ...
    def roll_optionSummary(self, symbol, exp_date, strategy):
        ...
        df.sort_values(by = [optionSummary.WIN_PROB, optionSummary.PNL], ascending=False, inplace=True)                                          

        opt = df.head(1).to_dict('records')[0]     
        
        if opt[optionSummary.MAX_LOSS]  == 0:
            self.logger.debug('roll max loss == 0 ?? %s %s %s max profit %.2f open price %.2f' % (symbol, exp_date, strategy, opt[optionSummary.MAX_PROFIT], opt[optionSummary.OPEN_PRICE]))             
            q=99
        else:
            q = self.risk_mgr.max_loss_per_position // (opt[optionSummary.MAX_LOSS] * 100)
            if q == 0:
                self.logger.debug('max loss %.2f exceeded max per position risk %.2f' % (opt[optionSummary.MAX_LOSS] * 100, self.risk_mgr.max_loss_per_position))
                return
    ...
    def update_optionSummary(self, pdf):
        today = datetime.now(timezone(app_settings.TIMEZONE))

        sdf = pd.read_sql_query("SELECT * FROM optionSummary WHERE status = '"+asset.OPENED+"'", self.db_conn)

        uuid_list = pdf[position.UUID].unique()      
        for uuid in uuid_list:                                   
            srow = sdf[sdf[optionSummary.UUID]==uuid].iloc[0]
            exp_date = srow[optionSummary.EXP_DATE]
            trade_date = srow[optionSummary.TRADE_DATE]            
            days_to_expire = (pd.Timestamp(exp_date).tz_localize(timezone(app_settings.TIMEZONE))-today).days                      
            if days_to_expire < 0:  
                self.expire_optionSummary(srow)
                continue
            strategy = srow[optionSummary.STRATEGY]
            ppdf = pdf[pdf[position.UUID]==uuid]
            quantity = srow[optionSummary.QUANTITY]            
            last_price = 0
            ...
            if stopped:              
                if afterHours():
                    msg = "[%s|%s] STOP PENDING [%s|%s|%s] [date open %s|reason:%s|pl:%.2f|gain:%.2f|q:%d]" % (self.user.user_name, self.account_name,\
                                strategy, symbol, exp_date, str(trade_date), stop_reason, pl, gain, quantity)   
                    self.logger.info(msg)             
                    lineNotifyMessage(msg)  
                else:
                    for i, prow in ppdf.iterrows():
                    ...
                    cursor.execute(sql, data)     
                    self.db_conn.commit()   
                    msg = "[%s|%s] STOP [%s|%s|%s] [date open %s|reason:%s|pl:%.2f|gain:%.2f|q:%d]" % (self.user.user_name, self.account_name,\
                                strategy, symbol, exp_date, str(trade_date), stop_reason, pl, gain, quantity)   
                    self.logger.info(msg)             
                    lineNotifyMessage(msg)             
                    if roll:            
    ...
    def stop_optionSummary_not_in_used(self, srow, stop_reason):
        ...
        sql = """update optionSummary set last_price=?, pl=?, gain=? status=? stop_date=?, stop_reason=>, stop_price=?, where uuid==?"""        
        data = (last_price, pl, gain, asset.CLOSED, stop_date, stop_reason, stop_price, asset.uuid)
        cursor = self.db_conn.cursor()    
        cursor.execute(sql, data)     
    ...
    def try_open_new_strategy_positions(self, watchlist=[], strategy_list=[]):
        ...
        if settings.DATABASES == 'sqlite3':                         
            ...
            if candidates.shape[0] > 0:
                ...
                candidates.sort_values([optionSummary.WIN_PROB, optionSummary.PNL],  ascending=False, inplace=True)

                candidates = candidates.groupby([optionSummary.SYMBOL, optionSummary.EXP_DATE, optionSummary.STRATEGY]).first()
                ...
                for index, opt in candidates.iterrows():  
                    ...
                    if df[(df[optionSummary.SYMBOL]==symbol) & (df[optionSummary.EXP_DATE] == exp_date) & (df[optionSummary.STRATEGY] == strategy)].shape[0] > 0:
                        continue

                    if opt[optionSummary.MAX_LOSS]  == 0:
                        self.logger.info('max loss == 0 ?? %s %s %s max profit %.2f open price %.2f' % (symbol, exp_date, strategy, opt[optionSummary.MAX_PROFIT], opt[optionSummary.OPEN_PRICE]))             
                        q=99
                    else:
                        q = self.risk_mgr.max_loss_per_position // (opt[optionSummary.MAX_LOSS] * 100)
                        if q == 0:
                            self.logger.info('max loss %.2f exceeded max per position risk %.2f' % (opt[optionSummary.MAX_LOSS] * 100, self.risk_mgr.max_loss_per_position))
                            continue
    ...
    def pick_strategy_candidates(self, watchlist, strategy_list):   
        ...
        for strategy in strategy_list:
            ...
            if strategy_candidates.shape[0] > 0:         
                strategy_candidates.sort_values(by = [optionSummary.WIN_PROB, optionSummary.PNL], inplace=True)                                                      
    ...
    def pick_option_long(self, watchlist, otype):
        ...
        for symbol in candidates:
            ...
            if otype == asset.CALL:                
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.LONG_CALL) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
            else:
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.LONG_PUT) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
    ...
    def pick_option_short(self, watchlist, otype):
        ...
        for symbol in candidates:
            ...
            if otype == asset.CALL:                
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.COVERED_CALL) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
            else:
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.SHORT_PUT) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
    ...
    def pick_vertical_call_spread(self, watchlist, credit=True):
        ...
        for symbol in candidates:
            ...
            if credit:                
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.CREDIT_CALL_SPREAD) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
            else:
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.DEBIT_CALL_SPREAD) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
    ...
    def pick_vertical_put_spread(self, watchlist, credit=True):
        ...
        for symbol in candidates:    
            ...
            if credit:                
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.CREDIT_PUT_SPREAD) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
            else:
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.DEBIT_PUT_SPREAD) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
    ...
    def pick_iron_condor(self, watchlist, credit=True):
        ...
        for symbol in candidates:
            ...
            if credit:                
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.CREDIT_IRON_CONDOR) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
            else:
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.DEBIT_IRON_CONDOR) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
    ...
    def pick_call_butterfly(self, watchlist, credit=True):
        ...
        for symbol in candidates:
            ...
            if credit:                
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.CREDIT_CALL_BUTTERFLY) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
            else:
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.DEBIT_CALL_BUTTERFLY) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
    ...
    def pick_put_butterfly(self, watchlist, credit=True):
        ...
        for symbol in candidates:
            ...
            if credit:                
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.CREDIT_PUT_BUTTERFLY) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
            else:
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.DEBIT_PUT_BUTTERFLY) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
    ...
    def pick_iron_butterfly(self, watchlist, credit=True):
        ...
        for symbol in candidates:
            ...
            if credit:                
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.IRON_BUTTERFLY) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
            else:
                data_list = set(self.mdf[(self.mdf[optionSummary.SYMBOL]==symbol) &
                                     (self.mdf[optionSummary.STRATEGY]==st.REVERSE_IRON_BUTTERFLY) &
                                     (self.mdf[optionSummary.STATUS]==asset.OPENED)][optionSummary.EXP_DATE].unique())
...
if __name__ == '__main__':
    ...
    site = site('mysite', DEFAULT_SITE_STRATEGY)

    jihuang = site.create_user('jihuang')

    winwin = jihuang.create_account('single', initial_balance=100000)

    watchlist = jihuang.get_watchlist('max_gain')

    #winwin.try_open_new_strategy_positions(watchlist=watchlist)


    df = winwin.try_open_new_strategy_positions()
     
    #, st.LONG_PUT, st.COVERED_CALL, st.SHORT_PUT])
