...
class EMExperimentConfig:
    ...
    def __init__(self,
        ...
        if root_dir is not None:
            ...
            if len(zarrs) < 1:
                raise EMExperimentException("Provided root_dir does not contain any .zarr files")

        # otherwise validate DB call
        else:
            if acquisition_ids is None and study_id is None:
                raise EMExperimentException("Must provide either acquisition_ids or study_id.")
            ...
            if segmentation_versions is not None and biomarker_versions is not None:
                if len(segmentation_versions) != len(biomarker_versions):
                    raise EMExperimentException("Must provide same number of segmentation and biomarker versions.")

            if acquisition_ids is not None and study_id is not None:
                warning("Both acquisition_ids and study_id provided. \
    ...
    def _validate_config_for_enable_db(self) -> None:
        ...
        if self.segmentation_versions is not None and self.biomarker_versions is not None:
            if len(self.segmentation_versions) != len(self.biomarker_versions):
                raise EMExperimentException("Must provide same number of segmentation and biomarker versions.")
            if len(self.segmentation_versions) != len(self.acquisition_ids):
                raise EMExperimentException("Must provide same number of segmentation versions and acquisition ids.")
            if len(self.biomarker_versions) != len(self.acquisition_ids):
                raise EMExperimentException("Must provide same number of biomarker versions and acquisition ids.")

        if self.segmentation_versions is None and self.biomarker_versions is not None:
            warning("No segmentation version provided, but biomarker version provided. Attempting to infer biomarker versions.")
            if len(self.segmentation_versions) != len(self.acquisition_ids):
                raise EMExperimentException("Must provide same number of segmentation versions and acquisition ids.")

            # TODO: infer segmentation version from biomarker version
            inferred_seg_versions = []
            for i in range(len(self.acquisition_ids)):
                seg_version = db.get_segmentation_version_from_biomarker_expression_version(bm_version=self.biomarker_versions[i],
                                                                                            acquisition_id=self.acquisition_ids[i],
                                                                                            conn=db.connect_sf())
                inferred_seg_versions.append(seg_version)
                self.segmentation_version = inferred_seg_versions

        if self.biomarker_versions is None and self.segmentation_versions is not None:
            warning("No biomarker version provided, but segmentation version provided. Attempting to infer biomarker versions.")
            if len(self.biomarker_versions) != len(self.acquisition_ids):
                raise EMExperimentException("Must provide same number of biomarker versions and acquisition ids.")

            # TODO: infer biomarker version from segmentation version
            inferred_bm_versions = []
            for i in range(len(self.acquisition_ids)):
                bm_version = db.get_biomarker_version_from_segmentation_version(bm_version=self.biomarker_versions[i],
                                                                                acquisition_id=self.acquisition_ids[i],
                                                                                conn=db.connect_sf())
                inferred_bm_versions.append(bm_version)
                self.biomarker_versions = inferred_bm_versions

        if self.segmentation_versions is None and self.biomarker_versions is None:
            raise EMExperimentException("Must specify either segmentation version or biomarker versions. Run `config.get_available_versions()` to see available versions.")
...
class EMExperiment:
    ...
    def __init__(self,
                 ...
                 config: Optional[EMExperimentConfig] = None) -> EMExperiment:
        """
        Args:
        ...
        if config is None:
            if experiment_name is not None:
                ...
                warning("No experiment name provided. Using default name 'emExperiment'.")
        ...
        for group in self._groups.keys():
            if group == 'ungrouped':
                ...
                for subgroup in self._groups[group].keys():
                    print(f"\t\t{subgroup}: {len(self._groups[group][subgroup])} emObjects")
    ...
    def add_subgroup(self, groupname: str, subgroupname: str, acquisition_ids: list) -> None:
        ...
        '''
        if groupname not in self._groups.keys():
            raise EMExperimentException(f"Group {groupname} does not exist. Use add_group to add a group.")
    ...
    def autogroup(self, covariate: str, acquisition_ids: Optional[list] = None) -> None:
        ...
        possible_covariates = [cov.lower() for cov in possible_covariates if cov is not None]
        ...
        missing_ais = set(acquisition_ids) - set(covariate_meta['ACQUISITION_ID'].unique())
        if len(missing_ais) > 0:
            raise EMExperimentException(f"Acquisition ids {missing_ais} do not have metadata covariate {covariate} assigned.")
        ...
        for value in covariate_values:
            ...
            value_ais = covariate_meta[covariate_meta['FEATURE_VALUE'].str.lower() == value]['ACQUISITION_ID'].unique().tolist()
    ...
    def add_acquisition(self, acq_id: Union[str, list], groupname: Optional[str] = None, subgroup: Optional[str] = None) -> None:
        ...
        if groupname is not None and subgroup is None:
            raise EMExperimentException("Must specify subgroup if groupname is specified.")
        ...
        if groupname not in self._groupnames:
            ...
            if subgroup not in self._groups[groupname].keys():
                ...
                warning(f"Subgroup {subgroup} does not exist. Creating a new subgroup...")
                ...
                for ai in acq_id:
                    if ai not in self._groups[groupname][subgroup]:
                        ...
                        warning(f"Acquisition {acq_id} already exists in group {groupname}. Skipping...")
    ...
    def get_experiment_metadata(self) -> pd.DataFrame:
        ...
        if self._acquisition_ids is not None:
            if type(self._acquisition_ids) is str:
                ...
                assert type(self._acquisition_ids) is list, "acquisition_ids must be a list of acquisition ids"

            meta = db.get_all_metadata_for_acquisition_id(self._acquisition_ids, conn=db.connect_sf())
            self._meta = meta
    ...
    def build(self,
              ...
              enable_internal_mode: Optional[bool] = False) -> None:
        '''
        ...
        if not local_mode and not enable_internal_mode:
            raise EMExperimentException("Must specify either local_mode or enable_internal_mode.")

        if local_mode and enable_internal_mode:
            raise EMExperimentException("Cannot specify both local_mode and enable_internal_mode.")
        ...
        if local_mode:
            ...
            if len(self._acquisition_ids) > 0:
                ...
                for acq_id in self._acquisition_ids:
                    if not os.path.exists(os.path.join(self._rootdir, f"{acq_id}.zarr")):
                        raise EMExperimentException(f"Acquisition {acq_id} not found in root directory.")
                    else:
                        self._zarrpaths[acq_id] = os.path.join(self._rootdir, f"{acq_id}.zarr")
            ...
            # It should be able to generate acquisition IDs from .zarr path names, I think this is how.
                ...
                if len(zarrs) < 1:
                    raise EMExperimentException("Provided zarr_dir does not contain any .zarr files")
                self._acquisition_ids = [os.basename(zarrs).split(".", 1)[0] for zarr in zarrs]
            ...
            if self.config is None:
                raise EMExperimentException("Cannot build emExperiment without providing an EMExperimentConfig.")
            ...
            if (self.config.biomarker_versions is None or self.config.biomarker_versions is None) and self._biomarker_versions is None and self._segmentation_versions is None:
            ...
            if self.config.study_id is not None and self.config.acquisition_ids is None:
                self.acquisition_ids = db.get_all_acquisition_ids_for_study_id(self.config.study_id,
                                                                               conn=db.connect_sf())
            ...
            for i in range(0, len(self._acquisition_ids)):
                emobject_configs.append(EMObjectConfig(acquisition_id=self.config.acquisition_ids[i],
                                        study_id=None,
            ...
            for acq_id in self.acquisitions:  # for some reason self._acquisition_ids is empty here? seems wrong.
                if not os.path.exists(os.path.join(self._rootdir, f"{acq_id}.zarr")):
                    raise EMExperimentException(f"Acquisition {acq_id} not found in root directory.")
                else:
                    self._zarrpaths[acq_id] = os.path.join(self._rootdir, f"{acq_id}.zarr")
    ...
    def get_available_versions(self) -> pd.DataFrame:
        ...
        if self._study_id is not None:
            pass  # TODO: handling if you pass a study with the intention of getting all available acquisition_ids

        else:
            study_ids = []
            for ai in self._acquisition_ids:
                si = db.get_study_id_for_acquisition_id(acquisition_id=ai, conn=db.connect_rds())
                study_ids.append(si)
            version_info = []

            for study_id in set(study_ids):
                study_id = str(study_id)
                version_df = db.get_biomarker_and_segmentation_versions_for_study_acquisitions(study_id=study_id, conn=db.connect_rds())
                version_info.append(version_df)

            if len(version_info) > 1:
                version_df = pd.concat(version_info)
            else:
                version_df = version_info[0]

            # this didn't ensure that you didn't pull acquisition ids that weren't in the original list
            version_df.set_index('ACQUISITION_ID', inplace=True)
            version_df = version_df.loc[self._acquisition_ids]
            version_df.dropna(inplace=True, how='any', axis=0)  # drop any rows that have a nan in them
            version_df = version_df.astype({'BIOMARKER_EXPRESSION_VERSION': int, 'SEGMENTATION_VERSION': int})

            # now check if there are any acquisitions that don't have a biomarker or segmentation version
            if len(version_df) != len(self._acquisition_ids):
                missing_acqs = set(self._acquisition_ids) - set(version_df.index)
                warning(f"Could not find biomarker or segmentation versions for the following acquisitions: {missing_acqs}")

            # TODO: Could automate the selection of versions if there's only one per acquisition id
    ...
    def segmentation_versions(self, segmentation_versions: List[Union[int, str]]) -> None:
        if len(segmentation_versions) != len(self._acquisition_ids):
            raise EMExperimentException("Number of segmentation versions provided does not match number of acquisitions.")
    ...
    def biomarker_versions(self, biomarker_versions: List[Union[int, str]]) -> None:
        if len(biomarker_versions) != len(self._acquisition_ids):
            raise EMExperimentException("Number of biomarker versions provided does not match number of acquisitions.")
