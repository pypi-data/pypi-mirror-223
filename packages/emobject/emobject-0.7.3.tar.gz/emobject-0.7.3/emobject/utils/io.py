...
def from_anndata(adata: AnnData,
                 ...
                 assay: Optional[str] = 'visium') -> emo.EMObject:
...
def layer_from_anndata(adata: AnnData,
                       dtype=int,
                       include_uns: Optional[bool] = False,
                       name: str = None,
                       assay: Optional[str] = 'visium',
                       spot_size: Optional[float] = None,
                       scale_factor: Optional[float] = None) -> emo.EMObject:
    '''Converts an anndata object to an EMObject.
...
def object_from_files(data: str,
                      obs: str,
                      var: str,
                      ...
                      delimiter: Optional[str] = ',') -> emo.EMObject:
...
def layer_from_files(data: str,
                     ...
                     spot_size: Optional[float] = None,
                     scale_factor: Optional[float] = None) -> BaseLayer:
    '''Converts files to an EMObject.
...
def from_geomx(workflow_directory, image_directory, save_directory):
    ...
    pkc_files = [file for file in pkc_files if not os.path.basename(file).startswith('.')]
    ...
    xml_file_paths = [os.path.join(image_directory, "".join([x, '.ome.xml']))
                      for x in image_file_names]
    img_file_paths = [os.path.join(image_directory, "".join([x, '.ome.tiff']))
                      for x in image_file_names]
...
def _extract_rois_from_xml(xml_fpath, validate=False):
    ...
    for i in range(len(rois)):
        ...
        for mask in models:
            ...
            bin_mask = _read_bindata_mask(dict(mask['bin_data'])['value'], width=mw, height=mh)
...
def __combine_rows_by_index(df, indices_to_combine, new_index):
    ...
    index_mapping = {idx: new_index if idx in indices_to_combine else idx for idx in df.index}
...
def _read_region_annotations(directory,
...
def _read_segment_properties(directory, file_name_pattern='Export1', sheet_name='SegmentProperties'):
    ...
    seg_properties[['slide name', 'ROI', 'segment']] = seg_properties['SegmentDisplayName'].str.split(" \| ", expand=True)  # noqa: W605
