    ...
    def _validate_config(self):
        ...
        if self.include_seg_mask:
            if self.seg_mask_type not in ['nucleus', 'cell']:
                raise EMObjectException(f"Specified segmentation mask type {self.seg_mask_type} unknown.")

        # Avoid the issue where user specifies an image but forces an empty biomarker list
        if self.include_img and (self.biomarkers == [] or self.biomarkers == np.array([])):
            warning("include_img is True, but no channels/biomarkers. Setting include_img to False.")
            self.include_img = False


def add_layer_from_enable_db(E: EMObject = None, config: EMObjectConfig = None) -> BaseLayer:
    ...
    if config.include_img:
        config.img = build_emimage_from_enable_db(acquisition_id=config.acquisition_id,
...
def build_emmask_from_enable_db(acquisition_id: Optional[str] = None,
    ...
    for j in range(1, len(new_seg_ids)):  # we skip 0 because that's a special background value.
    ...
    return EMMask(masks=np.stack(acquisition_masks, axis=0), mask_idx=np.array(mask_names))


def build_emimage_from_enable_db(acquisition_id: Optional[str] = None,
                                 study_id: Optional[int] = None,
...
def save(E: EMObject = None,
    ...
    if E.img is not None:
        ...
        _ = img.create_dataset(name='CODEX', data=E.img.img, compressor=compressor) # noqa
        _ = img.create_dataset(name="CODEX_ax", data=E.img.channels, compressor=compressor,
                               dtype=object,
                               object_codec=JSON())
    ...
    if E.mask is not None:
        ...
        if E.mask._style == 'tensor':
            _ = mask.create_dataset(name='masktensor', data=E.mask.mask, compressor=compressor)
            _ = mask.create_dataset(name='maskix', data=E.mask.mask_names)
        else:
            _ = mask.create_dataset(name='maskix',
                                    data=E.mask.mask_names,
                                    dtype=object,
                                    object_codec=JSON())
            _ = mask.create_dataset(name='maskdims',
                                    data=E.mask.dims,
                                    dtype=object,
                                    object_codec=JSON())
    ...
    for layer in E.layers:
        ...
        if E._layerdict[layer].data is not None:
            _ = layer_group.create_dataset(name='data',
        ...
        if E._layerdict[layer]._obs is not None:
            _ = layer_group.create_dataset(name='obs',
                                           data=E._layerdict[layer]._obs.to_numpy(),
                                           compressor=compressor,
                                           dtype=object,
                                           object_codec=JSON())

            _ = layer_group.create_dataset(name='obs_y',
        ...
        if E._layerdict[layer]._var is not None:
            _ = layer_group.create_dataset(name='var',
                                           data=E._layerdict[layer]._var.to_numpy(),
                                           compressor=compressor,
                                           dtype=object,
                                           object_codec=JSON())
            _ = layer_group.create_dataset(name='var_y',
        ...
        if E._layerdict[layer]._sobs is not None:
            _ = layer_group.create_dataset(name='sobs',
                                           data=E._layerdict[layer]._sobs.to_numpy(),
                                           compressor=compressor,
                                           dtype=object,
                                           object_codec=JSON())

            _ = layer_group.create_dataset(name='sobs_y',
        ...
        _ = E.pos  # in case it wasn't ever made ou need to call this.

        for coord_sys in list(E._layerdict[layer]._pos.keys()):
            pos.create_dataset(name=coord_sys, data=E._layerdict[layer]._pos[coord_sys], compressor=compressor)
            pos.create_dataset(name=f"{coord_sys}_y",
        ...
        ix.create_dataset(name='obsax',
                          data=E._layerdict[layer]._obs_ax.to_numpy())
        ix.create_dataset(name='varax',
                          data=E._layerdict[layer]._var_ax.to_numpy(),
                          dtype=object,
                          object_codec=JSON())

        if E._sobs_ax is not None:
            ix.create_dataset(name='sobsax',
                              data=E._layerdict[layer]._obs_ax.to_numpy())
    ...
    state_dict = {'version': __version__,
                  'defaultlayer': E._defaultlayer,
                  'layers': {}}

    for layer in E.layers:
        state_dict['layers'][layer] = {'assay': E._layerdict[layer]._assay,
...
def load(path: str = None) -> EMObject:
    ...
    with open(os.path.join(path, 'state.yml'), 'r') as stream:
        ...
        for layer in E.layers:
            ...
            E._layerdict[layer]._scale_factor = state_dict['layers'][matching_key]['scale_factor']
            E._layerdict[layer]._spot_size = state_dict['layers'][matching_key]['spot_size']
...
def __load_pos(pos_path: str = None,
               obs_ix=None) -> dict:
...
def __load_img(path: str = None) -> list:
    ...
    if os.path.exists(os.path.join(path)):
        ...
        if not len(emimg_to_add) == 1:
            raise EMObjectException("Attempted to add multiple images to single EMObject. This is not yet implemented.")
...
def __load_deprecated(path: str = None) -> EMObject:
    ...
    if os.path.exists(os.path.join(path, 'core')):
        ...
        if os.path.exists(os.path.join(path, 'core', 'pos')):
            ...
            # get position info
            coord_sys_to_add = glob.glob(os.path.join(pos_path, '*'))
            coord_sys_to_add = np.array([c.split('/')[-1].split('_')[0] for c in coord_sys_to_add])
            ...
            for coord_sys in coord_sys_to_add:
                data = zarr.convenience.load(os.path.join(pos_path, coord_sys))
                cols = zarr.convenience.load(os.path.join(pos_path, f"{coord_sys}_y"))
                _posdict[coord_sys] = pd.DataFrame(data=data, columns=cols, index=obs_ix)
    ...
    E = EMObject(data=pd.DataFrame(data=data, columns=var_ax, index=obs_ix),
                 obs=obs,
                 var=var,
                 pos=_posdict,
                 sobs=sobs,
