...
class BaseLayer:
    def __init__(self,
                 ...
                 spot_size: Optional[float] = None) -> BaseLayer:
        """
    ...
    def name(self, value) -> None:
        if type(value) != str:
            raise EMObjectException(f'Layer names must be of type str, but received\
    ...
    def _get_data_axes(self, data) -> tuple:
        ...
        if type(data) == pd.DataFrame:
            ...
            raise EMObjectException(f"Expected data of type np.ndarray, \
    ...
    def _build_pos(self,
        ...
        if pos is not None:
            if type(pos) == dict:
                ...
                cols = ['x', 'y', 'z']
                pos_dict[self._lname] = pd.DataFrame(pos.astype(np.float32), index=_obs_ax, columns=cols[:pos.shape[1]])
        # To Do: align the indices.
        else:
...
class LayeredData:
    ...
    def __init__(self,
                 initial_layer: Optional[BaseLayer] = None) -> LayeredData:
