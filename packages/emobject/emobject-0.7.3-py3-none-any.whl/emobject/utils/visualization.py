...
def plot_cells(E: EMObject = None,
    ...
    x = np.multiply(E.pos[coordinate_system].iloc[:, 0].to_numpy(dtype=np.float32), scale_factor)
    y = np.multiply(E.pos[coordinate_system].iloc[:, 1].to_numpy(dtype=np.float32), scale_factor)
    size *= scale_factor
...
def plot_channels(E: EMObject = None,
                  channels: Optional[list] = None,
                  colors: Optional[list] = ['blue', 'magenta', 'green', 'yellow', 'red', 'cyan', 'white'],
    ...
    if len(channels) > len(colors) or len(channels) > len(color_dict):
        raise EMObjectException('Too many channels to plot. \
...
def plot_mask_overlay(E: EMObject = None,
...
def plot_cell_overlay(E: EMObject = None,
                      coordinate_system: Optional[str] = None,
                      factor: Optional[str] = None,
                      size: Optional[float] = 3,
    ...
    if coordinate_system is None:
        if len(list(E.pos.keys())) == 1:
            ...
            raise EMObjectException(f'Multiple coordinate systems found. Please specify one of {list(E.pos.keys())}.')
