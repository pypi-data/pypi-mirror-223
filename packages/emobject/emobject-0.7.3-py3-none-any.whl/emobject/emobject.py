...
class EMObject(LayeredData, BaseLayer):
    ...
    def __init__(self,
                 ...
                 first_layer_name: Optional[str] = None) -> EMObject:
    ...
    def _build_meta(self,
                    meta: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        """
        Store meta data
        """
    ...
    def _build_seg(self, coord_sys=None) -> np.ndarray:
        ...
        if self.mask is None:
            ...
            raise EMObjectException("Spatial information not stored in pos, \
                cannot generate seg.")
        ...
        if coord_sys is None:
            ...
            if len(self._layerdict[self._activelayer]._pos.keys()) > 1:
                warning(f"Multiple spatial coordinates detected, using first coordinate set: {list(self._layerdict[self._activelayer]._pos.keys())[0]}. If build fails, try specifying a coordinate system with E.build_seg(coord_sys='X').")
        ...
        for mask_name in n_seg.keys():
            ...
            try:
                coords = self._layerdict[self._activelayer]._pos[orig_coord_key].to_numpy()
            except AttributeError:
                if self._layerdict[self._activelayer] == 'visium':
                    warning("Visium data detected, applying scalefactor to segment coordinates.\
                            Be sure the appropriate scalefactor is set (E.scalefactor = X)")

                    coords = self._layerdict[self._activelayer]._pos[orig_coord_key].to_numpy() * self.scale_factor
                else:
                    coords = self._layerdict[self._activelayer]._pos[orig_coord_key].to_numpy()
            ...
            for si in seg_ids:
                if si == 0:
                    continue
                val = mask[coords[:, 1], coords[:, 0]]  # are x and y reversed?
                idx, = np.where(val == si)
                seg[idx, mask_idx] = si
            mask_idx += 1
        return seg
    ...
    def var(self, value: Optional[Union[np.array, pd.DataFrame]]) -> None:
        if value.shape[0] != self.var_ax.shape[0]:
            raise EMObjectException("Must be a `n_var` length array of arbitrary\
                 width.")
    ...
    def obs(self, value: Optional[Union[np.array, pd.DataFrame]]) -> None:
        if value.shape[0] != self.obs_ax.shape[0]:
            raise EMObjectException("Must be a `n_obs` length array of arbitrary\
                 width.")
...
'''
    ...
    def add_anno(self,
                 ...
                 mask: Optional[Union[int, str]] = None) -> None:
        ...
        if name is not None:
            ...
            if name.shape[0] < n_names_reqd:
                ...
                warning(f"Expected {n_names_reqd} names but received \
                    {name.shape[0]} names. Generating dummy annotation names.")
                n_new_names = n_names_reqd - name.shape[0]
                new_names = np.array([f"anno_{i}" for i in range(0,
                                                                 n_new_names)])
                name = np.concatenate([name, new_names])
        else:
            if type(value) != pd.DataFrame:
                #  No names provided, handle this.
                warning("No annotation names received. Generating dummy annotation\
                    names.")
        ...
        if attr == 'var':
            ...
            for n in name:
                if n in existing_annos:
                    raise EMObjectException(f"The annotation name {n} already \
                        exists in the {attr} attribute. Please provide a unique \
                        name for the new annotation.")

            if type(value) == np.ndarray:
                _appenddf = pd.DataFrame(data=value,
                                         columns=name,
                                         index=self._layerdict[layer]._var_ax)
            elif type(value) == pd.DataFrame:
                _appenddf = value
                if name is not None:
                    _appenddf.columns = name
                if not _appenddf.index.equals(self._var_ax):
                    warning("Provided dataframe index does not match the var axis. Attempting to join on index.")

            if self._layerdict[layer]._var is None or self._layerdict[layer]._var.shape[1] == 0:
                self._layerdict[layer]._var = _appenddf
            else:
                # left join on index
                self._layerdict[layer]._var = self._layerdict[layer]._var.join(_appenddf, how='left')
            ...
            if type(value) == np.ndarray:
                _appenddf = pd.DataFrame(data=value,
                                         columns=name,
                                         index=self._layerdict[layer]._obs_ax)
            elif type(value) == pd.DataFrame:
                _appenddf = value
                if name is not None:
                    _appenddf.columns = name
                if not _appenddf.index.equals(self._layerdict[layer]._obs_ax):
                    warning("Index of new annotation does not match obs index. Attempting to join on index value.")

            if self._layerdict[layer]._obs is None or self._layerdict[layer]._obs.shape[1] == 0:
                self._layerdict[layer]._obs = _appenddf
            else:
                # join the new annotation horizontally on the index
                self._layerdict[layer]._obs = self._layerdict[layer]._obs.join(_appenddf, how='left') # noqa
            ...
            if type(mask) == str:
                if mask not in self.mask.mask_names:
                    raise EMObjectException(f"Mask name {mask} is not in the current\
                     object.")
            else:
                raise EMObjectException(f"Mask must be a string. Received {type(mask)}.")
            ...
            if value.shape[0] != anno_idx.shape[0]:
                if anno_idx.shape[0] - value.shape[0] == 1:
                    ...
                    raise EMObjectException('Received one fewer segment annotations than expected.\
                    ...
                    raise EMObjectException(f"Annotation length does not match the\
            ...
            if self._layerdict[layer]._obs is None or self._layerdict[layer]._obs.shape[1] == 0:
                ...
                self._layerdict[layer]._sobs[mask] = pd.concat([self._layerdict[layer]._sobs[mask], # noqa
                                                                _appenddf],
                                                               axis=1)
        else:
            raise EMObjectException(f"Unrecognized attribute to annotate. Must be one\
    ...
    def del_anno(self,
                 ...
                 name: Optional[str] = None,
                 layer: Optional[str] = None,
                 mask: Optional[Union[str, int]] = None) -> None:
        ...
        if attr is not None:
            if attr == 'obs':
                ...
                self._layerdict[layer]._obs.drop(columns=name,
                                                 axis=0,
                                                 inplace=True)
            elif attr == 'var':
                assert name in self._layerdict[layer]._var.columns
                self._layerdict[layer]._var.drop(columns=name,
                                                 axis=0,
                                                 inplace=True)
            elif attr == 'sobs':
                # check that segment is valid.
                ...
                if type(mask) == str:
                    if mask not in self.mask.mask_names:
                        raise EMObjectException(f"Mask name {mask} is not in the\
                             current object.")
                    else:
                    ...
                    if mask >= len(self.mask.mask_names) or mask < 0:
                        raise EMObjectException(f"Mask index {mask} is \
                            out of range for the current object.")

                assert name in self._layerdict[layer]._sobs[mask].columns

                # delete the annotation
                self._layerdict[layer]._sobs[mask].drop(columns=name,
                                                        axis=0,
                                                        inplace=True)
        self._validate()
    ...
    def loc(self,
            ...
            layer: Optional[str] = None) -> EMObject:
    ...
    def slice(self,
        ...
        for layer in layers:
            ...
            if self._layerdict[layer].segmentation is None and self._layerdict[layer]._assay == 'visium':
                try:
                    test_key = list(self._layerdict[layer].obs.keys())[0]
                    visium_mask = helpers.build_visium_segmentation_mask(spot_coords=self._layerdict[layer].pos[test_key],
                    ...
                    raise EMObjectException("Could not build segmentation mask for Visium data. Be sure to specify a spot_size in the metadata.\
            ...
            assert len(self._layerdict[layer].data) == len(self._layerdict[layer].pos[test_key])
        ...
        for coord_sys in self._layerdict[anchor_layer]._pos.keys():
            pos_dict[coord_sys] = self._layerdict[anchor_layer]._pos[coord_sys].loc[obs_subset, :]
        ...
        try:
            sparse_anchor_mask = sparse.coo_matrix(self.mask.mloc(self._layerdict[anchor_layer].segmentation))
        except TypeError:
            sparse_anchor_mask = sparse.coo_matrix(self.mask.mloc(self._layerdict[anchor_layer].segmentation).squeeze())
        obs_subset_anchor_mask_ix, = np.where(np.isin(sparse_anchor_mask.data, obs_subset))  # this is the "binarized array"
        ...
        for layer in layers:
            if layer != anchor_layer:
                ...
                subset_ids = np.unique(layer_segmentation[obs_subset_anchor_mask_ix_rr, obs_subset_anchor_mask_ix_cc])
                ...
                pos_dict = {}
                for coord_sys in self._layerdict[layer]._pos.keys():
                    pos_dict[coord_sys] = self._layerdict[layer]._pos[coord_sys].loc[subset_ids, :]

                new_layer = BaseLayer(data=self._layerdict[layer].data.loc[subset_ids, :],
    ...
    def _observations_for_segment(self,
        ...
        for si in segment_id:
            ...
            if len(segment_ix) == 0:
                ...
                target_observations = np.unique(self.mask.mloc(self._layerdict[target_layer].segmentation)[segment_ix_rr, segment_ix_cc])
    ...
    def _validate(self) -> None:
        ...
        for layer in self.layers:
            ...
            obs_ix_data = np.array(self._layerdict[layer].data.index)  # use arrays for consistency
            ...
            if self._layerdict[layer]._obs is not None:
                if not np.array_equal(np.array(self._layerdict[layer]._obs.index), obs_ix_data):
                    # the content of the index is not the same, check if the length is
                    if len(self._layerdict[layer]._obs.index) != len(obs_ix_data):
                        raise EMObjectException('Observation index mismatch (different lengths!).')
                    else:
                        # the length is the same, check if the order is the same
                        warning('Content of observation index is not the same as data. Renaming data index...')
                        self._layerdict[layer].data.index = self._layerdict[layer]._obs.index
            ...
            if self._layerdict[layer].var is not None:
                if not np.array_equal(np.array(self._layerdict[layer].var.index), var_ix_data):
                    # the content of the index is not the same, check if the length is
                    if len(self._layerdict[layer].var.index) != len(var_ix_data):
                        raise EMObjectException('Variable index mismatch (different lengths!).')
                    else:
                        # the length is the same, check if the order is the same
                        warning('Content of variable index is not the same as data. Renaming data columns...')
                        self._layerdict[layer].data.columns = self._layerdict[layer].var.index
        ...
        if self._layerdict[layer]._pos is not None:
            for key in self._layerdict[layer]._pos.keys():
                if not np.array_equal(np.array(self._layerdict[layer]._pos[key].index), np.array(self._layerdict[layer].data.index)):
                    raise EMObjectException('Observation index mismatch (pos).')

        # Check that segment axis is consistent
        if self._seg is not None and self._mask is not None and self._seg.shape[1] != len(self._mask.mask_names):
    ...
    def add_coordinate_system(self,
        ...
        if label is None:
            raise EMObjectException("Must provide a name/label for this new coordinate system.")

        if coords is None:
            raise EMObjectException("Must provide coordinate values for this new coordinate system.")
        ...
        if type(coords) == np.ndarray:
            ...
            coords = pd.DataFrame(coords, index=self._layerdict[layer].data.index, columns=cols)
        _ = self.pos  # make sure pos exists
        self.pos[label] = coords

    def add_mask(self,
                 mask: Optional[np.ndarray] = None,
                 mask_name: Optional[Union[np.ndarray, list]] = None) -> None:
    ...
    def set_layer_segmentation(self, layer: Optional[str], segmentation: str) -> None:
        ...
        if layer is None:
            layer = self._activelayer
            warning(f"Layer not specified, setting segmentation for active layer {layer}.")
    ...
    def drop_obs(self, obs: Union[list, np.ndarray]) -> None:
        ...
        self._layerdict[self._activelayer]._obs_ax = np.array(self._layerdict[self._activelayer].data.index)
        ...
        if self._seg is not None:
            ...
            try:
                ...
                warning("Failed to reconstruct `.seg`. Please pass an explicit coordinate system to `E.build_seg()`.")
                pass
    ...
    def drop_var(self, var: Union[list, np.ndarray]) -> None:
        ...
        self._layerdict[self._activelayer]._var_ax = np.array(self._layerdict[self._activelayer].data.columns)
    ...
    def add_measurements(self,
        ...
        if type(measurements) == np.ndarray:
            measurements = pd.DataFrame(measurements, index=self._layerdict[layer].data.index, columns=var_names)

        if measurements.shape[0] != self.n_obs:
            raise EMObjectException(f"New measurements must have the same number of rows as existing data objects. Found {measurements.shape[0]} rows, expected {self.n_obs}.")

        if not np.all(measurements.index == self._layerdict[layer].data.index):
            raise EMObjectException("New measurements must have the same index as existing observations.")

        self._layerdict[layer].data = pd.concat([self._layerdict[layer].data, measurements], axis=1)
        self._layerdict[layer]._var_ax = np.array(self._layerdict[layer].data.columns)

        # also need to update var, which is indexed
        if self._layerdict[layer].var is not None and self._layerdict[layer].var.shape[1] > 0:
            self._layerdict[layer].var = pd.concat([self._layerdict[layer].var, pd.DataFrame(np.empty(measurements.shape[1], self.var.shape[1]), index=measurements.columns)], axis=0)
        else:
            self._layerdict[layer].var = pd.DataFrame(data=None, index=self._layerdict[layer]._var_ax)

        self._validate()

    def slice_on_segment(self,
                         segment_id: Union[int, list, np.ndarray],
                         mask_name: str = None,
        ...
        return self._observations_for_segment(segment_id=segment_id,
                                              mask_name=mask_name,
