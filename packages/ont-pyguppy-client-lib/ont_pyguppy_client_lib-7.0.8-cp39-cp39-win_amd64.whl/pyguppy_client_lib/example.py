...
# **helper_functions** A set of functions for running a Guppy basecall server and
# loading reads from fast5 and/or pod5 files.
# **client_lib** A compiled library which provides direct Python bindings to Guppy's
# C++ API.
# **pyclient** A user-friendly wrapper around **client_lib**. This is what you
# should use to interact with a Guppy basecall server.
...
# --------------------------
# There must be a Guppy basecall server running in order to communicate with it.
...
# The second argument is the directory where the guppy_basecall_server executable
