...
from Swagger2Tests.constants.base_constants import *
import requests
        ...
        for path, pathData in pathsValue.items():
            for method, methodData in pathData.items():
                ...
                for items in methodData['parameters']:
                    ...
                    if 'originalRef' in items['schema']:
    ...
    def parsePathRequestBody(cls, params: dict):
        for key, value in params.items():
            if isinstance(value, dict):
                ...
                if value == 'integer':
                    params[key] = Helper.get_random_int()
                if value == 'boolean':
                    params[key] = Helper.get_random_bool()
                if value == 'number':
                    params[key] = Helper.get_random_float()
        return params
