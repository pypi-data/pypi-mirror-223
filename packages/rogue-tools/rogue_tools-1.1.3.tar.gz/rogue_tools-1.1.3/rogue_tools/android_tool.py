...
class DevicesManager(metaclass=thread_tool.Singleton):
    ...
    def init_device_list(self, try_times=15):
        '''连接设备'''
        for _ in range(try_times):
            ...
            # 提取设备名称，忽略空行
            device_names = [
                re.match(r'(.+?)(?:\tdevice|$)', line).group(1) 
                for line in result.stdout.split('\n')[1:]
                if line.strip()
            ]
            # 创建设备对象字典
            self.devices = {
                device_name: Android(self.adb_path, device_name) 
    ...
    def run_in_all(self, adb_cmd, device_name=None):
    ...
    def install_apk(self,local_path,device_name=None):
        ...
        return self.run_in_all(install_apk_cmd,device_name)
