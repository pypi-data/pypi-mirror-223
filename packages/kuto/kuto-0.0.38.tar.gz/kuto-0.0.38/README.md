...
class HomePage(kuto.Page):
    ...
    SETTING = kuto.Elem(text='设置')


class TestCaseClass(kuto.Case):
    ...
    def test_case1(self):
        ...
        assert '设置' in self.driver.page_content
```
...
class TestParameter(kuto.Case):
    ...
    @file_data(file='test_data.json')
    def test_json(self, param):
        print(param)

    @file_data(file='test_data.yml', key='names')
    ...
    @file_data(file='test_data.xlsx', row=1)
