...
class TestCase:
    ...
    def __init__(self, function = (lambda *args, **kwargs: ((args), kwargs) if kwargs else args), *_, **kwargs):
        if len(kwargs) == 0: self.mode = TestCase._Mode.match
        elif len(kwargs) == 1 and ("validator" in kwargs): self.mode = TestCase._Mode.validate; self.validator = kwargs["validator"]
...
def parametrized(
    expected_io: list[
        ...
        (tuple[tuple[Any, ...] | list[Any], dict[str, Any], Any | TestCase | Exception]) |
        (list[tuple[Any, ...] | list[Any], Any | TestCase | Exception]) | 
        (list[tuple[Any, ...] | list[Any], dict[str, Any], Any | TestCase | Exception])
    ...
    class _TestMode(Enum):
        ...
        @classmethod
        def from_expect(cls: _TestMode, expect: Any | TestCase | Exception) -> _TestMode:
            # Exception test:
            try:
    ...
    def parametrized_decorator(function):
        def parse_case(*, args: Optional[tuple[Any, ...] | list[Any]] = (), kwargs: Optional[dict[str, Any]] = {}, expect: Any | TestCase | Exception) -> None:
            ...
            match test_mode:
                case _TestMode.normal_case:
                    ...
                    try:
                        result = function(*args, **kwargs) if kwargs else function(*args);callable_expect = callable(expect);expectation_passed = expect(result) if callable_expect else expect == result
                        if expectation_passed: print(f"\x1B[38;5;154mTest passed: {info} -> {repr(result)} {f'so {expect.__name__}({repr(result)}) was truthy' if callable_expect else f'== {repr(expect)}'}\x1B[0m")
                        else: print(f"\x1B[38;5;196mTest failed: {info} -> {repr(result)} {f'so {expect.__name__}({repr(result)}) was falsy' if callable_expect else f'instead of {expect}'}\x1B[0m")
                    except Exception as exc: print(f"\x1B[38;5;99mTest raised an exception: {info} -> {repr(exc)}\x1B[0m") # something gone wrong, because this is not an exception test, so the test basically "failed"
                case _TestMode.case_test:
                    ...
                    try: result_0 = function(*args, **kwargs) if kwargs else function(*args)
                    except Exception as exc_0_buf: exc_0 = exc_0_buf
                    try: result_1 = expect.function(*args, **kwargs) if kwargs else expect.function(*args)
                    except Exception as exc_1_buf: exc_1 = exc_1_buf
                    if exc_0 and exc_1: print(f"\x1B[38;5;196mCase test failed: both functions raised an exception: {repr(exc_0)} : {repr(exc_1)}\x1B[0m") # Both functions raised an exception
                    elif exc_0 and not exc_1: print(f"\x1B[38;5;196mCase test failed: {function.__name__} raised an exception {repr(exc_0)}\x1B[0m") # Only the first (decorated) function raised an exception
                    elif exc_1 and not exc_0: print(f"\x1B[38;5;196mCase test failed: {expect.function.__name__} raised an exception {repr(exc_1)}\x1B[0m") # Only the second (provided in the case instance) function raised an exception
                    if exc_0 or exc_1: return None
                    match expect.mode:
                        case TestCase._Mode.match:
                            if result_0 == result_1: print(f"\x1B[38;5;154mMatch case passed: {info} == {buf_info} -> {repr(result_0)} | {repr(result_1)}\x1B[0m") # Match passed
                            else: print(f"\x1B[38;5;196mMatch case failed: {info} -> {repr(result_0)} ; {buf_info} -> {repr(result_1)}\x1B[0m") # Match failed
                        case TestCase._Mode.validate:
                            buf_res: Any = expect.validator(result_0, result_1); buf_info: str = f"{expect.validator.__name__}({repr(result_0)}, {repr(result_1)})"
                            if buf_res: print(f"\x1B[38;5;154mValidation case passed: {buf_info} -> {buf_res} [is truthy]\x1B[0m") # Validation passed
                            else: print(f"\x1B[38;5;196mValidation case failed: {buf_info} -> {buf_res} [is falsy]\x1B[0m") # Validation failed
                case _TestMode.exception_test:
                    ...
                    try: result = function(*args, **kwargs) if kwargs else function(*args);print(f"\x1B[38;5;202mException test didnt result in an exception {info} -> {repr(result)} instead of raising {repr(expect())}\x1B[0m")
                    except Exception as exc:
                        if type(exc) == expect: print(f"\x1B[38;5;50mException test passed: {info} -> {repr(exc)}\x1B[0m")
                        else: print(f"\x1B[38;5;196mException test failed: {info} -> {repr(exc)} instead of {repr(expect())}\x1B[0m")
        def test_cases() -> None:
