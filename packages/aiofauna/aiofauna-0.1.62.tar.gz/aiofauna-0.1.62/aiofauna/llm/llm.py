...
class LLMStack(APIClient):
    ...
    async def upsert_messages(
        ...
        upsert_request = UpsertRequest(
            vectors=[
                UpsertVector(id=str(uuid4()),values=user_embedding, metadata={"text": prompt}),
                UpsertVector(id=str(uuid4()),values=openai_embedding, metadata={"text": text}),
            ],
            namespace=namespace,
        )
    ...
    @handle_errors
    async def ingest_bulk(self, request: IngestRequest, chunksize: int = 32):
        total_chunks = len(request.texts) // chunksize + (len(request.texts) % chunksize > 0)

        for chunk in tqdm(
            chunker(request.texts, chunksize),
            total=total_chunks,
            desc="Ingesting",
            unit="chunks",
        ):
...
async def parse_openai_response(  # pylint: disable=dangerous-default-value
    ...
    if "function_call" in choice:
        ...
        return FunctionResponse(function=name.lower(), data=result) 
    return FunctionResponse(function="chat", data=choice["content"])
