...
from pulsar_sdk_py.schemas.schemas import (
    NFTItem,
    Timeseries,
    ResolvedName,
    ProtocolData,
...
class PulsarSDK:
    ...
    def __init__(self, api_key):
        ...
        ws_client = PulsarSDK._WebsocketClient(api_key=api_key)
        self.tokens = self._TokenRestClient(headers=headers)
        self.name_service = self._NameServiceRestClient(headers=headers)
        self.nfts = self._NFTRestClient(headers=headers)
        self.protocols = self._ProtocolRestClient(headers=headers)
    ...
    class _TokenRestClient(_RestClient):
        ...
        def get_token_info_by_id(self, token_id: str) -> ExtendedToken:
            func_name = "get_token_info_by_id"
            response = self._RestClient__get_request_on_endpoint(
                func_name=func_name, request_type="GET", token_id=token_id
            )
