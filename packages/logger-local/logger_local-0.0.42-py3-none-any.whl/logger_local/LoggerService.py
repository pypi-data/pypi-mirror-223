...
class LoggerService:
    ...
    def __init__(self):
        if (logzio_token is None):
            raise Exception(
                "Please set in your .env file LOGZIO_TOEKN=cXNHuVkkffkilnkKzZlWExECRlSKqopE")
        self.fields = None
        self._writer = Writer()
    ...
    def get_logger_table_fields(self):
        if (self.fields is None and self.writeToSql is True):
            object1 = {
                'record': {'severity_id': 500, 'severity_name': 'Information', 'component_id': '102', 'component_name': 'Logger Python', 'message': "get_logger_table_fields activated"},
                'severity_id': 500,
                'component_id': 102,
    ...
    def get_component_info(self, component_id):
        ...
        if result:
            ...
            if (getattr(self, 'component_type') is None):
                setattr(self, 'component_type', component_type)
            if (getattr(self, 'component_category') is None):
                setattr(self, 'component_category', component_category)
            if (getattr(self, 'testing_framework') is None):
                setattr(self, 'testing_framework', testing_framework)
            if (getattr(self, 'api_type') is None):
    ...
    def setWriteToSql(self, value):
        ...
        self.get_logger_table_fields()
        for field in self.fields:
            setattr(self, field, self.additinal_fields.get(
