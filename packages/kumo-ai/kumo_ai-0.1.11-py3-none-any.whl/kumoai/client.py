...
class KumoClient:
    ...
    def list_training_jobs(
        ...
        url = f'{self._base_url}/predictive_queries/{pquery_id}/training_jobs'
        if job_status:
            url = f'{url}?job_status={job_status.value}'
        res = self._session.get(url)
        _check_success(res)
        return [
    ...
    def wait_training_job(self,
                          ...
                          *,
                          ...
                          check_success: bool = False) -> TrainingJobResource:
        ...
        Args:
            ...
            check_success (bool): If true, raise RuntimeError if the final job
            status is not successful (either failed or cancelled).
        ...
        if check_success and job.job_status_report.status != JobStatus.DONE:
            raise RuntimeError(
                f'Training job did not finish successfully: {job}')
        return job
    ...
    def wait_batch_prediction_job(
        ...
        *,
        ...
        if check_success and job.job_status_report.status != JobStatus.DONE:
            raise RuntimeError(
                f'Batch prediction job did not finish successfully: {job}')
        return job
