...
from eulith_web3.whitelists import ClientWhitelist, ClientWhitelistHash, get_client_whitelist_typed_data, CurrentWhitelists
...
def get_api_access_token(eulith_url: URI, eulith_refresh_token: str) -> ApiToken:
    """
    (DEPRECATED) You should authenticate by passing the hash of your refresh token
    in the URL like this: https://eth-main.eulithrpc.com/v0?token=<SHA256_HASH_REFRESH_TOKEN>
    ...
    warn('API access tokens are deprecated, use the hash of your '
         'refresh token in the url instead like '
         'https://eth-main.eulithrpc.com/v0?token=<SHA256_HASH_REFRESH_TOKEN>',
...
def refresh_token_to_refresh_hash(refresh_token: str) -> str:
...
class EulithData:
    ...
    def __init__(self, eulith_url: Union[URI, str],
        ...
        self.eulith_refresh_token_hash: str = refresh_token_to_refresh_hash(eulith_refresh_token)
        auth_params = {'token': self.eulith_refresh_token_hash}
        self.eulith_url = URI(add_params_to_url(str(eulith_url), auth_params))

        self.private = private
        self.eulith_refresh_token: str = eulith_refresh_token

        decoded = jwt.decode(self.eulith_refresh_token, options={"verify_signature": False})

        one_year = 60 * 60 * 24 * 365
        exp = decoded.get('exp', int(time.time()) + one_year)

        # We're using refresh tokens in the interim while we deprecate access tokens
        self.api_access_token = ApiToken(self.eulith_refresh_token, exp)
...
class EulithWeb3(Web3):
    ...
    def _make_http(self):
        ...
        http = HTTPProvider(endpoint_uri=url)
