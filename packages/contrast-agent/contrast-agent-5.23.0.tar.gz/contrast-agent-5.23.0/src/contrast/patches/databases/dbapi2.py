...
class Dbapi2Patcher(object):
    ...
    def _build_execute_patch(self):
        ...
        def build_execute_patch(orig_func, patch_policy):
            def patched_method(*args, **kwargs):
                ...
                """
                self._safe_create_arch_component_and_append_to_context()
    ...
    def _safe_create_arch_component_and_append_to_context(self):
        """
        Create ArchComponent with the given database information and append to context.database_info
        """
        context = contrast.CS__CONTEXT_TRACKER.current()
        if not context:
            return
        arch_comp = ArchitectureComponent(self.dbinfo).to_json()
        context.database_info.append(arch_comp)
