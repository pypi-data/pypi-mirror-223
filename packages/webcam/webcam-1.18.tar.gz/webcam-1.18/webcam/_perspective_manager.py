...
class _PerspectiveManager:
    def __init__(self, homography_matrix: np.ndarray|list[list[float], ...], default_w: int, default_h: int,
        ...
        homography_matrix = self.__apply_rescale_to_homography_matrix(m=homography_matrix, w=default_w, h=default_h)
    ...
    def __apply_rescale_to_homography_matrix(self, m: np.ndarray, w: int, h: int,
                                             original_aspect_ratio: float|None = None) -> np.ndarray:
        """
        Apply a rescale to the given homography matrix
        :param m: np.ndarray. The matrix to be rescaled.
        :param w: int. The new width.
        :param h: int. The new height.
        :param original_aspect_ratio: float. The original aspect ratio (w/h) of the image where the homography matrix
        was estimated. If None, the aspect ratio is estimated based on the new image size.
        :return: np.ndarray. The rescaled matrix.
        """
        if original_aspect_ratio is None:
            original_aspect_ratio = w / h

        # Estimate the original width and height based on the new image size and aspect ratio
        original_width_estimate = h * original_aspect_ratio
        original_height_estimate = w / original_aspect_ratio

        # Calculate scale factors
        scale_x = w / original_width_estimate
        scale_y = h / original_height_estimate
