...
class NSFChart(pg.GraphicsLayoutWidget):
    ...
    def plot_data(self, y: Union[list, np.ndarray], x: Union[list, np.ndarray, None] = None, layer_index: int = 0, max_index:int=-1):
    ...
    def plot_channel(self, channel:sci_channel.SciChannel, layer_index: int = 0, max_index:int=-1):
        if layer_index < len( self.plot_layers):
            p = self.plot_layers[layer_index]
    ...
    def plot_stream(self, stream: sci_stream.SciStream, channel_index: int = 0, layer_index: int = 0, max_index:int=-1):
        if layer_index < len( self.plot_layers):
            p = self.plot_layers[layer_index]
