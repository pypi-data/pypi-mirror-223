...
export const Notebook = (props) => {
    ...
    useEffect(() => {
        if (uid && serviceManager && kernelManager && effectiveKernel) {
            const adapter = new NotebookAdapter({
                ...
                uid,
            }, injectableStore, serviceManager);
            setAdapter(adapter);
    ...
    return (_jsxs("div", { style: { height, width: '100%', position: "relative" }, id: "dla-Jupyter-Notebook", children: [props.Toolbar && _jsx(props.Toolbar, { notebookId: props.uid }), _jsxs(Box, { className: "box-notebook", sx: {
                ...
                }, children: [_jsx(_Fragment, { children: portals?.map((portal) => portal) }), _jsx(Box, { children: adapter &&
                            _jsx(LuminoNotebook, { adapter: adapter }) })] })] }));
};
