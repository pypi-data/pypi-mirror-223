...
fn subset_loop(idx_dir: &str, kmer_buf: &mut [u8], score_buf: &mut Vec<u8>,
               ...
               mut count: u64, metadata: &mut PKMeta, kmer_bufsize: usize,
               kmer_bitsize: usize,
...
fn subset_loop_tar(idx_dir: &str, tar_file: &str,
    ...
    for kmers_entry in Archive::new(tar_k).entries().expect("Can't read tar file") {
        ...
        let mut k_in_str = format!("{0:?}", &k_in_path);
        k_in_str.pop();
        if (&k_in_str).ends_with("kmers.bgz") {
            ...
            for scores_entry in Archive::new(tar_s).entries().expect("Can't read tar file") {
                ...
                let mut s_in_str = format!("{0:?}", &s_in_path);
                s_in_str.pop();
...
pub fn subset(idx_dir: &str, tar_file: &str, subset_genomes: PKGenomes,
    ...
    let count = 0;
    if tar_file.len() > 0 {
        subset_loop_tar(&idx_dir, &tar_file, &mut kmer_buf, &mut score_buf,
                        &mut kmers_out, &mut scores_out, count, &mut metadata,
        ...
        subset_loop(&idx_dir, &mut kmer_buf, &mut score_buf, &mut kmers_out,
                    &mut scores_out, count, &mut metadata,
                    KMER_BUFSIZE, KMER_BITSIZE, n_superset_genomes,
