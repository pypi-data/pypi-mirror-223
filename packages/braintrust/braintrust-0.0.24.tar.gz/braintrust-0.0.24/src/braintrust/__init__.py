...
def log(input=None, output=None, expected=None, scores=None, metadata=None, id=None, inputs=None):
    """
    ...
    return _state.current_experiment.log(
        input=input, output=output, expected=expected, scores=scores, metadata=metadata, id=id, inputs=inputs
    )
...
class Dataset(ModelWrapper):
    ...
    def __init__(self, project_name: str, name: str = None, description: str = None):
        args = _populate_args(
            {"project_name": project_name, "org_id": ORG_ID}, dataset_name=name, description=description
        )
