...
class Evaluation:
    ...
    """
    def __init__(self, alpha, beta, predict_proba_fn, instance_probability, base_counterfactuals, disagreement, data_instance, theta, parallel, gamma, target_class):
        self.alpha = alpha
        self.beta = beta
    ...
    def wachter_cf(self, candidate_instance, base_cf_scores):
        penalty = self.misclassification_penalty_wachter(candidate_instance)
        proximity_score = self.disagreement.calculate_proximity(self.data_instance, candidate_instance)

        return penalty + proximity_score
