...
def pack(args):
    ...
    try:
        packignore = questionary.select(
            ...
            choices=["replit-python", "none", ".packignore"]
        ).ask()
        ...
        if packignore == '.packignore':
            pk = parse_packignore(folder_path)
            print(json.dumps(pk, indent=2))
            _ok = input("Is this OK? (y/n) ")

            if _ok.lower() != 'y':
                print(termcolor.colored("operation canceled by user", "red"))
                exit(1)
        ...
        print(termcolor.colored("packnow ", "red") + "operation failed! (removed zip due to error)")
...
def parse_packignore(folder_path: str) -> dict[str, list[Optional[str]]]:
    if os.path.exists(folder_path + '.packignore'):
        with open(folder_path + '.packignore', 'r', encoding='utf-8') as file:
            content = file.read()
        ...
        for line in content.splitlines():
            if line.startswith('*/'):
                ...
                if line.endswith('/'):
                    # is folder
                    meta['any_pos_dirs'].append(line[2:][:-1])
                else:
                    # is file
                    meta['any_dir_files'].append(line[2:])

            elif line.startswith('/'):
                if line.endswith('/'):
                    # is folder: dirs
                    meta['dirs'].append(line[1:][:-1])
                else:
                    # is top dir file
                    meta['top_dir_files'].append(line[1:])

            elif line.startswith('#'):
                continue

            else:
                meta['full_dir_files'].append(line)
