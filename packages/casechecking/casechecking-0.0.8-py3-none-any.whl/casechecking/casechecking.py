...
class casechecking(object):
    ...
    def define_boundaries(self):
        ...
        define_boundaries() is called when a casechecking is initialized.
        define_boundaries() should be overridden in a child class to add
        boundary handlers.

        A casechecking without boundary handlers makes little sense.
        """
    ...
    def prepare_string(self, s) -> str:
        ...
        Executed during casechecking initialization providing an opportunity
        for child classes to manipulate the string. By default, the string
        is not manipulated.

        Can be overridden.

        :param s: The raw string supplied to the casechecking constructor.
        :type s: str
