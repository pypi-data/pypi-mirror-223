...
def send_clear_outpost_reminder(config, serviceConfigId):
    ...
    clientClearOutpostPublishAttrs = config.findConfigs(
        ["services.rabbitmq.producers.actions.{}".format(OUTPOST_ACTION_CLEAR.lower())]
    )[0]
    clientClearOutpostPublishAttrs["routing_key"] = clientClearOutpostPublishAttrs[
        "routing_key"
    ].format(service_id=serviceConfigId)

    remindCreateReminderPublishAttrs = config.findConfigs(
        ["services.rabbitmq.producers.actions.{}".format(REMIND_ACTION_CREATE.lower())]
    )[0]

    outpostData = {
        "msg_queue_info": remindCreateReminderPublishAttrs,
        "msg": {
            ...
            "data": {
                ...
                "to_msg_queue": clientClearOutpostPublishAttrs,
                "msg": {
                    "action": OUTPOST_ACTION_CLEAR,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        msmqHandler.declareQueues(serviceConfigId)
        msmqHandler.bindQueues(serviceConfigId)
        send_clear_outpost_reminder(msmqHandler.getConfig(), serviceConfigId)
        msmqHandler.consumeQueues(serviceConfigId, self.__callbackFn)
