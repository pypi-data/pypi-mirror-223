    ...
    while (1):
        ...
        for file in listOfInputs: 
            ...
            while (jobsAndDagsRunning >= batches*(parallelGenerations-1)) or filesInTemp >= parallelGenerations*batches:
                ...
                if heldJobs > 0 and releaseJobsTrials < 25 and releaseJobsTrials%5 !=0:
                    os.system("condor_release --all")
                    releaseJobsTrials += 1
                    logger.debug(f"Held Jobs: {heldJobs}, release trials: {releaseJobsTrials}")
                
                elif heldJobs > 0 and releaseJobsTrials <= 25 and releaseJobsTrials%5==0:

                    condor_memory = int(3*2048 + int(releaseJobsTrials/5)*2048)
                ...
                elif heldJobs > 0 and releaseJobsTrials > 25:
                    
                    logger.warning(f"Memory is currently set to : {condor_memory} but it is still not enough. Updated your search parameters to be less memory heavy!")
