...
class FigureStream(Figure):
    ...
    def __init__(self, host='localhost', port='5000', endpoint='', *args, **kwargs):
        ...
        self.size = 'auto'
        self.set_dpi(100)
        # self.tight_layout(rect=[0, 0, 1, 1])
        # self.tight_layout()
        # self.subplots_adjust(left=0, right=1, bottom=0, top=1)
    ...
    def _video_feed(self):
        """"""
        self.proccess_interact(request.values)
        ...
        if self.size == 'auto':
            ...
            if (
                ...
                self.set_size_inches(float(width) / self.get_dpi(),
                                     float(height) / self.get_dpi(),
                                     )
                # self.set_size_inches(float(width) / 100, float(height) / 100)
    ...
    # ----------------------------------------------------------------------
    def proccess_interact(self, values):
        """"""
        for key in values:
            if key in ['width', 'height', 'background']:
                continue
            if interact := getattr(self, f'interact_{key.lower().replace(" ", "_")}', False):
                interact(values.get(key))
