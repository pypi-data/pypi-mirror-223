...
def fast(image, threshold=20.0, arc_length=9, non_max=True, feature_ratio=0.05, edge=3):
    ...
    """
    out = Features()
    safe_call(backend.get().flare_fast(c_pointer(out.feat),
...
def harris(image, max_corners=500, min_response=1E5, sigma=1.0, block_size=0, k_thr=0.04):
    ...
    """
    out = Features()
    safe_call(backend.get().flare_harris(c_pointer(out.feat),
...
def orb(image, threshold=20.0, max_features=400, scale = 1.5, num_levels = 4, blur_image = False):
    ...
    feat = Features()
    desc = Array()
    safe_call(backend.get().flare_orb(c_pointer(feat.feat), c_pointer(desc.arr), image.arr,
...
def hamming_matcher(query, database, dim = 0, num_nearest = 1):
    ...
    index = Array()
    dist = Array()
    safe_call(backend.get().flare_hamming_matcher(c_pointer(index.arr), c_pointer(dist.arr),
...
def nearest_neighbour(query, database, dim = 0, num_nearest = 1, match_type=MATCH.SSD):
    ...
    index = Array()
    dist = Array()
    safe_call(backend.get().flare_nearest_neighbour(c_pointer(index.arr), c_pointer(dist.arr),
                                                 query.arr, database.arr,
                                                 c_dim_t(dim), c_dim_t(num_nearest),
                                                 match_type.value))
    return index, dist
...
def match_template(image, template, match_type = MATCH.SAD):
    ...
    """
    out = Array()
    safe_call(backend.get().flare_match_template(c_pointer(out.arr),
...
def susan(image, radius=3, diff_thr=32, geom_thr=10, feature_ratio=0.05, edge=3):
    ...
    """
    out = Features()
    safe_call(backend.get().flare_susan(c_pointer(out.feat),
...
def dog(image, radius1, radius2):
    ...
    out = Array()
    safe_call(backend.get().flare_dog(c_pointer(out.arr),
...
def sift(image, num_layers=3, contrast_threshold=0.04, edge_threshold=10.0, initial_sigma = 1.6,
    ...
    feat = Features()
    desc = Array()
    safe_call(backend.get().flare_sift(c_pointer(feat.feat), c_pointer(desc.arr),
...
def gloh(image, num_layers=3, contrast_threshold=0.04, edge_threshold=10.0, initial_sigma = 1.6,
    ...
    feat = Features()
    desc = Array()
    safe_call(backend.get().flare_gloh(c_pointer(feat.feat), c_pointer(desc.arr),
...
def homography(x_src, y_src, x_dst, y_dst, htype = HOMOGRAPHY.RANSAC,
    ...
    H = Array()
    inliers = c_int_t(0)
    safe_call(backend.get().flare_homography(c_pointer(H), c_pointer(inliers),
