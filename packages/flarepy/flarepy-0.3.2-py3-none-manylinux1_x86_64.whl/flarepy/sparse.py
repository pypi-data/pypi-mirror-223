...
def create_sparse(values, row_idx, col_idx, nrows, ncols, storage = STORAGE.CSR):
    ...
    storage : optional: flarepy.STORAGE. default: flarepy.STORAGE.CSR.
          - Can be one of flarepy.STORAGE.CSR, flarepy.STORAGE.COO.
    ...
    out = Array()
    safe_call(backend.get().flare_create_sparse_array(c_pointer(out.arr), c_dim_t(nrows), c_dim_t(ncols),
...
def create_sparse_from_dense(dense, storage = STORAGE.CSR):
    ...
    out = Array()
    safe_call(backend.get().flare_create_sparse_array_from_dense(c_pointer(out.arr), dense.arr, storage.value))
    return out
...
def convert_sparse_to_dense(sparse):
    ...
    """
    out = Array()
    safe_call(backend.get().flare_sparse_to_dense(c_pointer(out.arr), sparse.arr))
    return out
...
def sparse_get_info(sparse):
    ...
    values : flarepy.Array containing non zero elements from sparse matrix
    row_idx : flarepy.Array containing the row indices
    col_idx : flarepy.Array containing the column indices
    ...
    col_idx = Array()
    stype = c_int_t(0)
    safe_call(backend.get().flare_sparse_get_info(c_pointer(values.arr), c_pointer(row_idx.arr),
...
def sparse_get_values(sparse):
    ...
    """
    values = Array()
    safe_call(backend.get().flare_sparse_get_values(c_pointer(values.arr), sparse.arr))
    return values
...
def sparse_get_row_idx(sparse):
    ...
    """
    row_idx = Array()
    safe_call(backend.get().flare_sparse_get_row_idx(c_pointer(row_idx.arr), sparse.arr))
    return row_idx
...
def sparse_get_col_idx(sparse):
    ...
    """
    col_idx = Array()
    safe_call(backend.get().flare_sparse_get_col_idx(c_pointer(col_idx.arr), sparse.arr))
    return col_idx
...
def sparse_get_nnz(sparse):
    ...
    """
    nnz = c_dim_t(0)
    safe_call(backend.get().flare_sparse_get_nnz(c_pointer(nnz), sparse.arr))
    return nnz.value
...
def sparse_get_storage(sparse):
    ...
    """
    storage = c_int_t(0)
    safe_call(backend.get().flare_sparse_get_storage(c_pointer(storage), sparse.arr))
...
def convert_sparse(sparse, storage):
    ...
    """
    out = Array()
    safe_call(backend.get().flare_sparse_convert_to(c_pointer(out.arr), sparse.arr, storage.value))
    return out
