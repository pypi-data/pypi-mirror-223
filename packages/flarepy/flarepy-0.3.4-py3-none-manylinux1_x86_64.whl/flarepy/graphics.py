...
class Window(object):
    ...
    def __init__(self, width=1280, height=720, title="flare"):
        ...
        safe_call(backend.get().flare_create_window(c_pointer(self._wnd),
    ...
    def __del__(self):
        ...
        """
        safe_call(backend.get().flare_destroy_window(self._wnd))
    ...
    def set_pos(self, x, y):
        ...
        """
        safe_call(backend.get().flare_set_position(self._wnd, c_int_t(x), c_int_t(y)))
    ...
    def set_title(self, title):
        ...
        """
        safe_call(backend.get().flare_set_title(self._wnd, title))
    ...
    def set_size(self, w, h):
        ...
        h  : int
           Height of window.
        """
        safe_call(backend.get().flare_set_size(self._wnd, w, h))
    ...
    def image(self, img, title=None):
        ...
        safe_call(backend.get().flare_draw_image(self._wnd, img.arr, c_pointer(_cell)))
    ...
    def scatter(self, X, Y, Z=None, points=None, marker=MARKER.POINT, title=None):
        ...
        if points is None:
            if Z is None:
                safe_call(backend.get().flare_draw_scatter_2d(self._wnd, X.arr, Y.arr,
                                                           marker.value, c_pointer(_cell)))
            else:
                safe_call(backend.get().flare_draw_scatter_3d(self._wnd, X.arr, Y.arr, Z.arr,
                                                           marker.value, c_pointer(_cell)))
        else:
            safe_call(backend.get().flare_draw_scatter_nd(self._wnd, points.arr, marker.value, c_pointer(_cell)))
    ...
    def scatter2(self, points, marker=MARKER.POINT, title=None):
        ...
        _cell = _Cell(self._r, self._c, title, self._cmap)
        safe_call(backend.get().flare_draw_scatter2(self._wnd, points.arr,
                                                 marker.value, c_pointer(_cell)))
    ...
    def scatter3(self, points, marker=MARKER.POINT, title=None):
        ...
        _cell = _Cell(self._r, self._c, title, self._cmap)
        safe_call(backend.get().flare_draw_scatter3(self._wnd, points.arr,
                                                 marker.value, c_pointer(_cell)))
    def plot(self, X, Y, Z=None, line = None, title=None):
        ...
        if line is None:
            if Z is None:
                safe_call(backend.get().flare_draw_plot_2d(self._wnd, X.arr, Y.arr, c_pointer(_cell)))
            else:
                safe_call(backend.get().flare_draw_plot_3d(self._wnd, X.arr, Y.arr, Z.arr, c_pointer(_cell)))
        else:
            safe_call(backend.get().flare_draw_plot_nd(self._wnd, line.arr, c_pointer(_cell)))
    ...
    def plot3(self, X=None, Y=None, Z=None, line=None, title=None):
        ...
        ptr = join(1, X, Y, Z)
        assert(ptr.numdims() == 2)
        _cell = _Cell(self._r, self._c, title, self._cmap)
        safe_call(backend.get().flare_draw_plot_nd(self._wnd, ptr.arr, c_pointer(_cell)))
        return
    ...
    def vector_field(self, xpoints, xdirs, ypoints, ydirs, zpoints=None, zdirs=None,
        ...
        if line is None:
            if Z is None:
                safe_call(backend.get().flare_draw_vector_field_2d(self._wnd,
            ...
            safe_call(backend.get().flare_draw_plot_nd(self._wnd, points.arr, dirs.arr, c_pointer(_cell)))
    ...
    def surface(self, x_vals, y_vals, z_vals, title=None):
        ...
        """
        _cell = _Cell(self._r, self._c, title, self._cmap)
        safe_call(backend.get().flare_draw_surface(self._wnd,
    ...
    def hist(self, X, min_val, max_val, title=None):
        ...
        """
        _cell = _Cell(self._r, self._c, title, self._cmap)
        safe_call(backend.get().flare_draw_hist(self._wnd, X.arr,
    ...
    def grid(self, rows, cols):
        ...
        """
        safe_call(backend.get().flare_grid(self._wnd, c_int_t(rows), c_int_t(cols)))
    ...
    def show(self):
        ...
        """
        safe_call(backend.get().flare_show(self._wnd))
    ...
    def close(self):
        ...
        """
        tmp = c_bool_t(True)
        safe_call(backend.get().flare_is_window_closed(c_pointer(tmp), self._wnd))
        return tmp
    ...
    def set_visibility(is_visible):
        ...
        """
        safe_call(backend.get().flare_set_visibility(self._wnd, is_visible))
    ...
    def set_axes_limits(self, xmin, xmax, ymin, ymax, zmin=None, zmax=None, exact=False):
        ...
        if (zmin is None or zmax is None):
            safe_call(backend.get().flare_set_axes_limits_2d(self._wnd,
    ...
    def set_axes_label_format(self, xformat="4.1%f", yformat="4.1%f", zformat="4.1%f"):
        ...
        safe_call(backend.get().flare_set_axes_label_format(self._wnd,
