...
class Random_Engine(object):
    ...
    def __init__(self, engine_type = RANDOM_ENGINE.PHILOX, seed = 0, engine = None):
        if (engine is None):
            ...
            safe_call(backend.get().flare_create_random_engine(c_pointer(self.engine), engine_type.value, c_longlong_t(seed)))
        else:
            self.engine = engine

    def __del__(self):
        safe_call(backend.get().flare_release_random_engine(self.engine))

    def set_type(self, engine_type):
        """
        Set the type of the random engine.
        """
        safe_call(backend.get().flare_random_engine_set_type(c_pointer(self.engine), engine_type.value))
    ...
    def get_type(self):
        ...
        safe_call(backend.get().flare_random_engine_get_type(c_pointer(rty), self.engine))
    ...
    def set_seed(self, seed):
        ...
        """
        safe_call(backend.get().flare_random_engine_set_seed(c_pointer(self.engine), c_longlong_t(seed)))
    ...
    def get_seed(self):
        ...
        """
        seed = c_longlong_t(0)
        safe_call(backend.get().flare_random_engine_get_seed(c_pointer(seed), self.engine))
        return seed.value
...
def randu(d0, d1=None, d2=None, d3=None, dtype=Dtype.f32, engine=None):
    ...
    engine : optional: Random_Engine. default: None.
             If engine is None, uses a default engine created by flarepy.
    ...
    if engine is None:
        safe_call(backend.get().flare_randu(c_pointer(out.arr), 4, c_pointer(dims), dtype.value))
    else:
        safe_call(backend.get().flare_random_uniform(c_pointer(out.arr), 4, c_pointer(dims), dtype.value, engine.engine))
...
def randn(d0, d1=None, d2=None, d3=None, dtype=Dtype.f32, engine=None):
    ...
    if engine is None:
        safe_call(backend.get().flare_randn(c_pointer(out.arr), 4, c_pointer(dims), dtype.value))
    else:
        safe_call(backend.get().flare_random_normal(c_pointer(out.arr), 4, c_pointer(dims), dtype.value, engine.engine))
...
def set_seed(seed=0):
    ...
    """
    safe_call(backend.get().flare_set_seed(c_ulonglong_t(seed)))
...
def get_seed():
    ...
    safe_call(backend.get().flare_get_seed(c_pointer(seed)))
    return seed.value
...
def set_default_random_engine_type(engine_type):
    ...
    """
    safe_call(backend.get().flare_set_default_random_engine_type(c_pointer(self.engine), engine_type.value))
...
def get_default_random_engine():
    ...
    safe_call(backend.get().flare_get_default_random_engine(c_pointer(default_engine)))
    safe_call(backend.get().flare_retain_random_engine(c_pointer(engine), default_engine))
