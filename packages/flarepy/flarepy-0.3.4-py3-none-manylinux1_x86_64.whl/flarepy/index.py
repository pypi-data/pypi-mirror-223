...
class Seq(ct.Structure):
    ...
    def __init__ (self, S):
        ...
        if _is_number(S):
            ...
            raise IndexError("Invalid type while indexing flarepy.array")
...
class Index(ct.Structure):
    ...
    """
    Container for the index class in flare C library
    ...
    def __init__ (self, idx):
        ...
        if isinstance(idx, BaseArray):
            ...
            if (idx.type() == Dtype.b8.value):
                safe_call(backend.get().flare_where(c_pointer(arr), idx.arr))
            else:
                safe_call(backend.get().flare_retain_array(c_pointer(arr), idx.arr))
    ...
    def __del__(self):
        if not self.isSeq:
            ...
            backend.get().flare_release_array(arr)
