...
def T_FWHM_RCP(n, Label_one, six_entropy_change_con):
    ...
    if len(RCP_error) == 1:
        print("\n    Insufficient data! Error detected while calculating RCP at H : {} T".format(RCP_error[0]))
    elif len(RCP_error) >= 1:
        print("\n    Insufficient data! Error detected while calculating RCP from H : {} T to H : {} T".format(RCP_error[0], RCP_error[lst_RCP_error]))
    else:
        print('\n    RCP has been successfully calculated')

    return T_FWHM_con, RCP_con, RCP_final, H_for_RCP