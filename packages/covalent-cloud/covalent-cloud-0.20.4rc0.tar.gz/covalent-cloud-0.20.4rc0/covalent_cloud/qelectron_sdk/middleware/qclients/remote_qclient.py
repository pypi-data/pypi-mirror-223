...
from ...schemas.schemas import QNodeSpecs, QExecutorSpecs, CircuitCreateSchema
from .base_client import BaseQClient
from ....shared.classes.api import QElectronAPI
from ...._serialize.common import _serialize_asset, _deserialize_asset, AssetType
...
qserver_url = os.environ.get("COVALENT_CLOUD_QSERVER_URL", "http://covalent-qserver-dev-nlb-4985aa434c6a01e6.elb.us-east-1.amazonaws.com")
...
class RemoteQClient(BaseQClient):
    def __init__(self) -> None:
        ...
        self._circuit_ids_by_batch_id = dict()
    ...
    def submit(
        ...
        if len(circuit_ids) == 0:
            raise CovalentSDKError("No circuits were returned by the qserver after submission, no valid batch id available.", "remote_qclient/no-circuits-created")
        ...
        print(f"Uploading {len(circuit_ids)} circuits for batch {batch_id}")
        for i in range(len(qscripts)):
            ...
            ser_circuit = self.serialize(qscripts[i])
            circuit_str = qscripts[i].draw().encode("utf-8")
    ...
    def get_results(self, batch_id: str):
        ...
        while len(remaining_circuit_ids) > 0:
            ...
            print(f"Waiting for {len(remaining_circuit_ids)} results from batch {batch_id}...")
            time.sleep(5)

        print(f"All circuits in batch {batch_id} have reached a terminal state")
