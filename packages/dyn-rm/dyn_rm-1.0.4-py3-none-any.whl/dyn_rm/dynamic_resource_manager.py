...
class ResourceManager:
    ...
    def stop_process_manager(self, name):
        ...
        if name == "PRRTE":
            #self.my_pmix.publish_data({'key': "DYN_RM_FINALIZE", 'value': "finalize", 'flags': 0, 'val_type': PMIX_STRING}, pset_name = self.dummy_pset_name)
            # give them a chance to terminate
            #sleep(1)
    ...
    def init_system_from_current_state(self, system_name, scheduling_policy):
        ...
        for jobid in jobids:
            ...
            for node, proc_ranks in zip(nodes, ppn):
                ...
                slots = self.my_pmix.get_node_slots(jobid, node)
                my_system.node_set_slots(node, slots)
    ...
    def _periodic_sched(self, interval, cur_it, max_it):
        ...
        cur_it = cur_it + 1
        if (max_it == None or cur_it <= max_it) and (len(self.my_system.jobs) > 0 or len(self.job_mix) > 0):
        ...
        for cmd in cmds:
            ...
            if rc == PMIX_SUCCESS:
                ...
                for node, proc_ranks in zip(nodes, ppn):
                    ...
                    slots = self.my_pmix.get_node_slots(jobid, node)
                    if -1 < slots: 
                        self.my_system.node_set_slots(node, slots)
