...
def checkcreate(directory):
    if not os.path.exists(directory): os.makedirs(directory), print(f'created directory: {directory}')

def saveornot(fig, name, form='svg', savefigs=False): # Save figure, or not
    if savefigs: fig.savefig(name+'.'+form)
