...
async def get_article(
    ...
    with client.context():
        article: Article | None = ndb.Key(
            Article, sha3_256_hash(f"{article_id}&{cursor}")
        ).get()
...
async def get_feed(
    ...
    with client.context():
        feed: Feed | None = ndb.Key(Feed, sha3_256_hash(f"{category}&{cursor}")).get()
    ...
    if not feed:
        ...
        if not feed:
            return FeedResponse(feed=[], cursor=cursor, next_cursor=None)
