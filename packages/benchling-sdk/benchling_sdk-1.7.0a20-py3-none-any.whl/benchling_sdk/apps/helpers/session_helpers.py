...
from typing import Generic, Iterable, List, Optional, Protocol, Type, Union
...
class AppUserFacingError(Exception):
    ...
    def messages(self) -> List[AppSessionMessageCreate]:
        ...
        return [AppSessionMessageCreate(content=str(self), style=AppSessionMessageStyle.ERROR)]
...
class SessionContextManager(AbstractContextManager, Generic[AppType]):
    ...
    def attach_canvas(self, canvas_id: str) -> None:
        ...
        canvas = self.app.benchling.apps.get_canvas_by_id(canvas_id)
    ...
    def detach_canvas(self) -> None:
        ...
        if self._attached_canvas_id:
            # TODO BNCH-57234 we can't update canvas without setting blocks and featureId
            canvas = self.app.benchling.apps.get_canvas_by_id(self._attached_canvas_id)
            canvas_update = AppCanvasUpdate(
                session_id=None, blocks=canvas.blocks, feature_id=canvas.feature_id
            )
