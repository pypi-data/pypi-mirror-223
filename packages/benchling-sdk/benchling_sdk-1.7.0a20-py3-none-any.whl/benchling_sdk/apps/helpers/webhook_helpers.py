...
def _production_webhook_jwks(app_definition_id) -> str:
    """Return the URL for retrieving production JWKs for an app."""
    return f"https://apps.benchling.com/v1/apps/{app_definition_id}/jwks"
...
def jwks_by_app(
    ...
    jwk_url = jwk_url_provider(app_definition_id)
...
def verify(app_definition_id: str, data: str, headers: dict, jwk_function: GetJwksFunction = None) -> None:
    ...
    if jwk_function is None:
        ...
        def _get_default_jwks(_app_definition_id):
            return jwks_by_app(_app_definition_id)

        jwk_function = _get_default_jwks
    jwks = jwk_function(app_definition_id)
