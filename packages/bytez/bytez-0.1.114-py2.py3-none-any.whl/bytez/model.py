...
def post(url, **kwargs):
    ...
    raise Exception(
        'API key invalid. Please vist https://discord.gg/drdZWv3szS to request an api key.')
...
class Model:
    ...
    def __inference(self, url: str, request_params: dict, api_key) -> bytes:
        ...
        if not api_key:
            raise Exception(
                'kwarg "api_key" not supplied. Remember to include api_key="your_api_key" when calling a model. To request an API key, please visit https://discord.gg/drdZWv3szS')
