...
class CurrentMasterData(CdkResource):
    ...
    def properties(
        ...
        if self.inventory_models is not None:
            settings["inventoryModels"] = [
                v.properties(
                )
                for v in self.inventory_models
            ]
        if self.simple_inventory_models is not None:
            settings["simpleInventoryModels"] = [
                v.properties(
                )
