...
class DuckDbTreeRewriter(rewrite.QueryTreeRewriter):
    ...
    def rewrite(self, tree: NodeRef, prev_query_tree_stage: QueryTreeComputeType) -> None:
        for i in tree.inputs:
            self.rewrite(tree=i, prev_query_tree_stage=prev_query_tree_stage)
        tree_node = tree.node
        if isinstance(tree_node, nodes.StagingNode):
            if prev_query_tree_stage != tree_node.query_tree_stage:
                return
