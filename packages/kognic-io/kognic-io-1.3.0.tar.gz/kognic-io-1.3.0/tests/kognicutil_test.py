...
class TestKognicUtils:
    ...
    def test_kognicutil_inputs(self, client: IOC.KognicIOClient):
        ...
        result = runner.invoke(
            kognicutil.inputs, ['--project', 'lidars_and_cameras-project', '--external-ids', 'input1', '--include-invalidated'],
            obj={'client': client}
        )
        assert result.exit_code == 0, result.output
    ...
    def test_kognicutil_inputs_view(self, client: IOC.KognicIOClient):
        runner = CliRunner()
        result = runner.invoke(
            kognicutil.inputs, ['--project', 'lidars_and_cameras-project', '--external-ids', 'input1', '--include-invalidated', '--view'],
            obj={'client': client}
        )
        assert result.exit_code == 0, result.output
