...
class TestLidarsAndCamerasSeqWithSparsePreAnnotations:
    ...
    def run_test_with_sparse_pre_annotation(self, client: KognicIOClient, pre_annotation: OLM.OpenLabelAnnotation):
        ...
        wait_for_scene_job(client=client, scene_uuid=scene_response.input_uuid, timeout=60)

        client.pre_annotation.create(scene_uuid=scene_response.input_uuid, pre_annotation=pre_annotation, dryrun=False)
        create_input_resp = client.lidars_and_cameras_sequence.create_from_scene(
            scene_uuid=scene_response.input_uuid, annotation_types=None, project=project, dryrun=False
        )
        assert create_input_resp is None
...
def build_sparse_pre_annotation_with_interpolated() -> OLM.OpenLabelAnnotation:
    ...
    return OLM.OpenLabelAnnotation(
        openlabel=OLM.Openlabel(
            ...
            objects={object_uuid: OLM.Object(name="MyNameIsBox",
                                             type="SpaceShip", 
                                             object_data=OLM.ObjectData(text=[OLM.Text(name="faction",
                                                                                       val="empire")]))},
            frames={
...
def build_sparse_pre_annotation_with_od_pointers() -> OLM.OpenLabelAnnotation:
    ...
    root_object = OLM.Object(name="MyNameIsBox",
                             type="SpaceShip",
...
def build_frame_object_interpolated(offset: int, interpolated: bool, occlusion: str) -> OLM.Objects:
    return OLM.Objects(
        object_data=OLM.ObjectData(
            cuboid=[build_cuboid(offset, [OLM.Boolean(name="interpolated", val=interpolated)])],
