...
class SerialAgg(AggEngine):
    ...
    def calc_agg(
        ...
        gdf = gdf.sort_values(data.id_feature).dissolve(
            by=data.id_feature, as_index=False
        )
        geo_index = np.asarray(
            gdf[data.id_feature].values, dtype=type(gdf[data.id_feature].values[0])
        )
        n_geo = len(geo_index)
...
class ParallelAgg(AggEngine):
    ...
    def calc_agg(
        ...
        gdf = data.feature
        # gdf.reset_index(drop=True, inplace=True)
        gdf = gdf.sort_values(self.id_feature, axis=0).dissolve(
            self.id_feature, as_index=False
        )
        ...
        for index, key in enumerate(wvars):
            ...
            with parallel_backend("loky", inner_max_num_threads=1):
                worker_out = Parallel(n_jobs=self._jobs)(
                    delayed(_grid_to_line_intersection)(chunk, data_array, variables)
                    for chunk in gdf_list
                )

            key_stats: pd.DataFrame = pd.concat(list(zip(*worker_out))[0])  # noqa B905
            key_points: gpd.GeoDataFrame = pd.concat(
                list(zip(*worker_out))[1]  # noqa B905
            )
            del worker_out
...
class DaskInterp(InterpEngine):
    ...
    def interp(  # noqa: C901
        ...
        for index, key in enumerate(wvars):
            ...
            bag = db.from_sequence(
                self._user_data.f_feature.reset_index().index.to_list()
            ).map(self.g2l)
            results = bag.compute()
            del bag
    ...
    def g2l(self, id):
        ...
        if variables["spacing"] == 0:
            x, y, dist = _get_line_vertices(
                geom=geom, calc_crs=variables["calc_crs"], crs=variables["grid_proj"]
            )
        ...
        interp_dataarray: xr.DataArray = data_array.to_dataset(
        ...
        interp_geo_df = _dataframe_to_geodataframe(
            interp_dataarray.to_dataframe(), crs=variables["grid_proj"]
        )
        ...
        # prefer date, feature id and varname, up front of dataframe.
        if not variables["t_coord"] is None:
            out_vals: dict[str, float] = {
                "date": interp_dataarray[variables["t_coord"]].values
            }
            ...
            out_vals: dict[str, float] = {
                variables["id_feature"]: line[variables["id_feature"]].values
            }
