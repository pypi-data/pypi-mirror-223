...
from fds.model.bucket_info import BucketBean
import os
import sys
...
class GalaxyFDSClient(object):
  ...
  def __init__(self, access_key=None, access_secret=None, config=None):
    ...
    if config == None:
      ...
      config.set_cdn_endpoint(self.load_cdn_endpoint())
  ...
  def load_cdn_endpoint(self):
    endpoint = None
    if endpoint is None and "XIAOMI_CDN_ENDPOINT" in os.environ:
      endpoint = os.environ["XIAOMI_CDN_ENDPOINT"]
    if endpoint is None:
      endpoint = self.load_config("xiaomi_cdn_endpoint")
    if endpoint is not None and len(endpoint.strip()) == 0:
      logging.warn(
        "endpoint is set to empty, please check ${XIAOMI_FDS_ENDPOINT} in environ variables, or \"xiaomi_cdn_endpoint\" in ~/.config/xiaomi/config")
  ...
  def does_bucket_exist(self, bucket_name):
    ...
    '''
    uri = self._formatUri(self._config.get_base_uri(), bucket_name, None)
  ...
  def get_bucket(self, bucket_name):
    '''
    Get the bucket info of a specified bucket.
    :param bucket_name: the bucket name of the specified bucket
    :return: The bucket info
    '''
    uri = self._formatUri(self._config.get_base_uri(), bucket_name, None)
    response = self._request.get(uri, auth=self._auth)
    if response.status_code == requests.codes.ok:
      bucket_bean = BucketBean(to_json_object(response.content))
      return bucket_bean
    else:
      headers = ""
      if self._config.debug:
        headers = ' header=%s' % response.headers
      message = 'Get bucket info failed, status=%s, reason=%s%s' % (
        response.status_code, response.content, headers)
      raise GalaxyFDSClientException(message)
  ...
  def list_buckets(self):
    ...
    '''
    uri = self._formatUri(self._config.get_base_uri(), None, None)
    response = self._request.get(uri, auth=self._auth)
    if response.status_code != requests.codes.ok:
      headers = ""
      if self._config.debug:
  ...
  def put_object(self, bucket_name, object_name, data, metadata=None):
    ...
    if filen < threshold_size:
      ...
      metadata.add_header(Common.MULITPART_UPLOAD_MODE, "DIRECT")
      upload_token = self.init_multipart_upload(bucket_name, object_name, metadata)
      metadata.remove_header(Common.MULITPART_UPLOAD_MODE)
      upload_list = []
      part_number = 1
      while (content):
  ...
  def put_object_directly(self, bucket_name, object_name, data, metadata=None):
    ...
    :return: The result of putting action server returns
    '''
    uri = self._formatUri(self._config.get_upload_base_uri(), bucket_name, object_name)
    if metadata is None:
      metadata = FDSObjectMetadata()
    inputstream = None
    if isinstance(data, bytes):
  ...
  def post_object(self, bucket_name, data, metadata=None):
    ...
    '''
    uri = self._formatUri(self._config.get_upload_base_uri(), bucket_name, None)
    if metadata is None:
      metadata = FDSObjectMetadata()
    if self._config.enable_md5_calculate:
      digest = hashlib.md5()
  ...
  def get_object(self, bucket_name, object_name, position=0, size=4096, stream=None,
    ...
    uri = self._formatUri(self._config.get_download_base_uri(), bucket_name, object_name)
    req_params = dict()
    if version_id:
    ...
    if response.status_code == requests.codes.ok or \
      ...
      if FDSObjectMetadata.USER_DEFINED_METADATA_PREFIX + 'content-length' in response.headers:
        summary.size = int(response.headers[FDSObjectMetadata.USER_DEFINED_METADATA_PREFIX + 'content-length'])
      else:
  ...
  def does_object_exists(self, bucket_name, object_name):
    ...
    '''
    uri = self._formatUri(self._config.get_base_uri(), bucket_name, object_name)
    response = self._request.head(uri, auth=self._auth)
    if response.status_code == requests.codes.ok:
      return True
    elif response.status_code == requests.codes.not_found:
  ...
  def rename_object(self, bucket_name, src_object_name, dst_object_name):
    ...
    '''
    uri = self._formatUri(self._config.get_base_uri(), bucket_name, src_object_name)
  ...
  def prefetch_object(self, bucket_name, object_name):
    ...
    :return: void
    '''
    return self.prefetch_uri(bucket_name, object_name, "")

  def prefetch_uri(self, bucket_name, object_name, cdn_uri):
    """prefetch a uri

    Args:
      bucket_name (str): bucket name
      object_name (str): bucketname
      uri (str): uri string
    """
    uri = self._formatUri(self._config.get_base_uri(), bucket_name, object_name)
    response = self._request.put(uri, auth=self._auth, data=cdn_uri, params={"prefetch": 'true'})
    if response.status_code != requests.codes.ok:
      headers = ""
      if self._config.debug:
        headers = ' header=%s' % response.headers
  ...
  def refresh_object(self, bucket_name, object_name):
    ...
    :return: void
    '''
    return self.refresh_uri(bucket_name, object_name, "")

  def refresh_uri(self, bucket_name, object_name, cdn_uri):
    """refresh uri
    ...
    """
    uri = self._formatUri(self._config.get_base_uri(), bucket_name, object_name)
    response = self._request.put(uri, auth=self._auth, data=cdn_uri, params={"refresh": 'true'})
    if response.status_code != requests.codes.ok:
      headers = ""
      if self._config.debug:
        headers = ' header=%s' % response.headers
  ...
  def init_multipart_upload_copy(self, bucket_name, object_name, metadata=None):
    '''
    Init a multipart upload copy session
    :param bucket_name:
    :param object_name:
    :param metadata:
    :return:
    ...
    if not metadata:
      metadata = FDSObjectMetadata()
    metadata.add_header(Common.MULITPART_UPLOAD_MODE, "MULTI_BLOB")
    return self.init_multipart_upload(bucket_name, object_name, metadata)
  ...
  def upload_part_copy(self, bucket_name, object_name, upload_id, part_number, source_bucket_name,
                       source_object_name, begin_range=None, end_range=None):
    '''
    ...
    metadata = FDSObjectMetadata()
    metadata.add_header(Common.COPY_SOURCE,
                        '/%s/%s' % (quote(source_bucket_name), quote(source_object_name)))
    if begin_range is not None and end_range is not None:
      metadata.add_header(Common.COPY_SOURCE_RANGE, "bytes=%d-%d" % (begin_range, end_range))
  ...
  def generate_presigned_uri(self, base_uri, bucket_name, object_name,
    ...
    try:
      uri = self._formatUri(base_uri, bucket_name, object_name)
  ...
  def generate_download_object_uri(self, bucket_name, object_name):
    ...
    '''
    return self._formatUri(self._config.get_download_base_uri(), bucket_name, object_name)
  ...
  def copy_object(self, src_bucket_name, src_object_name, dst_bucket_name, dst_object_name):
    ...
    :return:
    '''
    uri = self._formatUri(self._config.get_upload_base_uri(), dst_bucket_name, dst_object_name)
  ...
  def _generate_anti_stealing_uri(self, base_uri, bucket_name, object_name, expiration, key):
    ...
    uri = self._formatUri(base_uri, bucket_name, object_name)
    return "%s?sign=%s&t=%s" % (uri, sign, t)
  ...
  def _restore_archived_object_(self, src_bucket_name, src_object_name):
    ...
    :param object_name:     The name of the object
    '''
    uri = self._formatUri(self._config.get_base_uri(), src_bucket_name, src_object_name)
    response = self._request.put(uri, auth=self._auth,
  ...
  def _formatUri(self, base_uri, bucket_name, object_name):
    u = urlparse.urlparse(base_uri)
    host = u.hostname
    if u.hostname == self._config.get_endpoint() and self._config.shouldUseVirtualAddressing() and bucket_name and bucket_name != "trash":
      host = "%s.%s" % (bucket_name, u.hostname)
    if not bucket_name:
      return "%s://%s/" % (u.scheme, host)
    elif not object_name:
      return "%s://%s/%s" % (u.scheme, host, quote(bucket_name))
    else:
      return "%s://%s/%s/%s" % (u.scheme, host, quote(bucket_name), quote(object_name))
