...
class YandexHome:
    ...
    async def get_devices(self, cached_user: dict = None):
        ...
        for device_ud in user_data['devices']:
            ...
            device = Device(device['id'], device['name'], device['aliases'], device['room'], device['external_id'], device['skill_id'], device['type'], groups, device['capabilities'], device['properties'], device_household, self.token)
