...
class YandexHome:
    ...
    async def get_devices(self):
        ...
        for device in user_data['devices']:
            ...
            for group in device['groups']:
                async with ClientSession() as session:
                    async with session.get(f'https://api.iot.yandex.net/v1.0/groups/{group}', headers=self.headers) as req:
                        group = await req.json()
                        devices = []
                        for device in group['devices']:
                            async with session.get(f'https://api.iot.yandex.net/v1.0/devices/{device["id"]}', headers=self.headers) as dev_req:
                                device = await dev_req.json()
                                devices.append(Device(device['id'], device['name'], device['aliases'], device['room'], device['external_id'], device['skill_id'], device['type'], groups, device['capabilities'], device['properties'], '0', self.token))
                        group = Group(group['id'], group['name'], group['aliases'], group['type'], group['state'], devices, self.token)
                        groups.append(group)
            device = Device(device['id'], device['name'], device['aliases'], device['room'], device['external_id'], device['skill_id'], device['type'], groups, device['capabilities'], device['properties'], device['household_id'], self.token)
    ...
    async def get_groups(self):
        ...
        for group in user_data['groups']:
            async with ClientSession() as session:
                async with session.get(f'https://api.iot.yandex.net/v1.0/groups/{group["id"]}', headers=self.headers) as req:
                    group = await req.json()
                    devices = []
                    for device in group['devices']:
                        devices.append(await self.get_device(device['id']))
                    group = Group(group['id'], group['name'], group['aliases'], group['type'], group['state'], devices, self.token)
                    groups.append(group)
        return groups
