...
from bgeditor.common.utils import get_dir, always_even
...
class VideoComp(Component):
    ...
    def get_clip(self):
        ...
        if self.opacity < 1.0:
            rs = rs.set_opacity(self.opacity)
        w, h = rs.size
        if self.position['width'] != w or self.position['height'] != h or 'zoom_in' in self.position:
            if('zoom_in' in self.position):
                zoom_in=self.position['zoom_in']
                ww = self.position['width'] + int(self.position['width'] * zoom_in/100)
                hh = self.position['height'] + int(self.position['height'] * zoom_in/100)
                ww= always_even(ww)
                hh=always_even(hh)
                space_x = (ww- self.position['width'])
                space_y = (hh- self.position['height'])
                zoom_in_pos = 'center'
