       ...
       @:param data_mappings 数据配置，结构如下：
            data_mappings = [
                ...
                },
                {
                    ...
                    # 主表匹配字段(匹配规则)
                    "unique_fields": [['transact_time', 'summary']],
                    # 作为唯一标识(外键)的字段(匹配规则)
                    "unique_foreign_fields": [{
                        ...
                        "account_id": {
                            "related_table": "user_base_info",
                            "unique_field_excel": "客户名称",
                            "unique_field_db": "user_name"
                        }}
                    ]
                }    
            ]
...
def execl_import(database_config, execl_file, date_fields, data_mappings, import_logs="./", import_category=False):
    # 数据库连接信息
            ...
            for mapping in data_mappings:
                ...
                try:
                    with engine.begin() as connection:
                        ...
                        where = {}
                        # 检查 unique_fields
                        for unique_fields in mapping.get('unique_fields', []):
                            if all(field in record for field in unique_fields):
                                query = f"SELECT id FROM {table.name} WHERE " + " AND ".join(
                                    f"{field} = :{field}" for field in unique_fields)
                                result = connection.execute(
                                    text(query),
                                    {field: record[field] for field in unique_fields}
                                )
                                ...
                                if result_row is None:
                                    ...
                                    where[mapping.get('main_foreign_keys')] = result_row[0]

                        # 检查 unique_foregard_fields
                        for unique_fk_dict in mapping.get('unique_foreign_fields', []):
                            for unique_fk, fk_conf in unique_fk_dict.items():
                                related_table = Table(fk_conf['related_table'], metadata, autoload_with=engine)
                                unique_field_excel = fk_conf['unique_field_excel']
                                unique_field_db = fk_conf['unique_field_db']
                                result = connection.execute(
                                ...
                                if result_row is None:
                                    ...
                                    where[unique_fk] = result_row[0]

                        if where:
                            sql = text(
                                "SELECT id FROM " + str(table) + " WHERE " + " AND ".join(
                                    f"{k} = :{k}" for k in where.keys()))
                            sql = sql.bindparams(**where)
                            result = connection.execute(sql)
                            result_row = result.fetchone()
                            if result_row is not None:
                                foreign_keys[mapping.get('extend_foreign_keys')] = result_row[0]
                            else:
                                record_exists = False
                        else:
                            record_exists = False
                        ...
                        if record_exists:
                            conditions = [table.c[key] == value for key, value in where.items()]
                            stmt = (
                                update(table)
                        ...
                        if mapping_table is not None:
                            ...
                            record_extend_exists = result.fetchone() is not None
                            if record_exists and record_extend_exists:
                                # 记录存在，执行更新操作
                                stmt = (
                                ...
                                data_to_insert[mapping.get('extend_foreign_keys', "")] = foreign_keys.get(
    ...
    if failures:
        ...
        filepath = f"error_log_{timestamp}.xlsx"  # 在文件名中添加时间戳
