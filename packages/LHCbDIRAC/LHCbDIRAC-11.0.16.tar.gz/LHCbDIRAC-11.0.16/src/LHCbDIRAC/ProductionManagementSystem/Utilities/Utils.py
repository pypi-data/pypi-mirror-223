        ...
        for group in groups:
            conn.execute(
             ...
             SimCondition, ProPath, thegroup, reqInform)"
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (reqId, reqType, reqWG, reqName, SimCondition, ProPath, group, informPeoples),
            )
...
def informPeople(rec, oldstate, state, author, inform):
    ...
    footer += f"DIRAC Web portal: https://lhcb-portal-dirac.cern.ch/DIRAC/s:{PathFinder.getDIRACSetup()}/g:"
    ...
    if authorMail:
        if state not in ["BK Check", "Submitted"]:
            if state == "BK OK":
                subj = f"DIRAC: please resign your Production Request {reqId}"
                body = "\n".join(
                    [
