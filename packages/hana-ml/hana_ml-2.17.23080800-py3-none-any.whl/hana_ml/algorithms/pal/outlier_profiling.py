...
from hana_ml.algorithms.pal.clustering import KMeansOutlier, DBSCAN
from hana_ml.algorithms.pal.svm import OneClassSVM
from hana_ml.algorithms.pal.stats import grubbs_test, iqr
...
class OutlierProfiling(PALBase):
    ...
    label_column = dict(DBSCAN='CLUSTER_ID',
                        OneClassSVM='SCORE',
                        IsolationForest='LABEL')
    default_profiling = dict(KMeansOutlier=dict(contamination=0.1),
                             DBSCAN=dict(minpts=None, eps=None),
                             OneClassSVM=dict(kernel='rbf', nu=0.1),
                             IsolationForest=dict(contamination=0.1),
                             grubbs_test=dict(method='two_sides'),
                             iqr=dict(multiplier=1.5))
    light_profiling = dict(KMeansOutlier=dict(contaimination=0.1),
                           DBSCAN=dict(minpts=None, eps=None),
                           IsolationForest=dict(contamination=0.1))
    def __init__(self,
                 profiling='default'):
    ...
    def fit_predict(self, data,
        ...
        categorical_variable : ListOfStrings or str, optional
            Specifies names of integer columns in ``data`` that should be treated as categorical.

            By default all integer columns are treated as numerical.

        string_variable : ListOfStrings or str, optional
        ...
        grubbs_cols : ListOfStrings or str, optional
        ...
        iqr_cols : ListOfStrings or str, optional
