...
import pandas as pd
try:
    from IPython.core.display import display, update_display, Javascript, HTML
...
class AutomaticObjProxy(object):
    ...
    def cancel_task(self):
        ...
        # avoid executing the same ID multiple times when send cancel command
        self.target_obj.progress_indicator_id = "cancelled_{}".format(self.progress_indicator_id)
...
class TaskManager(threading.Thread):
    def __init__(self, automatic_obj_proxy: AutomaticObjProxy, connection_context: ConnectionContext, update_ui_interval, fetch_table_interval):
        ...
        self.cancelled = False   # fronted send cancel
...
class FetchProgressStatusFromSystemTableTask(threading.Thread):
    ...
    def parse_fetched_data(self, fetched_data, fetched_columns):
        ...
        if self.initialized_progress_status is None:
            ...
            self.initialized_progress_status = {
                'EXECUTION_ID': self.manager.automatic_obj_proxy.progress_indicator_id,
    ...
    def run(self):
        while True:
            if self.manager.is_interrupted() or self.manager.is_cancelled() or self.is_fetch_completed():
...
class UpdateProgressStatusToUITask(object):
    ...
    def __init__(self, manager: TaskManager, update_ui_interval):
        ...
        self.frame_file_path = get_tempdir_path() + os.sep + self.frame_id

        self.self_timer = None
        self.update_completed = False

    def is_update_completed(self):
        return self.update_completed
    ...
    def update_display(self, js_str):
        update_display(Javascript("{};{};".format(js_str, "{}".format("const scripts = document.getElementsByTagName('script');for (let i = 0; i < scripts.length; i++) {const hanamlPipelinePNode = scripts[i].parentNode;if(hanamlPipelinePNode.tagName == 'DIV' && scripts[i].innerText.indexOf('hanamlPipelinePNode') >= 0){hanamlPipelinePNode.remove();}}"))), display_id=self.frame_id)

    def generate_js_str(self, progress_status_str):
        js_str = "targetWindow['{}']={}".format(self.manager.automatic_obj_proxy.progress_indicator_id, progress_status_str)
...
class PipelineProgressStatusMonitor(object):
    ...
    update_ui_interval : float, optional
    ...
    """
    def __init__(self, connection_context: ConnectionContext, automatic_obj, update_ui_interval=0.01, fetch_table_interval=1):
    ...
    def start(self):
        ...
        TaskManager(
            ...
            self.fetch_table_interval).start()
