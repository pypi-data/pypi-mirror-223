...
class AssertsContentEssayweb(SqlsContentEssayweb):
    ...
    def assert_content_essayweb_add(self, _assert=True, **kwargs):
        ...
        # flag = self.compare_json_list(self.res, out, ["publishChannel", "essPicUrl", "publishType", "author", "essayId", "title", "content", "status", "publishTime"])
        # assert flag, "数据比较不一致"
    ...
    def assert_content_essayweb_getbyid(self, _assert=True, **kwargs):
        ...
        # flag = self.compare_json_list(self.res, out, ["subjectId", "essayId"])
        # assert flag, "数据比较不一致"
