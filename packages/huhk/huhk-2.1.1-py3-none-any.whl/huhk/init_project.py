...
class GetApi(ProjectBase):
    ...
    @staticmethod
    def get_testcase_path(path=None):
        testcase_path = GetApi._get_service_value("testcase_path", _type=1)
        if not testcase_path or not os.path.exists(testcase_path):
            ...
            for dirpath, dirnames, filenames in os.walk(path):
                if "apis" in dirnames and "asserts" in dirnames and "funs" in dirnames and "sqls" in dirnames \
                    ...
                    testcase_path = os.path.join(os.path.dirname(os.path.dirname(dirpath)), "testcase")
                    GetApi._set_service_value("testcase_path", testcase_path, _type=1)
        if not testcase_path and len(path) > 5:
            GetApi.get_testcase_path(os.path.dirname(path))
    ...
    @staticmethod
    def get_running_path(path=None):
        testcase_dir = GetApi.get_testcase_path()
        if testcase_dir:
