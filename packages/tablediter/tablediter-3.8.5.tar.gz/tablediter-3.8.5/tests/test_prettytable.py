...
def helper_table(rows=3):
    t = tablediter(["Field 1", "Field 2", "Field 3"])
    v = 1
...
class TestNoneOption:
    def test_none_char_valid_option(self):
        tablediter(["Field 1", "Field 2", "Field 3"], none_format="")

    def test_none_char_invalid_option(self):
        with pytest.raises(TypeError) as exc:
            tablediter(["Field 1", "Field 2", "Field 3"], none_format=2)
...
""".strip()
    ...
    def test_replace_none_all(self):
        t = tablediter(["Field 1", "Field 2", "Field 3"], none_format="N/A")
...
class TestBuildEquivalence:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                pytest.lazy_fixture("row_tablediter"),
                pytest.lazy_fixture("col_tablediter"),
            ),
            (
                pytest.lazy_fixture("row_tablediter"),
                pytest.lazy_fixture("mix_tablediter"),
            ),
        ],
    )
    def test_equivalence_ASCII(self, left_hand: tablediter, right_hand: tablediter):
    ...
    def test_equivalence_HTML(self, left_hand: tablediter, right_hand: tablediter):
    ...
    def test_equivalence_latex(self, left_hand: tablediter, right_hand: tablediter):
...
class TestFieldNameLessTable:
    ...
    def test_can_string_ASCII(self, field_name_less_table: tablediter):
    ...
    def test_can_string_HTML(self, field_name_less_table: tablediter):
        output = field_name_less_table.get_html_string()
        assert "<th>Field 1</th>" in output
        assert "<td>Adelaide</td>" in output

    def test_can_string_latex(self, field_name_less_table: tablediter):
        output = field_name_less_table.get_latex_string()
        assert "Field 1 & Field 2 & Field 3 & Field 4 \\\\" in output
        assert "Adelaide & 1295 & 1158259 & 600.5 \\\\" in output

    def test_add_field_names_later(self, field_name_less_table: tablediter):
...
class TestAlignment:
    ...
    def test_aligned_ascii(
        self, aligned_before_table: tablediter, aligned_after_table: tablediter
    ):
...
@pytest.fixture(scope="function")
def city_data_tablediter():
    """Just build the Australian capital city data example table."""
    pt = tablediter(["City name", "Area", "Population", "Annual Rainfall"])
...
class TestOptionOverride:
    ...
    def test_border(self, city_data_tablediter: tablediter):
        default = city_data_tablediter.get_string()
        override = city_data_tablediter.get_string(border=False)
        assert default != override

    def test_header(self, city_data_tablediter):
        default = city_data_tablediter.get_string()
        override = city_data_tablediter.get_string(header=False)
        assert default != override

    def test_hrules_all(self, city_data_tablediter):
        default = city_data_tablediter.get_string()
        override = city_data_tablediter.get_string(hrules=ALL)
        assert default != override

    def test_hrules_none(self, city_data_tablediter):
        default = city_data_tablediter.get_string()
        override = city_data_tablediter.get_string(hrules=NONE)
        assert default != override
...
class TestOptionAttribute:
    ...
    def test_set_for_all_columns(self, city_data_tablediter):
        city_data_tablediter.field_names = sorted(city_data_tablediter.field_names)
        city_data_tablediter.align = "l"
        city_data_tablediter.max_width = 10
        city_data_tablediter.start = 2
        city_data_tablediter.end = 4
        city_data_tablediter.sortby = "Area"
        city_data_tablediter.reversesort = True
        city_data_tablediter.header = True
        city_data_tablediter.border = False
        city_data_tablediter.hrules = True
        city_data_tablediter.int_format = "4"
        city_data_tablediter.float_format = "2.2"
        city_data_tablediter.padding_width = 2
        city_data_tablediter.left_padding_width = 2
        city_data_tablediter.right_padding_width = 2
        city_data_tablediter.vertical_char = "!"
        city_data_tablediter.horizontal_char = "~"
        city_data_tablediter.junction_char = "*"
        city_data_tablediter.top_junction_char = "@"
        city_data_tablediter.bottom_junction_char = "#"
        city_data_tablediter.right_junction_char = "$"
        city_data_tablediter.left_junction_char = "%"
        city_data_tablediter.top_right_junction_char = "^"
        city_data_tablediter.top_left_junction_char = "&"
        city_data_tablediter.bottom_right_junction_char = "("
        city_data_tablediter.bottom_left_junction_char = ")"
        city_data_tablediter.format = True
        city_data_tablediter.attributes = {"class": "tablediter"}
        assert (
            city_data_tablediter.get_string() == city_data_tablediter[:].get_string()
        )

    def test_set_for_one_column(self, city_data_tablediter):
        city_data_tablediter.align["Rainfall"] = "l"
        city_data_tablediter.max_width["Name"] = 10
        city_data_tablediter.int_format["Population"] = "4"
        city_data_tablediter.float_format["Area"] = "2.2"
        assert (
            city_data_tablediter.get_string() == city_data_tablediter[:].get_string()
        )

    def test_preserve_internal_border(self):
        pt = tablediter(preserve_internal_border=True)
...
class TestBasic:
    ...
    def test_table_rows(self, city_data_tablediter: tablediter) -> None:
        rows = city_data_tablediter.rows
        assert len(rows) == 7
        assert rows[0] == ["Adelaide", 1295, 1158259, 600.5]

    def _test_no_blank_lines(self, table: tablediter):
        string = table.get_string()
        lines = string.split("\n")
        assert "" not in lines

    def _test_all_length_equal(self, table: tablediter):
        string = table.get_string()
        lines = string.split("\n")
    ...
    def test_no_blank_lines(self, city_data_tablediter):
        """No table should ever have blank lines in it."""
        self._test_no_blank_lines(city_data_tablediter)

    def test_all_lengths_equal(self, city_data_tablediter):
        """All lines in a table should be of the same length."""
        self._test_all_length_equal(city_data_tablediter)

    def test_no_blank_lines_with_title(self, city_data_tablediter: tablediter):
        """No table should ever have blank lines in it."""
        city_data_tablediter.title = "My table"
        self._test_no_blank_lines(city_data_tablediter)

    def test_all_lengths_equal_with_title(self, city_data_tablediter: tablediter):
        """All lines in a table should be of the same length."""
        city_data_tablediter.title = "My table"
        self._test_all_length_equal(city_data_tablediter)

    def test_all_lengths_equal_with_long_title(
        self, city_data_tablediter: tablediter
    ):
        """All lines in a table should be of the same length, even with a long title."""
        city_data_tablediter.title = "My table (75 characters wide) " + "=" * 45
        self._test_all_length_equal(city_data_tablediter)

    def test_no_blank_lines_without_border(self, city_data_tablediter: tablediter):
        """No table should ever have blank lines in it."""
        city_data_tablediter.border = False
        self._test_no_blank_lines(city_data_tablediter)

    def test_all_lengths_equal_without_border(self, city_data_tablediter: tablediter):
        """All lines in a table should be of the same length."""
        city_data_tablediter.border = False
        self._test_all_length_equal(city_data_tablediter)

    def test_no_blank_lines_without_header(self, city_data_tablediter: tablediter):
        """No table should ever have blank lines in it."""
        city_data_tablediter.header = False
        self._test_no_blank_lines(city_data_tablediter)

    def test_all_lengths_equal_without_header(self, city_data_tablediter: tablediter):
        """All lines in a table should be of the same length."""
        city_data_tablediter.header = False
        self._test_all_length_equal(city_data_tablediter)

    def test_no_blank_lines_with_hrules_none(self, city_data_tablediter: tablediter):
        """No table should ever have blank lines in it."""
        city_data_tablediter.hrules = NONE
        self._test_no_blank_lines(city_data_tablediter)
    ...
    def test_no_blank_lines_with_hrules_all(self, city_data_tablediter: tablediter):
        """No table should ever have blank lines in it."""
        city_data_tablediter.hrules = ALL
        self._test_no_blank_lines(city_data_tablediter)
    ...
    def test_no_blank_lines_with_style_msword(self, city_data_tablediter: tablediter):
        """No table should ever have blank lines in it."""
        city_data_tablediter.set_style(MSWORD_FRIENDLY)
        self._test_no_blank_lines(city_data_tablediter)
    ...
    def test_no_blank_lines_with_int_format(self, city_data_tablediter: tablediter):
        """No table should ever have blank lines in it."""
        city_data_tablediter.int_format = "04"
        self._test_no_blank_lines(city_data_tablediter)
    ...
    def test_no_blank_lines_with_float_format(self, city_data_tablediter: tablediter):
        """No table should ever have blank lines in it."""
        city_data_tablediter.float_format = "6.2f"
        self._test_no_blank_lines(city_data_tablediter)
    ...
    def test_no_blank_lines_from_csv(self, city_data_from_csv: tablediter):
        """No table should ever have blank lines in it."""
        self._test_no_blank_lines(city_data_from_csv)

    def test_all_lengths_equal_from_csv(self, city_data_from_csv: tablediter):
        """All lines in a table should be of the same length."""
...
class TestEmptyTable:
    ...
    def test_print_empty_true(self, city_data_tablediter: tablediter):
        empty_pt = tablediter()
        ...
        assert empty_pt.get_string(
            print_empty=True
        ) != city_data_tablediter.get_string(print_empty=True)

    def test_print_empty_false(self, city_data_tablediter: tablediter):
        empty_pt = tablediter()
        empty_pt.field_names = ["City name", "Area", "Population", "Annual Rainfall"]
        ...
        assert empty_pt.get_string(
            print_empty=False
        ) != city_data_tablediter.get_string(print_empty=False)
...
class TestSlicing:
    def test_slice_all(self, city_data_tablediter: tablediter):
        sliced_pt = city_data_tablediter[:]
        assert city_data_tablediter.get_string() == sliced_pt.get_string()

    def test_slice_first_two_rows(self, city_data_tablediter: tablediter):
        sliced_pt = city_data_tablediter[0:2]
    ...
    def test_slice_last_two_rows(self, city_data_tablediter: tablediter):
        sliced_pt = city_data_tablediter[-2:]
        string = sliced_pt.get_string()
        assert len(string.split("\n")) == 6
...
class TestSorting:
    def test_sort_by_different_per_columns(self, city_data_tablediter: tablediter):
        city_data_tablediter.sortby = city_data_tablediter.field_names[0]
        old = city_data_tablediter.get_string()
        for field in city_data_tablediter.field_names[1:]:
            city_data_tablediter.sortby = field
            new = city_data_tablediter.get_string()
            assert new != old

    def test_reverse_sort(self, city_data_tablediter: tablediter):
        for field in city_data_tablediter.field_names:
            city_data_tablediter.sortby = field
            city_data_tablediter.reversesort = False
            forward = city_data_tablediter.get_string()
            city_data_tablediter.reversesort = True
            backward = city_data_tablediter.get_string()
    ...
    def test_sort_key(self, city_data_tablediter: tablediter):
        # Test sorting by length of city name
        def key(vals):
            vals[0] = len(vals[0])
            return vals

        city_data_tablediter.sortby = "City name"
        city_data_tablediter.sort_key = key
        assert (
            city_data_tablediter.get_string().strip()
            == """
+-----------+------+------------+-----------------+
| City name | Area | Population | Annual Rainfall |
+-----------+------+------------+-----------------+
...
class TestFloatFormat:
    def test_no_decimals(self, float_pt: tablediter):
        float_pt.float_format = ".0f"
        float_pt.caching = False
        assert "." not in float_pt.get_string()

    def test_round_to_5DP(self, float_pt: tablediter):
    ...
    def test_pad_with_2Zeroes(self, float_pt: tablediter):
...
class TestPositionalJunctions:
    ...
    def test_Default(self, city_data_tablediter: tablediter):
        city_data_tablediter.set_style(DOUBLE_BORDER)
...
╚═══════════╩══════╩════════════╩═════════════════╝""".strip()
    ...
    def test_NoHeader(self, city_data_tablediter: tablediter):
        city_data_tablediter.set_style(DOUBLE_BORDER)
        city_data_tablediter.header = False
...
╚═══════════╩══════╩═════════╩════════╝""".strip()
    ...
    def test_WithTitle(self, city_data_tablediter: tablediter):
        city_data_tablediter.set_style(DOUBLE_BORDER)
        city_data_tablediter.title = "Title"
...
╚═══════════╩══════╩════════════╩═════════════════╝""".strip()
    ...
    def test_WithTitleNoHeader(self, city_data_tablediter: tablediter):
        city_data_tablediter.set_style(DOUBLE_BORDER)
        city_data_tablediter.title = "Title"
        city_data_tablediter.header = False
        assert (
...
╚═══════════╩══════╩═════════╩════════╝""".strip()
    ...
    def test_HruleAll(self, city_data_tablediter: tablediter):
        city_data_tablediter.set_style(DOUBLE_BORDER)
        city_data_tablediter.title = "Title"
        city_data_tablediter.hrules = ALL
        assert (
...
╚═══════════╩══════╩════════════╩═════════════════╝""".strip()
    ...
    def test_VrulesNone(self, city_data_tablediter: tablediter):
        city_data_tablediter.set_style(DOUBLE_BORDER)
        city_data_tablediter.vrules = NONE
        assert (
            city_data_tablediter.get_string().strip()
            == "═══════════════════════════════════════════════════\n"
            "  City name   Area   Population   Annual Rainfall  \n"
    ...
    def test_VrulesFrameWithTitle(self, city_data_tablediter: tablediter):
        city_data_tablediter.set_style(DOUBLE_BORDER)
        city_data_tablediter.vrules = FRAME
        city_data_tablediter.title = "Title"
        assert (
            city_data_tablediter.get_string().strip()
            == """
...
class TestJSONConstructor:
    def test_JSONAndBack(self, city_data_tablediter: tablediter):
        json_string = city_data_tablediter.get_json_string()
        new_table = from_json(json_string)
        assert new_table.get_string() == city_data_tablediter.get_string()


class TestHtmlConstructor:
    def test_HtmlAndBack(self, city_data_tablediter: tablediter):
        html_string = city_data_tablediter.get_html_string()
        new_table = from_html(html_string)[0]
        assert new_table.get_string() == city_data_tablediter.get_string()

    def test_HtmlOneAndBack(self, city_data_tablediter: tablediter):
        html_string = city_data_tablediter.get_html_string()
        new_table = from_html_one(html_string)
        assert new_table.get_string() == city_data_tablediter.get_string()

    def test_HtmlOneFailOnMany(self, city_data_tablediter: tablediter):
        html_string = city_data_tablediter.get_html_string()
        html_string += city_data_tablediter.get_html_string()
        with pytest.raises(ValueError):
...
def japanese_pretty_table():
    pt = tablediter(["Kanji", "Hiragana", "English"])
...
def emoji_pretty_table():
    ...
    pt = tablediter(["Thunderbolt", "Lightning"])
...
class TestMultiPattern:
    @pytest.mark.parametrize(
        ...
        [
            (
                pytest.lazy_fixture("city_data_tablediter"),
                """
+-----------+------+------------+-----------------+
| City name | Area | Population | Annual Rainfall |
+-----------+------+------------+-----------------+
...
+-----------------+-----------------+
    ...
    def test_multi_pattern_outputs(
        self, pt: tablediter, expected_output: str, test_type: str
    ):
...
@pytest.fixture(scope="function")
def unpadded_pt():
    pt = tablediter(header=False, padding_width=0)
    pt.add_row("abc")
    pt.add_row("def")
    pt.add_row("g..")
    return pt
...
class TestUnpaddedTable:
    def test_unbordered(self, unpadded_pt: tablediter):
...
"""
    ...
    def test_bordered(self, unpadded_pt: tablediter):
...
class TestCustomFormatter:
    ...
    def test_init_custom_format_throw_error_is_not_callable(self):
        with pytest.raises(ValueError) as e:
            tablediter(custom_format={"col1": "{:.2}"})
    ...
    def test_use_custom_formatter_for_int(self, city_data_tablediter: tablediter):
        city_data_tablediter.custom_format["Annual Rainfall"] = lambda n, v: f"{v:.2f}"
        assert (
            city_data_tablediter.get_string().strip()
            == """
+-----------+------+------------+-----------------+
...
""".strip()
    ...
    def test_custom_format_multi_type(self):
        pt = tablediter(["col_date", "col_str", "col_float", "col_int"])
...
class TestRepr:
    def test_default_repr(self, row_tablediter: tablediter):
        assert row_tablediter.__str__() == row_tablediter.__repr__()

    def test_jupyter_repr(self, row_tablediter: tablediter):
        assert row_tablediter._repr_html_() == row_tablediter.get_html_string()
...
class TestClearing:
    def test_clear_rows(self, row_tablediter: tablediter):
        t = helper_table()
    ...
    def test_clear(self, row_tablediter: tablediter):
        t = helper_table()
        t.add_row(["a", "b", "c"], divider=True)
        t.clear()
        assert t.rows == []
