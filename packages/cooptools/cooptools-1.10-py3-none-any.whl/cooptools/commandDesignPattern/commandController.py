...
from typing import List, Tuple, TypeVar, Dict, Protocol
import threading
import copy
import logging
import pprint
...
class CommandStore(Protocol):
    ...
    def last_cached(self, max_idx = None) -> Tuple[T, int]:
        max_cached_idx = max(x for x in self.get_cached().keys() if max_idx is None or x <= max_idx)
        last_cached_state = self.get_cached()[max_cached_idx]
