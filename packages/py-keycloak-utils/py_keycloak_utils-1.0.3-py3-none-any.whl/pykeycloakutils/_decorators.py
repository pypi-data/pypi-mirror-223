    ...
    def funct_decorator(org_function):
        ...
        def start_session(*args, **kwargs):
            with local_session() as session:
                #session.execute(text(f"SET search_path TO tenant_{urlparse(request.url).hostname.split('.')[0]}"))
                print(f"session tenant: {request.headers.get('X-Tenant-Id')}")
                session.execute(text(f"SET search_path TO tenant_{request.headers.get('X-Tenant-Id')}"))
