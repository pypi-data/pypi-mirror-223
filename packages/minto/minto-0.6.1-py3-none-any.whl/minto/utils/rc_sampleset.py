...
def from_old_sampleset(sampleset: typ.Union[SampleSet, jm.SampleSet]) -> SampleSet:
    ...
    for i, sol in enumerate(solutions):
        energy = sampleset.evaluation.energy
        objective = sampleset.evaluation.objective
        ...
        data.append(
            Sample(
                vars=sol,
                evaluation_result=EvaluationResult(
                    energy=energy[i] if energy is not None else 0.0,
                    objective=objective[i] if objective is not None else 0.0,
                    constraints=constraint,
                    penalties={},
                ),
                num_occurrences=sampleset.record.num_occurrences[i],
                meta_info={},
                run_id=run_id,
            )
        )
