...
SERVER_ID = 'HiveMind SSDP Server V0.1'
...
class SSDPServer(threading.Thread):
    ...
    def datagram_received(self, data, host_port):
        ...
        # LOG.info('SSDP command %s %s - from %s:%d' % (cmd[0], cmd[1], host,
        #                                            port))
        ...
        if cmd[0] == 'M-SEARCH' and cmd[1] == '*':
            ...
            LOG.warning('Unknown SSDP command %s %s' % (cmd[0], cmd[1]))
    ...
    def register(self, manifestation, usn, st, location, server=SERVER_ID, cache_control='max-age=1800', silent=False,
        ...
        LOG.info('Registering %s (%s)' % (st, location))
    ...
    def send_it(self, response, destination, delay, usn):
        # LOG.debug('send discovery response delayed by %ds for %s to %r' % (
        #    delay, usn, destination))
        try:
            self.sock.sendto(response.encode(), destination)
        except (AttributeError, socket.error) as msg:
            LOG.warning("failure sending out byebye notification: %r" % msg)
    ...
    def discovery_request(self, headers, host_port):
        ...
        # LOG.info('Discovery request from (%s,%d) for %s' % (host, port,
        #                                                  headers['st']))
        # LOG.info('Discovery request for %s' % headers['st'])
    ...
    def do_notify(self, usn):
        ...
        if self.known[usn]['SILENT']:
            return
        LOG.info('Sending alive notification for %s' % usn)
        ...
        resp.extend(('', ''))
        LOG.debug('do_notify content', resp)
        try:
            self.sock.sendto('\r\n'.join(resp).encode(), (SSDP_ADDR, SSDP_PORT))
            self.sock.sendto('\r\n'.join(resp).encode(), (SSDP_ADDR, SSDP_PORT))
        except (AttributeError, socket.error) as msg:
            LOG.warning("failure sending out alive notification: %r" % msg)

    def do_byebye(self, usn):
        """Do byebye"""

        LOG.info('Sending byebye notification for %s' % usn)
        ...
        try:
            ...
            resp.extend(map(lambda x: ': '.join(x), stcpy.items()))
            resp.extend(('', ''))
            # LOG.debug('do_byebye content', resp)
            if self.sock:
                try:
                    self.sock.sendto('\r\n'.join(resp), (SSDP_ADDR, SSDP_PORT))
                except (AttributeError, socket.error) as msg:
                    LOG.error("failure sending out byebye notification: %r" %
                              msg)
        except KeyError as msg:
            LOG.error("error building byebye notification: %r" % msg)
