        ...
        def make_fc(archive, debug=False):
            ...
            df = df.with_columns(
                pl.col("FilingCharges").apply(
                    lambda x: re.split(
                        r"(?m)(ALCOHOL|BOND FORFEITURE|CONSERVATION|DOCKET|DRUG|GOVERNMENT|HEALTH|MUNICIPAL|OTHER|PERSONAL|PROPERTY|SEX|TRAFFIC|DOCKET/MISC|REVENUE – PSC \(PUBLIC SERVICE COMMISSION\)|MUNICIPAL ORDINANCE)\s*$",
                        x,
                    )
                )
            )
            ...
            with console.status("Parsing disposition charges…"):
                ...
                df = df.with_columns(
                    [
                        ...
                        pl.col("Row")
                        .str.extract(
                            r"(BOUND|GUILTY PLEA|WAIVED TO GJ|DISMISSED|TIME LAPSED|NOL PROSS|CONVICTED|INDICTED PRIOR TO ADJUDICATIO N|DISMISSED|FORFEITURE|TRANSFER|REMANDED|WAIVED|ACQUITTED|WITHDRAWN|PETITION DENIED|PRETRIAL|COND\. FORF\. SET ASIDE|COND\. FORF\.|PROBATION REVOKED|OTHER|PROBATION NT REVOKE|PROBATION/S|NO PROBABLE CAUSE|PETITION GRANTED|PROBATION TERMINATED|FINAL FORF\. SET ASIDE|DOCKETED)"
                        )
                        ...
                        .str.strip()
                        .alias("DispositionDescription"),
                        pl.col("Row")
                        .str.extract(
                            r"(WAIVED TO GJ \d\d/\d\d/\d\d\d\d|GJ|GUILTY PLEA|WAIVED TO GJ|DISMISSED|TIME LAPSED|NOL PROSS|CONVICTED|INDICTED PRIOR TO ADJUDICATIO N|DISMISSED|FORFEITURE|TRANSFER|REMANDED|WAIVED|ACQUITTED|WITHDRAWN|PETITION DENIED|PRETRIAL|COND\. FORF\. SET ASIDE|COND\. FORF\.|PROBATION REVOKED|OTHER|PROBATION NT REVOKE|PROBATION/S|NO PROBABLE CAUSE|PETITION GRANTED|PROBATION TERMINATED|FINAL FORF\. SET ASIDE|DOCKETED)\s+(ALCOHOL|BOND FORFEITURE|CONSERVATION|DOCKET|DRUG|GOVERNMENT PUBLIC|HEALTH|MUNICIPAL|OTHER|PERSONAL|PROPERTY|SEX OFFENSE|TRAFFIC|DOCKET/MISC)\s+(TRAFFIC MISDEMEANOR|BOND|FELONY|MISDEMEANOR|OTHER|VIOLATION|TRAFFIC)\s+(.+?) ([A-Z0-9]{3}-\s*[A-Z0-9]{3}-\s*[^\s]+)",
                            group_index=4,
                        )
                        .alias("DescriptionFirstLine"),
                        pl.col("Row")
                        .str.extract(
                            r"(ALCOHOL|BOND FORFEITURE|CONSERVATION|DOCKET|DRUG|GOVERNMENT PUBLIC|HEALTH|MUNICIPAL|OTHER|PERSONAL|PROPERTY|SEX OFFENSE|TRAFFIC|DOCKET/MISC|REVENUE – PSC \(PUBLIC SERVICE COMMISSION\)|MUNICIPAL ORDINANCE)\s*(TRAFFIC MISDEMEANOR|BOND|FELONY|MISDEMEANOR|OTHER|VIOLATION|TRAFFIC)",
                            group_index=2,
                        )
                        .alias("TypeDescription"),
                        pl.col("Row")
                        .str.extract(
                            r"(ALCOHOL|BOND FORFEITURE|CONSERVATION|DOCKET|DRUG|GOVERNMENT PUBLIC|HEALTH|MUNICIPAL|OTHER|PERSONAL|PROPERTY|SEX OFFENSE|TRAFFIC|DOCKET/MISC|REVENUE – PSC \(PUBLIC SERVICE COMMISSION\)|MUNICIPAL ORDINANCE)"
                        )
                        .alias("Category"),
                    ]
                )
                    ...
                    .then(
                        pl.concat_str(
                            [
                                ...
                                pl.lit(" "),
                                pl.col("DispositionDescription"),
                            ]
                        )
                    )
                    .otherwise(pl.col("DispositionDescription"))
                    .alias("DispositionDescription")
                )
                # replace stray \ with escape \\
                ...
                df = df.with_columns(
                    pl.when(
                        pl.col("DispositionCite").is_null()
                        & pl.col("DispositionDescription").str.contains(r"\d+ CFR \d+")
                    )
                    .then(
                        pl.col("DispositionDescription")
                        .str.extract(r"(\d+ CFR \d+\.\s*\d+\(?.?\)?\(?.?\)?)")
                    ...
                    .otherwise(pl.col("DispositionCite"))
                    .alias("DispositionCite"),
                    pl.when(
                        pl.col("DispositionCite").is_null()
                        & pl.col("DispositionDescription").str.contains(r"\d+ CFR \d+")
                    ...
                    .then(
                        pl.col("DispositionDescription").str.replace(
                            r"\d+ CFR \d+\.\s*\d+\(?.?\)?\(?.?\)?", ""
                        )
                    )
                    .otherwise(pl.col("DispositionDescription"))
                ...
                # fix ACT XXXX-XX cite
                df = df.with_columns(
                    pl.when(pl.col("DispositionCite").is_null())
                    .then(
                        pl.col("DispositionDescription").str.extract(r"(ACT \d+-\d+)")
                    )
                    .otherwise(pl.col("DispositionCite"))
                    .alias("DispositionCite")
                )
                ...
                df = df.with_columns(
                    pl.when(pl.col("DispositionCite").is_null())
                    .then(
                        pl.col("DispositionDescription").str.extract(r"(PSC-\d[^\s]+)")
                    )
                    .otherwise(pl.col("DispositionCite"))
                    .alias("DispositionCite")
                )
                ...
                df = df.with_columns(
                    pl.when(pl.col("DispositionCite").is_null())
                    .then(pl.col("DispositionDescription").str.extract(r"(SCR-\d+)"))
                    .otherwise(pl.col("DispositionCite"))
                    .alias("DispositionCite")
                )
                # fix 760-\d+ cite
                ...
                df = df.with_columns(
                    pl.when(pl.col("DispositionCite").is_null())
                    .then(
                        pl.col("DispositionDescription").str.extract(r"(\d+-\d[^\s]+$)")
                    )
                    .otherwise(pl.col("DispositionCite"))
                    .alias("DispositionCite")
                )
                ...
                df = df.with_columns(
                    [
                        pl.when(
                            pl.col("DispositionDescription").str.contains("RULE 32")
                            | (
                                pl.col("Row").str.contains("RULE 32")
                                & pl.col("DispositionDescription").is_null()
                            )
                        )
                        .then(pl.lit("RULE 32"))
                        .otherwise(pl.col("DispositionCite"))
                ...
                df = df.with_columns(
                    [
                        pl.when(
                            pl.col("DispositionDescription").str.contains(
                                "PROBATION REV"
                            )
                        )
                        .then(pl.lit("PROBATION REV"))
                ...
                df = df.with_columns(
                    pl.when(pl.col("DispositionCite").is_null())
                    .then(pl.col("DispositionDescription").str.extract(r"(\d+\.\d+)"))
                    .otherwise(pl.col("DispositionCite"))
                    .alias("DispositionCite"),
                    pl.when(pl.col("DispositionCite").is_null())
                    .then(
                ...
                df = df.with_columns(
                    pl.concat_str(
                        [pl.col("CaseNumber"), pl.lit("-"), pl.col("#")]
                    ).alias("ChargeID")
                )
                fc = self.filing_charges(log=False).with_columns(
                    pl.concat_str(
                        [pl.col("CaseNumber"), pl.lit("-"), pl.col("#")]
                    ).alias("ChargeID")
                )
                fc = fc.select(
                    [
                        pl.col("ChargeID"),
                        pl.col("ID"),
                        pl.col("Description").alias("FilingDescription"),
                        pl.col("Cite").alias("FilingCite"),
                    ]
                )
                df = df.join(fc, on="ChargeID", how="left")
                df = df.drop_nulls("Row")
                df = df.with_columns(
                    [
                        pl.when(
                            pl.col("FilingDescription").str.contains(
                                pl.col("DispositionDescription")
                            )
                            | pl.col("DispositionDescription").is_null()
                        )
                        .then(pl.col("FilingDescription"))
                        .otherwise(pl.col("DispositionDescription"))
                        .alias("Description"),
                        pl.when(
                            pl.col("FilingDescription").str.contains(
                ...
                df = df.with_columns(
                    [
                        pl.when(
                            pl.col("Description").is_null()
                            & pl.col("Row").str.contains("BOND FORT")
                        )
                ...
                # fix ugly -- -- from cite in description and hanging ADJUDICATIO N
                df = df.with_columns(
                    pl.col("Description")
                    .str.replace(r"-+ +-+", "")
                ...
                df = df.with_columns(
                    pl.col("CourtActionDate").dt.to_string("%y-%m-%d").alias("CourtActionDateStr")
                )
                df = df.fill_null("")
                df = df.with_columns(
                    pl.concat_str(
