...
GIT_PAT_KEY = "GIT_PERSONAL_ACCESS_TOKEN"
CRT_EXECUTION_KEY = "CIA_EXECUTION_URL"
...
class GitPackageMixin:
    def __init__(self, git: str) -> None:
        super().__init__()
        self.git = git
        # Check if the env is on CRT. If so, we stick to ssh
        post_commit_url = os.getenv(CRT_EXECUTION_KEY, "")
        if post_commit_url != "" and "crt.prod.linkedin.com" in post_commit_url:
            return
        # If env is darwin and url is using ssh, convert url to https, which accepts Personal Access Token (PAT)
        if self.git.startswith(("https", "http")):
            return
        # we always need the PAT to clone
        pat = self.fetch_pat(GIT_PAT_KEY)
        # Get suffix like "github.com:linkedin-multiproduct/in-dbt-utils.git"
        # e.g "github.com:linkedin-multiproduct/in-dbt-utils.git"
        mp_suffix_with_colon = self.git.split("@")[1]
        # Remove colon from suffix, replace with '/' like any https url
        # e.g "github.com/multiproduct/in-dbt-utils.git"
    ...
    @staticmethod
    def fetch_pat(key_name: str) -> Optional[str]:
        pat = os.getenv(key_name)
        if pat is None:
            warn_or_error(
                "Please set the environment variable {}".format(key_name),
                log_fmt=ui.red("ENV KEY ERROR: {}"),
            )
            raise EnvironmentError("Personal Access Token for GitHub not found in the env")
        return pat
