...
#-----------------------------------------------
def main(uname, uid, hash):
    make_game_html(uname, uid, hash)
    logging.basicConfig(
        level=logging.INFO,
...
def save_values(uname, uid, hash):
    ...
    with open('src\\ladderbot\\assets\\creds.json', 'w') as file:
        json.dump(data, file)
#-----------------------------------------------
def load_values():
    try:
        with open('src\\ladderbot\\assets\\creds.json', 'r') as file:
            data = json.load(file)
...
root.iconbitmap("src\\ladderbot\\assets\\Logo.ico")
