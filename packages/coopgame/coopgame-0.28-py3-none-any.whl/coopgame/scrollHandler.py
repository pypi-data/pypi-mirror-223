...
class ScrollTileHandler:
    ...
    def __init__(self,
        ...
        self.tile_size_pxls = tile_size_pxls
    ...
    def tile_window_coord(self, tile):
        tile_coords = sec_utl.coord_of_sector(tile, self.tile_size_pxls)
        return self.scroll_handler.coord_in_window(tile_coords)


    @property
    def ScrollTile(self):
        return sec_utl.sector_from_coord(self.scroll_handler.scroll, self.tile_size_pxls)

    @property
    def ScrollTileCoords(self):
        return sec_utl.coord_of_sector(self.ScrollTile, self.tile_size_pxls)

    @property
    def WorldTile(self):
        return sec_utl.sector_from_coord(self.scroll_handler.focal_point, self.tile_size_pxls)

    @property
    def WorldTileCoord(self):
        return sec_utl.coord_of_sector(self.WorldTile, self.tile_size_pxls)

    @property
    def WindowTile(self):
        return sec_utl.sector_from_coord((self.scroll_handler.focal_point[0] + self.scroll_handler.scroll[0],
                                          self.scroll_handler.focal_point[1] + self.scroll_handler.scroll[1]),
                                         self.tile_size_pxls)

    @property
    def WindowTileCoord(self):
        return sec_utl.coord_of_sector(self.WindowTile, self.tile_size_pxls)
