...
def hive_model_emotives(ensembles: dict) -> dict:
    ...
    Returns:
        dict: Dictionary of Averaged Emotives
    """
    return average_emotives([prediction_ensemble_model_classification(ensemble) for ensemble in ensembles.values()])


def prediction_ensemble_model_classification(ensemble: list) -> Union[Counter, None]:
...
def most_common_ensemble_model_classification(ensemble: list) -> Union[str, None]:
    result = prediction_ensemble_model_classification(ensemble=ensemble)
    if result is None:
        return result
    return result.most_common(1)[0][0]

def hive_model_classification(ensembles: dict) -> Union[Counter, None]:
