...
class GenomeOptimizer:
    ...
    def __init__(self,
        ...
        self.toolbox.register("mate", crossover, toolbox=self.toolbox)
        self.toolbox.register("mutate", mutate, toolbox=self.toolbox)
...
def crossover(individual1, individual2, toolbox):
...
def mutate(individual: dict, toolbox, mu: float = 0.0, sigma: float = 0.1):
    ...
    for node in individual:
        for k in individual[node]:
            ...
            new_individual[node][k] = mutate_variable(
                val=individual[node][k], key=k, mu=mu, sigma=sigma)

    return creator.Individual(new_individual),


def mutate_variable(val: Union[float, int], key: str, mu: float, sigma: float) -> float:
