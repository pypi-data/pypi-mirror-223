...
from sirius.communication.discord import TextChannel, Bot, Server, get_timestamp_string
...
class Discord:
    ...
    # @classmethod
    # def get_notification_channel(cls) -> TextChannel:
    #     if cls.wise_channel is not None:
    #         return cls.wise_channel
    #
    #     cls.bot = Bot.get()
    #     cls.server = cls.bot.get_server()
    #     cls.wise_channel = cls.server.get_text_channel(AortaTextChannels.WISE.value)
    #     return cls.wise_channel

    @classmethod
    def notify(cls, message: str) -> None:
        pass
        # (Discord.get_notification_channel()).send_message(message)
...
class CashAccount(Account):
    ...
    def transfer(self, to_account: Union["CashAccount", "ReserveAccount", "Recipient"], amount: Decimal, reference: str | None = None, is_amount_in_from_currency: bool = False) -> "Transfer":
        ...
        if isinstance(to_account, CashAccount):
            ...
            Discord.notify(f"**Intra-Account Transfer**:\n"
            ...
            Discord.notify(f"**Third-Party Transfer**:\n"
                           f"Timestamp: {get_timestamp_string(datetime.datetime.now())}\n"
                           f"From: *{self.currency.value}*\n"
    ...
    @common.only_in_dev
    def _set_balance(self, amount: Decimal) -> None:
    ...
    @common.only_in_dev
    def _set_maximum_balance(self, amount: Decimal) -> None:
        if self.balance <= amount:
            return
        ...
        if amount_to_deduct > maximum_transfer_amount:
            self.transfer(hkd_account, maximum_transfer_amount)
            self._set_maximum_balance(amount)
        else:
            self.transfer(hkd_account, amount_to_deduct, is_amount_in_from_currency=True)
...
class ReserveAccount(Account):
    ...
    def transfer(self, to_account: "CashAccount", amount: Decimal) -> "Transfer":
        amount = amount.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    ...
    def _simulate_top_up(self, amount: Decimal) -> None:
        ...
        cash_account.transfer(self, amount)
        self.profile.wise_account._initialize()
    ...
    def _set_minimum_balance(self, amount: Decimal) -> None:
        ...
        cash_account.transfer(self, amount_to_top_up)
    ...
    def _set_maximum_balance(self, amount: Decimal) -> None:
        ...
        cash_account: CashAccount = self.profile.get_cash_account(self.currency, True)
        hkd_account: CashAccount = self.profile.get_cash_account(common.Currency.HKD, True)
        self.transfer(cash_account, amount_to_deduct)

        if amount_to_deduct > maximum_transfer_amount:
            cash_account.transfer(hkd_account, maximum_transfer_amount)
            self._set_maximum_balance(amount)
        else:
            quote: Quote = Quote.get_quote(self.profile, cash_account, hkd_account, amount_to_deduct, True)
            cash_account.transfer(hkd_account, quote.from_amount)
