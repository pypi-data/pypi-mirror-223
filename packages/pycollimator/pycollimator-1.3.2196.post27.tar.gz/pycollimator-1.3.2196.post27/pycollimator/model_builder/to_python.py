...
def _to_python_str(
    ...
    for k, v in model_builder.parameters.items():
        if v.default_value is not None:
            lines.append(
                f'{builder_name}.add_parameter("{k}", "{v.value}", '
                f'default_value="{v.default_value}", description="{v.description}")'
            )
        else:
            lines.append(f'{builder_name}.add_parameter("{k}", "{v.value}")')
    ...
    if output_groups:
        for node, group in model_builder.groups.items():
            ...
            groups.add(node.id)
            lines.append(f"{node.name}_builder = make_{_normalize_name(node.name)}_builder()")

    # Submodels
    if output_submodels:
        for node, submodel in model_builder.submodels.items():
            submodel_code, other_submodels = _to_python_str(submodel, model_name=submodel.name)
    ...
    for node in model_builder.nodes.values():
        ...
        if type(node) is ReferenceSubmodel:
            ...
            lines.append(f'{node.name} = {builder_name}.add_reference_submodel("{node.name}", {submodel_name}_builder)')
        elif type(node) is Group or type(node) is Submodel:
            lines.append(f'{node.name} = {builder_name}.add_group("{node.name}", {node.name}_builder)')
        else:
