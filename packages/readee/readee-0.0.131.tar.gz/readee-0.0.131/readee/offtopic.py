...
OFFTOPIC_ATT = [
	'teaserContentWrap', 'artikel', 'latest', 'more', 'facebook', 'ts-fab-wrapper',
	...
	'sharedaddy', 'ArticleTags_root', 'imgWrap__nav', 'elementor-location-header', 
	'elementor-post-navigation', 'elementor-location-popup', 'article/support/request'
]
...
def _isOffTopic(attrs):
	...
	for k, v in attrs.items():
		if k not in ['data-component', 'data-test-id'] and matchKey(k, ['href', 'src', 'url', 'alt', 'data', 'xmlns:fb']):
			continue
...
def _decompseAds(soup):
	...
	for item in soup.find_all("p"):
		...
		if 'align=' in str(item) and 'right' in str(item) and item.text and item.text.startswith('文：'):
			_decompose(item)
		if item.parent and "['picBox', 'full', 'rechts']" in str(item.parent.attrs):
			_decompose(item)
