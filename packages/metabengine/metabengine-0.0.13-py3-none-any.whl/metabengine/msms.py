...
def spec_similarity(spec1, spec2, mz_tol=0.01, sim_method='cosine', search_type='exact', filter_low_int=True, exclude_precursor=True, remove_precursor_mz=1.6):
    ...
    if isinstance(spec1, dict):
        spec1 = Scan()
        spec1.precs_mz = spec1['PrecursorMZ']
        spec1.prod_mz_seq = spec1['prodMZ']
        spec1.prod_int_seq = spec1['prodIntensity']

    if isinstance(spec2, dict):
        spec2 = Scan()
        spec2.precs_mz = spec2['PrecursorMZ']
        spec2.prod_mz_seq = spec2['prodMZ']
        spec2.prod_int_seq = spec2['prodIntensity']
