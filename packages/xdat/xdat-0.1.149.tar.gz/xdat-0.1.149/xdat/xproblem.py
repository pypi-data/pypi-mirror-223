...
class ModelEnsemble:
    ...
    def _predict_x_dist(self, func_name, X, with_bagging=None, axis=1):
        with_bagging = with_bagging if with_bagging is not None else self.with_bagging
        X = self.prepare_X(X)

        if not with_bagging:
            all_y = [getattr(clf, func_name)(X) for clf in self.clfs]

        else:
            all_y = []
            for clf in self.clfs:
                with xmodels._prepare_for_dist(clf):
                    all_y.append(getattr(clf, func_name)(X).T)
    ...
    def predict_dist(self, X, with_bagging=None):
