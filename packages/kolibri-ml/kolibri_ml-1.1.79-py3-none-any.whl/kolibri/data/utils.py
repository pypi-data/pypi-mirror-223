...
from kolibri.default_configs import RESOURCES_AUTO_DOWNLOAD
...
def find(resource_name, paths=None, raise_error=True):
    """
...
def load(
    ...
    if RESOURCES_AUTO_DOWNLOAD:
        try:
            download(resource_url, raise_on_error=True)
        except:
            try:
                pkg = resource_url.split("/")[1]
                download(pkg)
            except:
                pass
    ...
    if format is None:
        print("No format is given, returning path")
        return _get_path(resource_url)
    ...
    if format == "raw":
        ...
        resource_val = set(line.decode("utf-8").strip('\n') for line in opened_resource)
    elif format == "yaml":
        import yaml
    ...
    if as_array and format == 'text':
        resource_val=[t.strip(' ').strip('\n') for t in resource_val.split('\n') if t.strip() != ""]
    return resource_val
