...
#include "../../../../src/sdot/Support/ThreadPool.cpp"
#include "../../../../src/sdot/Support/CbQueue.cpp"
#include "../../../../src/sdot/Support/Assert.cpp"
#include "../../../../src/sdot/Support/Mpi.cpp"

#include "../../../../src/sdot/Integration/Arfd.cpp"

#ifdef PD_WANT_STAT
#include "../../../../src/sdot/Support/Stat.cpp"
#endif
...
#include "../../../../src/sdot/PowerDiagram/get_der_integrals_wrt_weights.h"
// #include "../../../../src/sdot/PowerDiagram/get_der_boundary_integral.h"
#include "../../../../src/sdot/PowerDiagram/get_distances_from_boundaries.h"
#include "../../../../src/sdot/PowerDiagram/get_boundary_integral.h"
#include "../../../../src/sdot/PowerDiagram/get_image_integrals.h"
...
namespace {
    ...
    template<int dim>
    struct PyCellCoordinates {
        std::vector<PyCellCoordinates<dim-1>> boundaries;
    };

    template<>
    struct PyCellCoordinates<1> {
        std::array<PD_TYPE,2> coordinates;
    };

    struct PyCellBoundary {
        std::size_t               num_dirac;
        PyCellCoordinates<PD_DIM> coordinates;
    };

    struct PyCell {
        std::vector<PyCellBoundary> boundaries;
    };

    // template<class Domain,class Grid,class FUNC>
    // std::vector<PyCell> get_cell_coordinates( pybind11::array_t<TF> &positions, pybind11::array_t<TF> &weights, Domain &domain, Grid &grid, const FUNC &func ) {
    //     auto ptr_positions = reinterpret_cast<const typename Grid::Pt *>( positions.data() );
    //     auto ptr_weights = reinterpret_cast<const typename Grid::TF *>( weights.data() );
    //     auto nb_diracs = positions.shape( 0 );

    //     std::vector<PyCell> res( nb_diracs );
    //     find_radial_func( func, [&]( const auto &ft ) {
    //         grid.for_each_laguerre_cell( [&]( auto &lc, auto num_dirac, int ) {
    //             PyCell &cell = res[ num_dirac ];
    ...
    //         //sdot::get_cell_coordinates( ptr_res, grid, domain, ptr_positions, ptr_weights, positions.shape( 0 ), ft );
    //     } );
