...
def machine_learn(nirs, labels, groups, model, features, normalize=False,
    ...
    train_size : float
        Percentage of the training set to keep. Used to study the impact of
        training set size. Defaults to ``1.`` to keep the whole training set.
        ...
        if groups is None:
            ...
            if train_size == 1.:
                nirs_train, labels_train = shuffle(
                    nirs_train, labels_train, random_state=SEED)
            else:
            ...
            if train_size == 1.:
                nirs_train, labels_train, groups_train = shuffle(
                    nirs_train, labels_train, groups_train, random_state=SEED)
            else:
                split = train_test_split(
...
def deep_learn(nirs, labels, groups, n_classes, model, features=None,
    ...
    checkpoints_folder = f'{out_path}checkpoints/'

    if os.path.isdir(checkpoints_folder):
        print('\tSaved training found, loading it...')
        with open(f'{checkpoints_folder}split.pickle', 'rb') as f:
        ...
        with open(f'{checkpoints_folder}split.pickle', 'wb') as f:
            pickle.dump(out_split, f)

        for k, out_idx in enumerate(out_split):
            print(f'\tTraining outer fold #{k+1}')
            ...
            torch.save(clf.state_dict(), f'{checkpoints_folder}model_k{k}.pt')
            with open(f'{checkpoints_folder}hps_k{k}.pickle', 'wb') as f:
                pickle.dump(best_hps, f)
            with open(f'{checkpoints_folder}results_k{k}.pickle', 'wb') as f:
    ...
    for k, out_idx in enumerate(out_split):
        print(f'\tTesting outer fold #{k+1}')
        nirs_train, nirs_test = nirs[out_idx[0]], nirs[out_idx[1]]
        ...
        clf = model_class(n_classes).double()
        clf.load_state_dict(torch.load(f'{checkpoints_folder}model_k{k}.pt'))
        with open(f'{checkpoints_folder}hps_k{k}.pickle', 'rb') as f:
            best_hps = pickle.load(f)
        with open(f'{checkpoints_folder}results_k{k}.pickle', 'rb') as f:
