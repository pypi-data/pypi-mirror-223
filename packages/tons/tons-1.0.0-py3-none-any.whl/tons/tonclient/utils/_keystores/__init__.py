...
class KeyStores:
    ...
    def keystore_already_exists(self, name: str) -> bool:
        """
        Check whether a keystore with the given name already exists.
    ...
    def get_keystore(self, keystore_name: Optional[str], raise_none: bool = False) -> Optional[BaseKeyStore]:
        ...
        try:
            return self.__load_keystore_and_upgrade(keystore_path)
    ...
    def restore_tons_keystore(self, name: str, filepath: str, keystore_type: KeyStoreTypeEnum,
        ...
        Notes:
            - The function internally uses `__load_keystore_and_upgrade()`
              and `__restore_unencrypted_keystore()` methods to handle the keystore restoration process.
        ...
        if encrypted:
            ...
            self.__restore_unencrypted_keystore(name, keystore_type, secret,
    ...
    def __restore_unencrypted_keystore(self, name: str, keystore_type, secret: str, keystore_backup: KeystoreBackup):
        name = self.__keystore_name(name)

        keystore = self.create_new_keystore(name, keystore_type, secret, save=False)

        for backup_record in keystore_backup.records:
            keystore.add_new_record(backup_record.name, backup_record.mnemonics.split(" "), backup_record.version,
                                    backup_record.workchain, backup_record.subwallet_id,
                                    backup_record.comment, save=False, allow_empty_name=True)

        keystore.contacts = keystore_backup.contacts
        keystore.save()
