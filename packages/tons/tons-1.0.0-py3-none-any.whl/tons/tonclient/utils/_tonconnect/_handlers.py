...
def initiate_connect_event(config, keystore, wallet_name, universal_link):
    wallet_record = keystore.get_record_by_name(wallet_name, raise_none=True)
    mnemonics = keystore.get_secret(wallet_record).split(" ")
...
def accept_request(keystore, wallet_name, dapp_client_id, event_id, request: Union[SendTransactionRequest], ton_client,
                   session, bridge):
    wallet_record = keystore.get_record_by_name(wallet_name, raise_none=True)
    mnemonics = keystore.get_secret(wallet_record).split(" ")
    _mnemonics, pub_k, _priv_k, wallet = Wallets.from_mnemonics(mnemonics, wallet_record.version,
