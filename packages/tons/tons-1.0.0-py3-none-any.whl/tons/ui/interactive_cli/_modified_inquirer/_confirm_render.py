...
class RemovePrevAfterEnterRender(ConfirmRender):
    ...
    def get_lines_count_to_erase(self):
        lines_count = len(text_to_not_formatted_displayed_lines(self.question.text_to_erase.rstrip('\n'),
                                                                self.terminal.width))
        # TODO: this is a crutch, there should be a way to get formatted header and options explicitly
        # lines_count += len(text_to_not_formatted_displayed_lines(f'[?] {self.question.message}:',
        #                                                          self.terminal.width))
        return lines_count
