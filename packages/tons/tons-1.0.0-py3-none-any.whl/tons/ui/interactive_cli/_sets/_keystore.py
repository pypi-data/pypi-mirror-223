...
class KeystoreBaseSet(ABC, BaseSet, KeyStoreMixin):
    def _get_secret(self, keystore_type):
...
class KeystoreSet(KeystoreBaseSet):
    ...
    def _handle_pending(self):
        ...
        table.align["Task"] = 'l'
        for background_task in self.ctx.background_task_manager.tasks_list:
            result_description = truncate(background_task.result_description, max_len=100)
            result_description += Fore.RESET
            table.add_row([background_task.description,
    ...
    def _handle_backup_keystore(self):
        ...
        ans = self._prompt([
            inquirer.Text("backup_file_path", message=f'Backup file path (relative to {getcwd_pretty()})'),
            ModifiedConfirm(
                "is_sure",
        ...
        if not self.ctx.keystore.type == KeyStoreTypeEnum.password:
            echo_error("Backup process only supports password type keystore")
            return
