    ...
    def filter_choices(self, filter_str: t.Optional[str]):
        if filter_str:
            self._choices = [choice for (choice, parsed_choice) in self.start_choices_generator()
                             if getattr(parsed_choice, "value", parsed_choice) is None
                             or filter_str.lower() in getattr(parsed_choice, "label", parsed_choice).lower()]

        else:
            self._choices = self._start_choices

    def start_choices_generator(self):
