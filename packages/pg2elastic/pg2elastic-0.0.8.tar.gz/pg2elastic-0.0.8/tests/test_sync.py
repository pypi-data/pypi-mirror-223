...
from pg2elastic.base import Base, Payload
from pg2elastic.exc import (
    InvalidTGOPError,
    PrimaryKeyNotFoundError,
    RDSError,
    SchemaError,
)
from pg2elastic.node import Node
from pg2elastic.singleton import Singleton
from pg2elastic.sync import settings, Sync
...
class TestSync(object):
    ...
    @patch("pg2elastic.sync.logger")
    def test_logical_slot_changes(self, mock_logger, sync):
        with patch("pg2elastic.sync.Sync.logical_slot_peek_changes") as mock_peek:
            mock_peek.side_effect = [
                [ROW("BEGIN: blah", 1234)],
                [],
            ]
            with patch("pg2elastic.sync.Sync.sync") as mock_sync:
        ...
        with patch("pg2elastic.sync.Sync.logical_slot_peek_changes") as mock_peek:
            ...
            with patch(
                "pg2elastic.sync.Sync.logical_slot_get_changes"
            ) as mock_get:
                with patch("pg2elastic.sync.Sync.sync") as mock_sync:
                    sync.logical_slot_changes()
                    mock_peek.assert_any_call(
        ...
        with pytest.raises(Exception) as excinfo:
            with patch(
                "pg2elastic.sync.Sync.logical_slot_peek_changes"
            ) as mock_peek:
                mock_peek.side_effect = [
                    [
                        ROW(
                ...
                with patch(
                    ...
                    with patch(
                        "pg2elastic.sync.Sync.parse_logical_slot",
    ...
    @patch("pg2elastic.sync.SearchClient")
    def test_sync_validate(self, mock_es):
        ...
        with pytest.raises(RuntimeError) as excinfo:
            with patch(
                "pg2elastic.base.Base.pg_settings",
        ...
        with pytest.raises(RuntimeError) as excinfo:
            with patch(
                "pg2elastic.base.Base.replication_slots",
                return_value=None,
            ):
                Sync(
                    document={
        ...
        with patch(
            "pg2elastic.base.Base._materialized_views",
    ...
    def test_status(self, sync):
        with patch("pg2elastic.sync.logger") as mock_logger:
            sync._status("mydb")
    ...
    def test_truncate_slots(self, mock_logger, sync):
        with patch("pg2elastic.sync.Sync.logical_slot_get_changes") as mock_get:
            sync._truncate = True
    ...
    @patch("pg2elastic.sync.SearchClient.bulk")
    @patch("pg2elastic.sync.logger")
    def test_pull(self, mock_logger, mock_es, sync):
        with patch("pg2elastic.sync.Sync.logical_slot_changes") as mock_get:
            sync.pull()
            txmin = None
    ...
    @patch("pg2elastic.sync.Sync._on_publish")
    ...
    def test_sync_analyze(self, sync):
        ...
        with patch("pg2elastic.sync.logger") as mock_logger:
            with patch("pg2elastic.sync.Sync.indices") as mock_indices:
    ...
    @patch("pg2elastic.search_client.SearchClient.bulk")
    ...
    def test__payload(self, sync):
        with patch("pg2elastic.sync.Sync._insert_op") as mock_op:
            with patch("pg2elastic.sync.logger") as mock_logger:
        ...
        with patch("pg2elastic.sync.Sync._update_op") as mock_op:
            with patch("pg2elastic.sync.logger") as mock_logger:
                for _ in sync._payloads(
                    [
                        Payload(
        ...
        with patch("pg2elastic.sync.Sync._delete_op") as mock_op:
            with patch("pg2elastic.sync.logger") as mock_logger:
                for _ in sync._payloads(
                    [
                        Payload(
        ...
        with patch("pg2elastic.sync.Sync._truncate_op") as mock_op:
            with patch("pg2elastic.sync.logger") as mock_logger:
                for _ in sync._payloads(
                    [
                        Payload(
    ...
    @patch("pg2elastic.sync.SearchClient._create_setting")
    ...
    @patch("pg2elastic.sync.Sync.teardown")
    def test_setup(self, mock_teardown, sync):
        with patch("pg2elastic.sync.Base.create_function") as mock_create_function:
            with patch("pg2elastic.sync.Base.create_view") as mock_create_view:
                with patch(
                    "pg2elastic.sync.Base.create_triggers"
                ) as mock_create_triggers:
                    with patch(
                        "pg2elastic.sync.Base.create_replication_slot"
    ...
    @patch("pg2elastic.redisqueue.RedisQueue.delete")
    def test_teardown(self, mock_redis, sync):
        with patch("pg2elastic.sync.Base.drop_function") as mock_drop_function:
            with patch("pg2elastic.sync.Base.drop_view") as mock_drop_view:
                with patch(
                    "pg2elastic.sync.Base.drop_triggers"
                ) as mock_drop_triggers:
                    with patch(
                        "pg2elastic.sync.Base.drop_replication_slot"
        ...
        with patch("pg2elastic.sync.logger") as mock_logger:
            with patch("pg2elastic.sync.Base.drop_replication_slot"):
    ...
    def test_payloads_in_batches(self, mocker, sync):
        ...
        with patch("pg2elastic.sync.Sync._update_op", return_value=filters):
            with patch("pg2elastic.sync.Sync.sync") as mock_sync:
    ...
    @patch("pg2elastic.sync.compiled_query")
    ...
    @patch("pg2elastic.sync.logger")
    @patch("pg2elastic.sync.Sync.refresh_views")
    @patch("pg2elastic.sync.Sync.on_publish")
    @patch("pg2elastic.sync.time")
    def test_poll_redis(
