...
from dna_engine.dna_engine import audio_manager
from dna_engine.dna_engine import input_manager
        ...
        try:
            if filename not in Game.__fonts:
                if is_system_font:
                    ...
                    Game.__fonts[filename] = pygame.freetype.Font("dist/" + filename if not Game.__is_running_through_bundle() else filename, font_size)
    ...
    def __get_texture(filename : str) -> pygame.Surface:
        try:
            ...
            if filename not in Game.__textures:
                #Game.__build_asset(filename)
                Game.__textures[filename] = pygame.image.load("dist/" + filename if not Game.__is_running_through_bundle() else filename).convert_alpha()
        except FileNotFoundError:
    ...
    def __get_audio(filename : str) -> pygame.mixer.Sound:
        try:
            ...
            if filename not in Game.__audio:
                #Game.__build_asset(filename)
                Game.__audio[filename] = pygame.mixer.Sound("dist/" + filename if not Game.__is_running_through_bundle() else filename)
        except FileNotFoundError:
