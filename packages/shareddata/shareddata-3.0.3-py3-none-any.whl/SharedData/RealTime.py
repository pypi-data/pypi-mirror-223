...
class RealTime():
    ...
    def accept_clients_thread(host, port):    
        ...
        while True:
            ...
            threading.Thread(target=RealTime.handle_client_thread, args=(conn, addr)).start()
                        ...
                        if data:                        
                            ...
                            if msgtype=='subscribe':
                                ...
                                if container=='table':
                                    ...
                                    sockdata['table'] = RealTime.shdata.table(database,period,source,tablename)
                                    ...
                                    sockdata['maxrows'] = int(np.floor(1024/table.itemsize))
                        else:
                    ...
                    try:                    
                        ...
                        if data == b'':
                            msg = 'Subscription %s,%s,%s,table,%s closed !' % \
                                (table.database,table.period,table.source,table.tablename)
                            ...
                            while len(bytes_buffer) >= shnumpy.itemsize:
                                ...
                                rec = np.frombuffer(record_data, dtype=shnumpy.dtype)
                                shnumpy.upsert(rec)
                
                    except Exception as e:
                        msg = 'Subscription %s,%s,%s,table,%s error!\n%s' % \
                            (table.database,table.period,table.source,table.tablename,str(e))
