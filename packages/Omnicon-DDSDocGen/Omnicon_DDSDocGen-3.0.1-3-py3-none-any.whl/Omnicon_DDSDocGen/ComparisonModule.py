...
class ComparisonModule:
    ...
    def check_id(new_element: CompElement, origin_element: CompElement):
        if new_element.introspection_element is not None and origin_element.introspection_element is not None:
            if new_element.introspection_element.id != origin_element.introspection_element.id:
                return f"Id changed from {origin_element.introspection_element.id} to " \
                       f"{new_element.introspection_element.id} ;\n"
        return ""
    ...
    def compare_enum_list(self, new_element: CompElement, origin_element: CompElement):
        ...
        for new_enum in new_enum_list:
            ...
            if not is_enum_found:
                ...
                differences += f"Enum '{new_enum.name}' added ;\n"

        for origin_enum in origin_enum_list:
            # Using the implementation of "__eq__" in EnumCompElement to compare enum elements
            if origin_enum not in new_enum_list:
                differences += f"Enum '{origin_enum.name}' deleted ;\n"
    ...
    def compare_comp_elements(self, new_element: CompElement, origin_element: CompElement):
        ...
        if differences == "" and new_element.is_discriminator:
            ...
            # See if need to check introspection element
    ...
    def compare_son_list_element_keys(self, new_son, origin_son):
        if self.is_compare_by_name:
            ...
            if new_son.introspection_element is None or origin_son.introspection_element is None:
                # Not all elements have introspection elements. in which case, answer is False
                return False
            return new_son.introspection_element.id == origin_son.introspection_element.id
        ...
        for i in range(1, len(new_sons_list)):
            if "(D)" in new_sons_list[i].hierarchy:
                ...
                if other_son_index is not None:
                    ...
                    if self.is_compare_by_name:
                        is_changes_found = self.compare_sons(
    ...
    def compare_sons(self, new_son: CompElement, origin_son: CompElement, full_type_name: str):
        is_changes_found = False
        ...
        if new_son.type_kind_name == "union":
    ...
    def compare_root_comp_elements(self, new_element: CompElement, origin_element: CompElement) -> (
            'CompElement', bool):
        """
        SELF HAS TO BE THE NEW AND OTHER IS ORIGIN! other is the new version.
        """
        is_changes_found = False
        ...
        if new_element.user_comment != origin_element.user_comment:
            new_element.user_comment = \
                f"CHANGED - from:\n '{origin_element.user_comment}'\nTo: \n'{new_element.user_comment}'"
            ...
            self.add_changes_to_changes_list(
                new_element.field_name,
                "",
                f"Topic level user comment changed from:\n '{origin_element.user_comment}'\n"
                f"To: \n'{new_element.user_comment}'",
