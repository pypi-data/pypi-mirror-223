...
def describe(args: argparse.Namespace) -> None:
    ...
    if args.collection is None and args.product is None:
        ...
        attributes = {
            ...
            "Published": "none"
            if (product.ingested is False)
            else f"{product.ingested.isoformat(timespec='milliseconds')}Z",
            "MD5": "none" if (product.md5 is False) else product.md5,
        }
        ...
        if product.orbit_is_LEO is True:
            LEO_attributes = {
        ...
        if product.entries is not False:
            lines = ["SIP Entries:"] + [f"\t{filenames}" for filenames in product.entries]
            logger.info("\n".join(lines))
...
def download(args: argparse.Namespace) -> None:
    ...
    safe_run(app, collection=collection, num_products=products_count, force=args.yes)
...
def cli(command_line: Optional[Sequence[str]] = None) -> None:
    ...
    parser_credentials.add_argument("ConsumerKey")
    parser_credentials.add_argument("ConsumerSecret")
    parser_credentials.set_defaults(func=credentials)

    # token parser
    parser_token = subparsers.add_parser(
        "token",
        help="generate an API access token",
    ...
    query_group.add_argument(
        ...
        "--query",
        nargs=1,
        help="Opensearch query",
    )
    query_group.add_argument("-c", "--collection", nargs="+", help="collection ID(s)")
    ...
    parser_download.add_argument(
        "--local-tailor",
        help="Local Tailor Id",
        metavar="LocalTailorId",
    )
    ...
    dir_group.add_argument(
        "--dirs", help="Forces downloading of products into separate folders", action="store_true"
    )
    parser_download.add_argument(
    ...
    parser_local_tailor = subparsers.add_parser(
        "local-tailor",
        description="Manage local-tailor connections",
        help="manage local-tailor connections",
        parents=[common_parser],
    )
    ...
    local_tailor_set_parser = local_tailor_subparsers.add_parser(
        "set",
        description=f"add local-tailor configuration",
        parents=[common_parser],
    )
    ...
    local_tailor_remove_parser = local_tailor_subparsers.add_parser(
        "remove",
        description="remove local-tailor configuration",
        parents=[common_parser],
    )
    ...
    local_tailor_show_parser = local_tailor_subparsers.add_parser(
        "show",
        description="show local-tailor configuration",
        parents=[common_parser],
    )
    ...
    local_tailor_list_parser = local_tailor_subparsers.add_parser(
        "instances",
        description="list local-tailor instance configurations",
        parents=[common_parser],
    )
