...
class aioDeezer:
    def __init__(self, arl, loop:asyncio.BaseEventLoop=None, logger:logging.Logger=None, aio_session:aiohttp.ClientSession=None, *args) -> None:
        self.arl = arl
        self.user = {}
        self.token = None
        self.loop = loop or asyncio.get_event_loop()
        self.logger = logger
        self.cookies = {'arl': arl}

        if aio_session:
            if self.logger: self.logger.info('[INIT] Using a Passed `aiohttp` Session')
            self.web_session = aio_session
            self.web_session.headers.update(networking_settings.HTTP_HEADERS)
            self.web_session.cookie_jar.update_cookies(self.cookies, response_url=URL(api_urls.DEEZER_URL))
        else:
            if self.logger: self.logger.info('[INIT] Creating a Web Session')
            self.web_session = aiohttp.ClientSession(headers=networking_settings.HTTP_HEADERS)
            self.web_session.cookie_jar.update_cookies(self.cookies, response_url=URL(api_urls.DEEZER_URL))

        self.loop.run_until_complete(self.get_user_data(True))
    ...
    async def get_cookies(self):
        ...
        """
        try:
            cookies = self.web_session.cookie_jar.filter_cookies(api_urls.DEEZER_URL)
            if cookies:
    ...
    async def _api_call(self, method, params={}):
        ...
        async with self.web_session.post(api_urls.API_URL, json=params, params={
            "api_version": "1.0",
            "api_token": token,
            "input": "3",
            "method": method
    ...
    async def _legacy_api_call(self, method, params={}):
        ...
        async with self.web_session.get(url, cookies=self.cookies, params=params) as response:
            data = await response.json()
        ...
        return data
    async def get_user_data(self, init=False):
    ...
    async def _get_poster(self, poster_id, size=500, ext="jpg"):
        ...
        async with self.web_session.get(url,cookies=self.cookies) as response:
            if response.status == 200:
                image_bytes = await response.content.read()
                answer["image"] = image_bytes
