...
class GDF():
    ...
    def read_gdf(
        ...
        with open(path, "r") as f:
            ...
            nodedef, edgedef = getdef(nodes), getdef(edges)
            nodes = pd.read_csv(StringIO(nodes), names=nodedef.keys(), dtype=nodedef, header=0, index_col="name")
            edges = pd.read_csv(StringIO(edges), names=edgedef.keys(), dtype=edgedef, header=0)
    ...
    def write_gdf(
        ...
        # Add attribute type to nodes and edges.
        nodes.columns = rename_columns(nodes)
        edges.columns = rename_columns(edges)

        # Write both to GDF file.
        nodes.to_csv(path, index=True, mode="w")
        edges.to_csv(path, index=False, mode="a")
