...
def sympy_to_pystr(sym) -> str:
    ...
    try:
        ...
        if (
            ...
            or isinstance(sym, sp.Symbol)
            or isinstance(sym, sp.Number)
...
def to_sympy(node: isl.AstExpr):
    ...
    if node.get_type() == isl.ast_expr_type.id:
        ...
        if node.op_get_type() == isl.ast_expr_op_type.minus:
            ...
            child_expr = []
            for child in [node.get_op_arg(i) for i in range(node.get_op_n_arg())]:
...
class SympyToPwAff:
    ...
    def __init__(self, space, constants=None):
        ...
        for name, (dt, idx) in var_dict.items():
            result[name] = isl.PwAff.from_aff(zero.set_coefficient_val(dt, idx, 1))
    ...
    def _get_val(self, v):
        if isinstance(v, sp.Integer):
            val = isl.Val(int(v))
            aff_v = isl.Aff.val_on_domain(isl.LocalSpace.from_space(self.space), val)
    ...
    def visit(self, sym_expr):
        ...
        elif isinstance(sym_expr, sp.Mod) and isinstance(sym_expr.args[1], sp.Integer):
