...
from .youtube import getYoutubeLinks
...
def getDomainPart(site):
	for key, sub_config in config.items():
		if key in site and sub_config.get('replace_domain'):
			return sub_config.get('replace_domain')
	return site.split('/')[2]
...
def getLinks(site):
	...
	domain_part = getDomainPart(site)
	links = [formatRawLink(link, domain_part, site) for link in links]
	# dedup, keep order
	links = [link for link in OrderedDict.fromkeys(links) if link]
	if '.douban.' in site:
		return list(getDoubanLinks(site, links, soup))
	if 'www.youtube.com/channel' in site:
		return getYoutubeLinks(soup)
