...
def post_data(
    ...
    """
    .. py:method:: General 'make API request' function for POST requests.
    Assigns headers and builds in retries and logging.
    :param url: str: API endpoint URL
    :param headers: Dict[str, str]: Headers
    :param data: Dict[str, Any]: Request payload/data
    :param files: Optional[Dict[str,FileStorage]: Files to be uploaded
    :param logger: Logger: Logger object for logging
    """
    base_log_record: Dict[str, Union[str, Dict[str, str]]] = dict(route=url, data=data)
    retry_count: int = 0
    ...
    while retry_count <= Config.MAX_RETRIES:
        ...
        try:
            response = requests.post(
                url, headers=headers, data=data, files=files, timeout=None
            )
        except Exception as e:
            response = e
    ...
    raise APIError(response, retry_count, url, data)
