...
# This code is adapted from the Mview code of
# QijingZheng(https://github.com/QijingZheng/VaspPoscarMayavi)
################################################################################
# START
################################################################################
...
def build_super331(poscar, ucell, pos, typeName):
    poscar1 = poscar
    pos_tmp = pos
    #print(pos)
    for ip in range(np.shape(pos)[0]):
            ...
            for j in range(-1, 2, 1):
                for k in range(0, 1, 1):
                    if i == 0 and j == 0 and k == 0:
                        ...
                        #print(pos)
                        pos1 = np.array([pos[ip] + ucell[0]*i + ucell[1]*j + ucell[2]*k])
                        pos2 = tuple(pos1[0])
                        poscar1.append(typeName)
                        poscar1.positions[-1] = pos2
                        pos_tmp = np.append(pos_tmp,pos1, axis=0)
                        #print(ip,i,j,k,pos_tmp)
    #print(pos_tmp)
    return pos_tmp, poscar1

def plot_bond(scene, poscar):
    
    nions = poscar.get_global_number_of_atoms()
    atom_chem_symb = np.array(poscar.get_chemical_symbols())
    #print(atom_chem_symb)
    ...
    # plot the bonds
    ############################################################
    # first, find out the possible combinations 
    type_of_bonds = []
    ...
    for itype in range(n_type_bonds):
        ...
        if ijs.size > 0:
            ...
            p_T[0::2, :] = p_A
            bond_A = scene.mlab.plot3d(p_T[:, 0], p_T[:, 1], p_T[:, 2],
            ...
            p_T[0::2, :] = p_B
            bond_B = scene.mlab.plot3d(p_T[:, 0], p_T[:, 1], p_T[:, 2],
...
def plot_str(filename, scene, supercell):
    # read the POSCAR
...
#    scene.mlab.clf()
    ...
    for itype in range(ntype):
        ...
        typeNo_tmp = atom_type_num[itype]
        # the coordinates for this type
        typePos_tmp = poscar.positions[typeID]
        #build a 3*3*1 supercell, assume the material is 2D
        typePos, poscar1 = build_super331(poscar, ucell, typePos_tmp, typeName)
        typeNo = typeNo_tmp * 9
        
       #print(typePos,typePos_tmp)
