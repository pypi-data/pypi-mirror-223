...
def test_completion_app_log_completion(test_llm_application, caplog):
...
def test_chat_app_log_completion(test_chat_application, caplog):
    test_chat_application.log_completion(
        open_ai_api_request={"request": "foo"},
        open_ai_api_response={"response": "bar"},
        request_attributes={"foo": "bar"},
        response_attributes={"bar": "baz"},
...
def test_completion_api_log_store_log(test_api_client):
    ...
    assert log_store.get_ingestion_endpoint() == "/api/v1/logging/completions"
...
def test_chat_api_log_store_log(test_api_client):
    ...
    assert log_store.queue.get() == {"foo": "bar"}
    assert log_store.get_ingestion_endpoint() == "/api/v1/logging/chat/completions"
    assert log_store.queue.empty()
    assert log_store.consumers[0].running
    assert log_store.consumers[0].func == log_store.consumer_func
