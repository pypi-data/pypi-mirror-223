...
class ResourceType(enum.Enum):
    ...
    It is feasible to accumulate additional resource if needed in order to satisfy
    resource windows, e.g., waiting for a time windows to open. This type of resource
    is prefered compared to non-disposable resourcs, if feasible, due to performance
    reasons.
    """
    NonDisposable = "N"
    """
    ...
    It is **not** feasible to accumulate additional resource if needed in order to
    satisfy resource windows, e.g., hard time windows does not allow waiting.
    """
