...
from copernicus_marine_client.utils import next_or_raise_exception
...
def _select_service_by_priority(
    ...
    if (
        ...
        in dataset_available_service_types
        and command_type == CommandType.SUBSET
        and subset_request is not None
    ):
        best_arco_service_type: CopernicusMarineDatasetServiceType = (
            _get_best_arco_service_type(
                subset_request, first_available_service.uri
            )
        )
...
def _select_dataset_service(
    ...
    return _select_service_by_priority(dataset, command_type, subset_request)
...
def get_dataset_service(
    ...
    if dataset_id:
        ...
        return _select_dataset_service(
            dataset,
            dataset_url,
            force_service_type,
            command_type,
