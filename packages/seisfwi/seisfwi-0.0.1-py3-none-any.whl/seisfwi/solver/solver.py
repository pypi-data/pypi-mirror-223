...
#
#
#   Solver module: a self-contained optimization package
# 
...
class Solver(object):
    '''
        Solver based on Fortran subroutines from SEISCOPE OPTIMIZATION TOOLBOX
    '''

    def __init__(self, niter_max = 20, conv = 1e-8, method = 'LBFGS', 
                 bound = False, lb = None, ub = None, 
                 lbfgs_memory = 5, nls_max=6, debug = False, suffix=''):
        '''
            Initialize the Solver class
        
            Parameters
            ----------
            :param int niter_max: maximum number of iterations
            :param float conv: convergence criterion
            :param str method: optimization method
            :param bool bound: bound constraints flag
            :param np.array lb: lower bound
            :param np.array ub: upper bound
            :param int lbfgs_memory: memory for the LBFGS method
            :param int nls_max: maximum number of linesearch
            :param bool debug: debug flag
            :param str suffix: suffix for history files
        '''

        self.niter_max = niter_max      # max number of iterations
        self.conv = conv                # convergence criterion
        self.method= method             # optimization method
        self.bound = bound              # bound constraints flag 
        self.lb = lb                    # lower bound
        self.ub = ub                    # upper bound
        self.nls_max = nls_max          # max number of linesearch
        self.debug = debug              # debug flag
        self.suffix = suffix            # suffix for history files

        # Default parameters
        self.threshold = 100.           # Tolerance on bound constraints satisfaction
        self.FLAG = 'INIT'              # solver status

        # Initialize linesearch parameters by default
        self.m1 = 1e-4                  # Wolfe conditions parameter 1 (Nocedal value)
        self.m2 = 0.9                   # Wolfe conditions parameter 2 (Nocedal value)
        self.mult_factor = 2            # bracketting parameter (My own value)
        ...
        if self.bound:
            if self.lb is None or self.ub is None:
                raise RuntimeError('lb and ub must be specified for bound constraints')
    
            if np.any(self.lb > self.ub):
                raise RuntimeError('lb must be less than ub')


    def iterate(self, x, fcost, grad, grad_preco = None):
        ...
        if grad_preco is not None:
            grad_preco = np.copy(grad_preco.flatten())

        # Preconditioned Steepest Descent: PSTD
        if self.method == 'SD':
            if grad_preco is None:
                raise AssertionError('grad_preco is required for SD method')
            x = self.PSTD(x, fcost, grad, grad_preco)

        # Preconditioned nonlinear conjugate gradient: PNLCG
        elif self.method == 'CG':
            if grad_preco is None:
                raise AssertionError('grad_preco is required for CG method')
        ...
        # Preconditioned Quasi-Newton l-BFGS method: PLBFGS
            ...
            if grad_preco is None:
                raise AssertionError('grad_preco is required for PLBFGS method')
        ...
        # Preconditioned Truncated Newton method: TRN
            ...
            if grad_preco is None:
                raise AssertionError('grad_preco is required for PTRN method')
    ...
    def descent_LBFGS(self, grad):
        ...
        # Safeguard 
        # norml2_descent = normL2(self.descent)
        # norml2_grad = normL2(grad)

        # if (norml2_descent > 2.0 * norml2_grad):
        #     print("LBFGS Warning: descent direction is too large, reset to -grad")
        #     self.descent[:] = -1. * grad[:]
    ...
    def linesearch(self, x, fcost, grad):
        ...
        if(self.first_ls):
            ...
            if (fcost <= (self.fk+(self.m1*self.alpha*self.q0))) and (self.q >= (self.m2*self.q0)):
                ...
                if(self.debug):
                    print('fcost :', fcost)
                    print('solver.f0    :', self.f0)
                    print('solver.fk    :', self.fk)
                    print('solver.alpha :', self.alpha)
                    print('solver.q     :', self.q)
                    print('solver.q0    :', self.q0)
                    print('m1           :', self.m1)
    ...
    def project(self, x):
        '''
        project the vector x into the box defined by the bound constraints solver.lb and solver.ub 
        '''
    ...
    def std_test_conv(self, fcost):
        ...
        If the current relative cost is lower than a value set by the user in solver.conv
    ...
    def print_info(self, fcost):
        ...
        if(self.FLAG == 'INIT'):
            f = open('iterate_%s_%s.log'%(self.method, self.suffix), 'w')
            if(self.method == 'SD'):
                f.write('********************************************************************************\n')
                f.write('         STEEEPEST DESCENT ALGORITHM         \n')
                f.write('********************************************************************************\n')
                print('********************************************************************************')
                print('         STEEEPEST DESCENT ALGORITHM         ')
                print('********************************************************************************')
                ...
                f.write('********************************************************************************\n')
                print('********************************************************************************')
                print('             PRECONDITIONED l-BFGS ALGORITHM                ')
                print('********************************************************************************')
            ...
            %(self.cpt_iter+1, fcost, ng, fcost/self.f0, self.alpha, self.cpt_ls, self.nfwd_pb))
            f.close()

            print('     Convergence criterion  : %10.2e' %self.conv)
            print('     Niter_max              : %10d'   %self.niter_max)
            print('     Initial cost is        : %10.2e' %self.f0)
            print('     Initial norm_grad is   : %10.2e' %ng)
            print('********************************************************************************')
            print('   Niter      fk         ||gk||       fk/f0        alpha        nls      ngrad   \n')
            print('%6d %12.2e %12.2e %12.2e %12.2e %8d %8d'
            %(self.cpt_iter+1, fcost, ng, fcost/self.f0, self.alpha, self.cpt_ls, self.nfwd_pb))



        elif(self.FLAG == 'CONV'):
            f = open('iterate_%s_%s.log'%(self.method, self.suffix), 'a')
            f.write('%6d %12.2e %12.2e %12.2e %12.2e %8d %8d\n'
            %(self.cpt_iter+1, fcost, ng, fcost/self.f0, self.alpha, self.cpt_ls, self.nfwd_pb))
            f.write('********************************************************************************\n')

            print('%6d %12.2e %12.2e %12.2e %12.2e %8d %8d'%(self.cpt_iter+1, fcost, ng, fcost/self.f0, 
                                                               self.alpha, self.cpt_ls, self.nfwd_pb))
            print('********************************************************************************')
            ...
            if(self.cpt_iter >= self.niter_max):
                ...
                print('  STOP: CONVERGENCE CRITERION SATISFIED        ')
                print('********************************************************************************\n')
    ...
    def print_info_PTRN(solver, fcost, FLAG):
        ''' Print information
        '''
        ...
        if(FLAG=='INIT'):
            ...
            f.write('   Niter      fk         ||gk||       fk/f0          alpha      nls     nit_CG      eta       ngrad     nhess \n')
            f.write('%6d %12.2e %12.2e %12.2e %12.2e %8d %8d %12.2e %8d %8d \n'%(
            solver.cpt_iter, fcost, solver.norm_grad, fcost/solver.f0, solver.alpha, solver.cpt_ls, solver.cpt_iter_CG, solver.eta, solver.nfwd_pb, solver.nhess))
            f.close()
            ...
            f.write('********************************************************************************\n')
            if(solver.cpt_iter==solver.niter_max):
                f.write('  STOP: MAXIMUM NUMBER OF ITERATION REACHED    \n')
            else:
                f.write('  STOP: CONVERGENCE CRITERION SATISFIED        \n')
            f.write('********************************************************************************\n')
            ...
            f.write('********************************************************************************\n')
            f.close()     
        elif(solver.comm=='DES1'):
            f = open('iterate_PTRN_CG.log', 'a')
            f.write('-------------------------------------------------------------------------------------------------\n')
            f.write(' NONLINEAR ITERATION %4d  ETA IS : %12.2e \n'%(solver.cpt_iter, solver.eta))
            f.write('-------------------------------------------------------------------------------------------------\n')
            f.write('  Iter_CG        qk       norm_res      norm_res/||gk||  \n')
            f.write('%8d %12.2e %12.2e %12.2e \n'%(solver.cpt_iter_CG, solver.qk_CG, solver.norm_residual, solver.norm_residual/solver.norm_grad))
            f.close()     

        elif(solver.comm=='DES2'):
            f = open('iterate_PTRN_CG.log', 'a')
            f.write('%8d %12.2e %12.2e %12.2e \n'%(solver.cpt_iter_CG, solver.qk_CG, solver.norm_residual, solver.norm_residual/solver.norm_grad))
            f.close()     
        else:
