...
class NEATViz:
    ...
    def showNapari(self):
        ...
        Y_path = os.path.join(self.csvdir, '.csv')
        Y = glob.glob(Y_path)
        Csvids = []

        self.oneat_widget.frameWidget.imageidbox.addItem("Select Image")
        self.oneat_widget.frameWidget.csvidbox.addItem("Select Csv")
        ...
        for i in range(0, len(Csvids)):
            self.oneat_widget.frameWidget.csvidbox.addItem(str(Csvids[i]))
