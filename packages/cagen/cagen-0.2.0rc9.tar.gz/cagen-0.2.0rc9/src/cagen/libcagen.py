...
class Collection:
    ...
    def select(self, somefunction: Optional[types.FunctionType], value: Optional[str]):
        """
        It return a list of entries.
        By default, it returns all entries
        Else, it returns [entry | somefunction(entry) == value]

        It's up to user to define useful functions for `Entry` collections: such backlinks,
        has_author, etc.
        """
        if somefunction == None:
            return self.entries
        else:
            return [entry for entry in self.entries if somefunction(entry) == value]
