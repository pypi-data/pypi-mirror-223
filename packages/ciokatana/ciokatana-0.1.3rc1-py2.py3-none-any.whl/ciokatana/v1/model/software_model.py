...
INITIAL_VALUE = "{}".format(k.NOT_CONNECTED)  # host-version/plugin-version
...
def create(node):
    ...
    Connected".
    """
    initial_value = _get_valid_paths(INITIAL_VALUE)[0]
...
def _get_valid_paths(current_value):
    ...
    if len(parts) > 0:
        ...
        if len(parts) > 1 and parts[1]:
            all_paths.append(current_value)
