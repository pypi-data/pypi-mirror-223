...
def ResBlock(modifiers, squeeze, augment, layer = None):
    ...
    if flavor == 'vanilla':
        return Vanilla_Block(depth, squeeze, augment, layer)

    elif flavor == 'bottleneck':
        return Bottleneck_Block(depth, squeeze, augment, layer)
...
class Vanilla_Block(Module):
    def __init__(self,  depth, squeeze, augment, layer = None):
        ...
        self.vanilla_block = Sequential(
                Conv2d(in_channels = i_channel, out_channels = o_channel, kernel_size = 3, stride = 1, padding = 1, bias = False, groups = 2),
                BatchNorm2d(o_channel, momentum = moment),
                acti,
                # DropBlock2d(p = 0.25, block_size = 2 ** (5 - layer_depth)),

                Conv2d(in_channels = o_channel, out_channels = o_channel, kernel_size = 3, stride = stride, padding = 1, bias = False, groups = 2),
                BatchNorm2d(o_channel, momentum = moment),
                # DropBlock2d(p = 0.25, block_size = 2 ** (5 - layer_depth)),
            )
        ...
        if layer == 'first' and layer_depth != 0: 
            ...
            self.downsample = Sequential(
                ...
                Conv2d(in_channels = i_channel, out_channels = o_channel, kernel_size = 1, stride = 1, padding = 0, bias = False, groups = 2),
                BatchNorm2d(o_channel, momentum = moment),
                acti,
                # DropBlock2d(p = 0.25, block_size = 2 ** (5 - layer_depth)),
            )
