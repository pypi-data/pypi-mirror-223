#!/usr/bin/env python
# cp from nacp-kvr.py , 2023.7.7    docker run -it --rm --name si -v /data/cikuu/pypi/en:/app wrask/cikuu python /app/sntspacy-si.py all gzjc 
...
def txtdocs(name:str='nju-2020'): #python sntspacy-si.py all nju-2020 --rids true
	for i,rid in enumerate(redis.r.zrange(f"rids:{name}",0,-1)): 
		for j,uid in enumerate(redis.r.zrange(f"riduid:{rid}",0,-1)):
			snts = redis.r.hget(f"htxt:rid-{rid}:uid-{uid}",'snts')
			if not snts: continue
			for k,snt in enumerate(json.loads(snts)): 
				spa = redis.r.hget(f"hsnt:{snt}", 'spacy')
				if spa: yield i * 1000000 + j * 1000 + k, spacy.from_json(json.loads(spa) )
...
class util(object): 
	def __init__(self, host='172.17.0.1'  if not 'Windows' in platform.system() else "files.jukuu.com", port=6666, withsnt:bool=False,interval:int=30) : 
	...
	def tok(self, name,batch:int=10000, rids:bool=False): 
		''' python nacp-kvr.py tok gzjc '''
		print ( 'tok started :', name, redis.r, flush=True)
		fire.ssi = defaultdict(Counter)
		for sid, doc in docs(name) if not rids else txtdocs(name):
			try: #17400000 Recently, owing to the COVID-19 pandemic, technology is ever more widely used and becomes a necessary means of instruction, by which teachers around the world conduct their teaching and give instructions on a variety of platforms or through different tools. 9.82495379447937
				if sid % batch == 0: print ( f"{name}:tok, sid={sid}, tim=", time.time() - fire.start , flush=True) 
				add("#|sntnum") 
				[add(f"LEM|{t.lemma_}", f"LEX|{t.text.lower()}",f"LEM|_sum", f"LEX|_sum") for t in doc if t.is_alpha] #[add(f"pos:LEM|{t.lemma_}", f"pos:LEX|{t.text.lower()}") for t in doc]  # LEM is a kind of pos, just like pos:VERB,pos:NOUN, ... 
				[add(f"pos|{t.pos_}", f"pos:{t.pos_}|{t.lemma_}", f"pos:{t.pos_}:tag|{t.tag_}", f"pos:{t.pos_}:{t.tag_}|{t.text.lower()}", 
						f"{t.pos_}:{t.lemma_}|TAG:{t.tag_}", 	) for t in doc if t.pos_ not in ('PROPN','X', 'PUNCT','NUM','CD','SPACE','SYM') and t.is_alpha  and t.lemma_ in wordlist] #f"{t.pos_}:{t.lemma_}|LEX:{t.text.lower()}",

				# gzjc:LEM:book #f"LEM:{t.lemma_}|LEM:{t.lemma_}", f"LEM:{t.lemma_}|LEX:{t.text.lower()}"
				[add( f"LEMLEX:{t.lemma_}|{t.text.lower()}"   #f"LEMLEX:{t.lemma_}|_sum",
					, f"LEMPOS:{t.lemma_}|{t.pos_}", f"LEMPOSTAG:{t.lemma_}|{t.pos_}:{t.tag_}") for t in doc if t.pos_ not in ('PROPN','X', 'PUNCT','NUM','CD','SPACE','SYM') and t.is_alpha and t.lemma_ in wordlist] #and t.is_lower
		...
		print("tok walking finished:" ) 
		[redis.r.zadd(f"{name}:{k}", v)  for k, v in fire.ssi.items()] # if not k.startswith('lem-lex') 
		# after tok is run,  gzjc:pos:VERB
		redis.r.zadd(f"{name}:#", {"wordsum": sum([i for s,i in redis.r.zrevrange(f"{name}:LEX",0,-1, True)]), "sntnum": redis.r.get(f"hsnt:{name}:#"), }) 
		for pos in ('NOUN','ADJ','ADV','VERB'):
			for lem, cnt in redis.r.zrevrange(f"{name}:pos:{pos}",0,-1, True):
				if lem.isalpha(): 
					redis.r.zadd(f"{name}:{pos}:{lem}", {"_sum": cnt})
		print("tok dumping finished:" ) 

	def trp(self, name,batch:int=10000, rids:bool=False): 
		''' python nacp-kvr.py trp gzjc '''
		print ( 'started trp:', name, redis.r, flush=True)
		fire.ssi = defaultdict(Counter)
		for sid, doc in docs(name)  if not rids else txtdocs(name): 
			if sid % batch == 0: print ( f"{name}:trp, sid={sid}, tim=", time.time() - fire.start , flush=True) 
			try:
				for t in doc: 
	...
	def kp(self, name,batch:int=10000, rids:bool=False): 
		...
		for sid, doc in docs(name)  if not rids else txtdocs(name): 
			...
			try:
				...
				exc_type, exc_value, exc_traceback_obj = sys.exc_info()
				traceback.print_tb(exc_traceback_obj)
		print("kp walking finished:" ) 
		for k, v in fire.ssi.items():  redis.r.zadd(f"{name}:{k}", v) 
		print("kp dumping finished:" ) 

	def all(self, name, rids:bool=False):
		start = time.time() 
		print ( 'start to delete:', name, flush=True) 
		[ redis.r.delete(k) for k in redis.r.keys(f"{name}:*")]
		self.tok(name, rids=rids)
		self.trp(name, rids=rids)
		self.kp(name, rids=rids)
		...
		for pos in ('NOUN','VERB','ADJ','ADV'): 
			...
			redis.r.zadd(f"{name}:{pos}:_avg", dict(si)) #redis.exceptions.DataError: ZADD requires at least one element/score pair
...
def docs(name:str='gzjc'):
	for i in range(redis.r.zcard(f"sntzset:{name}")): 
		snts = redis.r.zrange(f"sntzset:{name}", i, i, withscores=True)
		if snts: 
			snt, sid = snts[0] 
			v = redis.r.get(f"sntzset:{name}:spacy:{int(sid)}")  
			yield i, spacy.from_json(json.loads(v) )  if v else None 
