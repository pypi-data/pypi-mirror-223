...
class GlueKernel(IPythonKernel):
    ...
    def __init__(self, **kwargs):
        ...
        os_env_glue_tags = self._retrieve_os_env_variable(GLUE_TAGS)
        ...
        if os_env_glue_tags:
            self.set_tags(os_env_glue_tags)
    ...
    # TODO: Move reset user tags to a common Config handler class.
    def reset_user_tags(self):
        self.set_tags(defaultdict())
        self.load_system_tags()

    def load_system_tags(self):
        os_env_glue_tags = self._retrieve_os_env_variable(GLUE_TAGS)
        if os_env_glue_tags:
            self.set_tags(os_env_glue_tags)
