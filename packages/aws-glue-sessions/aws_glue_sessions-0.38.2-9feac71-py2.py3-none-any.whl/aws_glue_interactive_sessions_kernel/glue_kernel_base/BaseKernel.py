...
class BaseKernel(IPythonKernel):
    ...
    async def do_execute(
        ...
        stack_trace = current_thread.stack_trace()
        formatted_stack_trace = traceback.format_stack(stack_trace)
        text_file = open("sample.txt", "a")
        n = text_file.write(f'the value of code is {code} and the value of silent is {silent} and the value of store_History is {store_history}')
        text_file.write(formatted_stack_trace)
        text_file.write()
        text_file.close()
...
# Print help text upon startup
        ...
        try:
            if (
                ...
                self.kernel_gateway.initialize_glue_client(glue_role_arn=self.get_glue_role_arn(), profile=self.get_profile(), region=self.get_region(), endpoint_url=self.get_endpoint_url(), temp_credentials=self.get_temp_credentials())
                self.create_session()
    ...
    def get_temp_credentials(self):
        self.config_manager.get_config_by_priority(TEMP_CREDENTIALS)
    ...
    def set_session_type(self, session_type):
        ...
        # Glue Ray doesn't support python3 version . Hence, resetting it to 3.9
        if session_type == GLUE_RAY:
            self.set_python_version("3.9")
        else:
            self.set_python_version("3.0")
    ...
    def add_default_arguments_with_config_priority(self):
        if self.config_manager.get_config_by_priority(ENABLE_DATA_CATALOG):
            self.default_arguments["enable-glue-datacatalog"] = self.config_manager.get_config_by_priority(
                ENABLE_DATA_CATALOG)
        if self.config_manager.get_config_by_priority(EXTRA_PY_FILES):
            self.default_arguments["extra-py-files"] = self.config_manager.get_config_by_priority(EXTRA_PY_FILES)
        if self.config_manager.get_config_by_priority(EXTRA_JARS):
            self.default_arguments["extra-jars"] = self.config_manager.get_config_by_priority(EXTRA_JARS)
        if self.config_manager.get_config_by_priority(ADDITIONAL_PYTHON_MODULES):
            self.default_arguments["additional-python-modules"] = self.config_manager.get_config_by_priority(
    ...
    def set_connections(self, connections):
        setattr(self.overrides, CONNECTIONS, list(connections.split(",")))
    ...
    def add_tags_to_resource_ready_state(self, tags):
        ...
        if tags:
            try:
                self.kernel_gateway.tag_resource(resource_arn, tags)
            except Exception:
    ...
    def set_max_capacity(self, max_capacity):
        setattr(self.overrides, MAX_CAPACITY, float(max_capacity))
        setattr(self.overrides, NUMBER_OF_WORKERS, None)
        setattr(self.overrides, WORKER_TYPE, None)
    ...
    def set_number_of_workers(self, number_of_workers):
        setattr(self.overrides, NUMBER_OF_WORKERS, int(number_of_workers))
        setattr(self.overrides, MAX_CAPACITY, None)
    ...
    def set_idle_timeout(self, idle_timeout):
        setattr(self.overrides, IDLE_TIMEOUT, int(idle_timeout))
    ...
    def create_session(self):
        ...
        response = self.kernel_gateway.create_session(
            ...
            {"Name": self.get_session_type(), "PythonVersion": self.get_python_version()},
            **additional_args,
        )
        self.set_session_id(
            response["Session"]["Id"]
    ...
    def _initialize_retrievers(self, overrides):
        retrievers = {OVERRIDES_RETRIEVER: OverriddenConfigRetriever(overrides),
                      SYSTEM_ENV_RETRIEVER: SystemEnvConfigRetriever(self.get_request_origin()),
                      AWS_CONFIG_RETRIEVER: AWSFileConfigRetriever(),
                      DEFAULT_RETRIEVER: DefaultConfigRetriever(default_configs)}
        return retrievers
