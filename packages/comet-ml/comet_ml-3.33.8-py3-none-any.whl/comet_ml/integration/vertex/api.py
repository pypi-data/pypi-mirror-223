...
def initialize_comet_logger(
    ...
    [CometVertexPipelineLogger.track_task](#comet_mlintegrationvertexcometvertexpipelineloggertrack_task),
    you don't need to use this function anymore.


    Args:
        experiment: An already created Experiment object. pipeline_job_name: string. The Vertex
        pipeline job name, see below how to get it automatically from Vertex. pipeline_task_name:
        string. The Vertex task name, see below how to get it automatically from Vertex.
        pipeline_task_uuid: string. The Vertex task unique id, see below how to get it automatically
        from Vertex.

    For example: ```python @kfp.dsl.v2.component def my_component() -> None:
...
def comet_logger_component(
    ...
    base_image=None,  # type: Optional[str]
    ...
    Deprecated: Use [CometVertexPipelineLogger](#cometvertexpipelinelogger) instead.
