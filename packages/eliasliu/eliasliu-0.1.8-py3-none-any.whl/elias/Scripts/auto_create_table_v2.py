...
def create_table(target_db_type, target_db, target_table, map_column_info):
    if target_db_type == "mysql":
        ...
        try:
            ...
            if cursor.fetchone()[0] <= 0:
                ...
                if orig_columns == map_column_info:
                    ...
                    logger.info(
                        f"Table {target_db}.{target_table} already exists but has different columns and types, so drop table {target_db}.{target_table}...")
                    # Table exists with different columns or types, drop it
                    cursor.execute(f"DROP TABLE {target_db}.{target_table}")
                    logger.info(f"Executing SQL to create table {target_db}.{target_table}:")
                    create_table_sql = "CREATE TABLE {}.{} (\n    {}\n)".format(target_db, target_table, ',\n    '.join(
                        [f"{col[0]} {col[1]}" for col in map_column_info]))
                    logger.info(f"Table {target_db}.{target_table} created by:{create_table_sql}")
                    cursor.execute(create_table_sql)
                    logger.info(f"Table {target_db}.{target_table} created successfully.")
        except Exception as e:
        ...
        odps_columns = [(col_info[0], col_info[1]) for col_info in map_column_info]
        ...
        create_table_sql = "CREATE TABLE {}.{} (\n    {}\n)".format(target_db, target_table, ',\n    '.join(
            [f"{col[0]} {col[1]}" for col in odps_columns]))

        try:
            # Check if the table exists

            orig_columns = get_columns("maxcompute", target_db, target_table)

            logger.info(f"Table {target_db}.{target_table} already exists.")

            # Drop the existing table if it exists and column information matches
            if orig_columns == odps_columns:
                logger.info(
                    f"Table {target_db}.{target_table} already exists with the same columns and types. Skipping table creation.")
                ...
                logger.info(
                    f"Table {target_db}.{target_table} already exists but have different columns and types,so drop table {target_db}.{target_table}...")
                logger.info(f"Executing SQL to drop table {target_db}.{target_table}:")
            ...
            # If the table doesn't exist, log the message and continue
            if "Table" in str(e) and "table not found" in str(e):
                logger.info(f"Table {target_db}.{target_table} does not exist. Proceeding with table creation...")
                # Execute the CREATE TABLE SQL statement
                logger.info(f"Executing SQL to create table {target_db}.{target_table}:")
                logger.info(f"Table {target_db}.{target_table} created by:{create_table_sql}")
...
def gen_datax_json_config(sourcedbtype, sourcedb, sourcetable, targetdbtype, targetdb, targettable, filename):
    ...
    if sourcedbtype == "mysql" and targetdbtype == "maxcompute":
        ...
        if target_column_names[-1].lower() == "mc_create_at" and source_column_names[-1].lower() != "mc_create_at":
            source_column_names.append("now()")
        split_pk_field = get_primary_key("mysql", sourcedb, sourcetable)
        job = {
            "job": {
                "setting": {
                    "speed": {"channel": 6},
                ...
                "content": [{
                    ...
                    "writer": {
                        ...
                        "parameter": {
                            ...
                            "password": mysql_passwd,
                            "preSql": [
                                f"truncate table `{targetdb}`.`{targettable}`"
                            ],
                            "column": target_column_names,
                            "connection": [{
                ...
                "content": [
                    {
                        ...
                        "writer": {
                            ...
                            "parameter": {
                                ...
                                "batchSize": 65536,
                                "column": target_column_names,
                                "preSql": [f"truncate table `{targetdb}`.`{targettable}` "],
                                "connection": [
                                    {
...
# 封装获取主键信息的函数
def get_primary_key(dbtype, db, table):
    try:
        connection = get_connection(dbtype, db)
        cursor = connection.cursor()
        sql = f"SHOW KEYS FROM `{db}`.`{table}` WHERE Key_name = 'PRIMARY';"
        cursor.execute(sql)
        keys_info = cursor.fetchall()
        if keys_info:
            primary_key = keys_info[0][4]
            return primary_key
        else:
            logger.info(f"Primary key not found for table '{table}' in database '{db}'.")
            return ''
    except Exception as e:
        raise ValueError(f"Failed to retrieve primary key for table '{table}' in database '{db}': {str(e)}")
...
def main(args):
    try:
        ...
        create_table(args.targetdbtype, args.targetdb, args.targettable, mapped_column_info)
        ...
        logger.info(f"python /opt/module/datax/bin/datax.py {filename}")
