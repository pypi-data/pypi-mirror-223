...
class scraper:
    ...
    def get_sub_sitemaps(self, sitemap_url_list, filter_string=None, speedlimit=1000):
        try:
    ...
    def get_rendered_page(self, url):
        try:
            ...
            print(f"Error Getting Rendered Page: {e}")
            return None
    ...
    def get_response(self, get_url, proxy=None):
        try:
            if proxy:
            ...
            print(f"Get Response Error: {e} URL: {get_url}")
            return None

    def get_json_request(self, get_url, proxy=None):
        try:
            if proxy:
                response = self.session.get(get_url, timeout=30, proxies={"http": proxy, "https": proxy},
                                       headers={'User-Agent': random.choice(self.user_agents_list)})
            ...
            print(f"Get Json Request Error: {e} URL: {get_url}")
            return None
