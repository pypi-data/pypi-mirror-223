...
class scraper:
    ...
    def get_links_from_sitemaps(self, sitemap_urls, filter_string=None, speedlimit=1000):
        ...
        try:
            ...
            logger.warning(f"Scraper Util - Warning Getting Links From Sitemap: {e}")
    ...
    def get_rendered_page(self, url):
        try:
            ...
            logger.warning(f"Scraper Util - Warning Getting Rendered Page: {e}")
            return None
    ...
    def get_rendered_page_json(self, url):
        try:
            ...
            logger.warning(f"Scraper Util - Warning Getting Rendered Page Json: {e}")
            return None
    ...
    def get_response(self, get_url, proxy=None, debug=False):
        try:
            ...
            if debug:
                logger.debug(" ")
                logger.debug(f"ScraperUtil - Get Response Debug: URL: {get_url}")
            ...
            logger.warning(" ")
            logger.warning(f"ScraperUtil - Get Json Request Warning: {e} URL: {get_url}")
            logger.warning("---------------Start Response----------------")
    ...
    def get_json_request(self, get_url, proxy=None, debug=False):
        try:
            if proxy:
                ...
                response = self.session.get(get_url, timeout=30, headers={'User-Agent': random.choice(self.user_agents_list)})

            if debug:
                logger.debug(" ")
                logger.debug(f"ScraperUtil - Get Json Request Debug: URL: {get_url}")
                logger.debug("---------------Start Response----------------")
                logger.debug(response)
                logger.debug("---------------End Response----------------")
                logger.debug(" ")
