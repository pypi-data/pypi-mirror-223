...
logging.basicConfig(filename=f"{format(workingdir)}/scraperutil.log", format='%(asctime)s %(message)s',filemode='w')  # Create and configure logger
...
class scraper:
    ...
    def get_response(self, get_url, proxy=None, debug=False):
        try:
            ...
            if debug:
                logger.warning(" ")
                logger.warning(f"Get Json Request Error: {e} URL: {get_url}")
