...
def finetune_runner(component_args: Namespace) -> None:
    ...
    if component_args.model_family == MODEL_FAMILY_CLS.HUGGING_FACE_IMAGE and master_process:
        # save the license file in the pytorch and mlflow model folders
        save_license_file(
            model_name_or_path=component_args.model_name_or_path,
            license_file_name=ModelSelectorDefaults.LICENSE_FILE_NAME,
            destination_paths=[component_args.pytorch_model_folder, component_args.mlflow_model_folder]
        )
