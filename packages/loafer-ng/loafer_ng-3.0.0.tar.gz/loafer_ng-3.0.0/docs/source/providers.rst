...
To use exponential backoff factor options must contain :code:`'BackoffFactor': 1.5` and optionally :code:`'VisibilityTimeout': 30` (30 is used as default)
Whenever exponential backoff is used, :code:`'AttributeNames': ['ApproximateReceiveCount']` is attached to receive message options (if not already in there) as is required to calculate visibility timeout.
