...
def raise_from_response(response: Response):
    ...
    try:
        raise KeycloakError(
            status_code = response.status_code, reason = response.json()
        )
...
class FastAPIKeycloak:
    ...
    def admin_token(self, value: str):
        ...
        if not decoded_token.get("resource_access") or not decoded_token.get("resource_access").get(
                "realm-management"
    ...
    def get_current_user(
        ...
        def current_user(
                token: str = Depends(self.user_auth_scheme)
        ) -> BaseModel:
    ...
    def open_id_configuration(self) -> dict:
        ...
        if not is_response_valid(response):
            raise KeycloakError(status_code = response.status_code,
    ...
    def _perform_user_roles_operation(self, roles: list[str], user_id: str, method: HTTPMethod):
        keycloak_roles = self.get_roles(roles)
        if len(keycloak_roles) == 0:
            raise HTTPException(status_code = 404, detail = f"None of the roles {roles} is not found")
    ...
    def get_user(self, user_id: str = None, query: str = ""):
        ...
        def impl():
            if user_id is None:
                ...
                return self._user_model(**response.json())
